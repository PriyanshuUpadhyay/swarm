...
- Use `NavigationStack`, `NavigationSplitView` and the modern `Tab` API where those containers
  match the screen. Swarm also has intentional custom workspace and window navigation.
...
- Generated asset symbols require build support. Swarm builds through SwiftPM and custom scripts;
...
- SwiftUI's WebKit views can replace some wrappers on macOS 26. Compare delegate, navigation and
  embedding requirements before changing Swarm's existing web view bridge.
