...
are fine when they belong to the same subject. Follow Swarm's one-subject-per-file guidance rather
...
Choose `TextField(axis: .vertical)` for suitable short multiline input. Use `TextEditor` or Swarm's
