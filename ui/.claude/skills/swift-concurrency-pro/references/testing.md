...
[testing skill](../../swift-testing-pro/SKILL.md) for Swarm's test commands and isolation rules.
...
- For suspected data races, use the existing nightly Thread Sanitizer job or a targeted run with
  `SWARM_UI_TEST_SWIFT_ARGS='--sanitize=thread'`. TSan cannot detect every logical actor-reentrancy bug.
