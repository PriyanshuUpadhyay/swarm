...
public struct CommandDisplay: Equatable, Sendable {
    ...
    private static func step(_ text: Substring, from directory: String) -> Step {
        ...
        // A word Swarm would have to run a shell to understand, and `cd -`, whose destination only
    ...
    /// Relative resolves against the worktree, because that is the directory every agent Swarm
    /// runs is spawned in.
