...
/// `.swarm/attachments/9JVKW4/shot.png` and fix the spacing" reaches it as one thought instead of
...
/// self-delimiting, so `.swarm/attachments/9JVKW4/Pasted 2026-08-20 at 22.29.20.png` survives its
...
///
/// **What counts as one.** Either the path is Swarm's own copy, under `.swarm/attachments`, which
...
public struct AttachmentDraft: Equatable, Sendable {
    ...
    /// The folder every attachment Swarm copies into a worktree is written to. A path under it is
    ...
    /// Swarm's own folder in somebody's checkout. A file inside it is a file this app either wrote
    /// or was pointed at, and never one the user happened to name in a sentence.
    public static let folder = ".swarm/"

    /// Whether a backticked run names a file this draft is carrying.
    ///
    /// Two ways to be one. Either it is a file in Swarm's own folder, which needs no telling: a
    /// copy under `.swarm/attachments`, the pull request instructions under `.swarm/scratch`, or
    /// the project's own `.swarm/pr-instructions.md`. That is what lets a sent turn draw the same
    ...
    /// path the agent cannot read only teaches it that Swarm lies about paths.
