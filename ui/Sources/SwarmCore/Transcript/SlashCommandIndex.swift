...
/// Finds commands and skills for Swarm's shared slash menu. Codex's own list supplies its
...
public enum SlashCommandIndex {
    ...
    /// mean nothing in a Swarm turn: `/vim`, `/terminal-setup`, `/statusline` and their like would
    /// be offers that go nowhere. So this is only the ones that are a prompt in their own right
    /// and that a Swarm session can actually carry out. Adding to it is a decision, not a sweep.
