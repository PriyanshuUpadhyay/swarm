...
///
/// Swarm writes paths into the messages it sends on the owner's behalf, in a code span, which is
/// how `AttachmentDraft` writes every path in a prompt: "Follow the instructions in
/// `.swarm/scratch/pr-instructions.md`." A path is a file everywhere else in this window (the
...
/// it governs what gets sent; it says yes only to Swarm's own folder and to paths the composer
...
public enum FileMention: Sendable {
    ...
    ///
    /// - A span with a `/` in it is a path. `.swarm/scratch/pr-instructions.md`,
    ///   `Sources/Swarm/Views/Palette.swift`, `app/Http/Kernel.php`. Nothing that reads as prose
    ...
    /// - `app`, for `model.app`, which is written in this repository every day. `Swarm.app` loses
    ...
    /// extension there, so `Info.entitlements` and `Swarm.xcodeproj` never reach this set.
