...
///
/// Swarm asks the coding agent to do the work rather than shelling out itself, because the agent
...
public enum PromptRegistry {
    ...
    ///
    /// The agent does the pushing and the `gh` call itself, rather than Swarm shelling out, so it
    /// can follow the project's own commit and pull request conventions. How it does that lives in
    /// `.swarm/pr-instructions.md`, or in Swarm's own copy of that file when the project has none,
    ...
    static let createPullRequest = PromptDefinition(
        ...
        summary: """
        Sent when you press Create pull request, with the project's `.swarm/pr-instructions.md` \
        attached.
        """,
        variables: [
    ...
    ///
    /// The agent commits rather than Swarm, for the same reason it opens the pull request rather
    /// than Swarm: a commit needs a message, the agent is the only party here that knows what it
    /// changed and how this project words a commit, and a message Swarm invented would be a lie
    /// in the repository's history forever. Swarm knows only that something is uncommitted.
    ...
    ///
    /// A turn rather than a note, and that is the one place Swarm deliberately parts company with
    /// Conductor, which attaches its equivalent to the composer as a file and lets the user's next
    /// message carry it. Swarm sends it, for two reasons. The session is the thing being kept
    ...
    ///
    /// Swarm used to run `gh pr merge` itself and then `git push --delete` behind it. It does not
    ...
    /// everybody, in `.swarm/conflict-instructions.md` or in the project settings window, and
    /// Swarm attaches that after Swarm's own and says it wins. See `ProjectInstructions`.
    ...
    /// reader rather than for the agent. Swarm's transcript rows belong to the chat they were
