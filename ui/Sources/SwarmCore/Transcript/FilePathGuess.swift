...
public enum FilePathGuess {
    ...
    ///
    /// Accepts `HarbourMap.php`, `Sources/Swarm/Views/Palette.swift`, `/Users/x/notes.md`.
    ...
    /// - directories, because a directory has no extension to recognise, and `Sources/Swarm` and
    ///   `Sources/Swarm.swift` are told apart by nothing else
    ...
    ///
    /// Swarm opens a file by handing the review a path relative to the worktree, and an agent
    ...
    ///
    /// A relative path is taken at face value. Every agent Swarm runs is spawned with the worktree
