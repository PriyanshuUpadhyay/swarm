...
public struct SubagentRow: Sendable, Hashable, Identifiable {
    ...
    public init(_ subagent: Subagent, now: Date = Date()) {
        ...
        // Swarm itself stored when there is no file. See `SubagentTranscript.live(streamLines:sessionID:)`.
        //
    ...
    /// **The seconds are Swarm's own count, raised by the CLI's when the CLI is counting.**
    ...
    /// running row said nothing and did not move. Swarm knows when `task_started` landed, which
