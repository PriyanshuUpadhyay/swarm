...
/// asserting on, and this file's own note said they would move into `SwarmCore` once the sending
...
public enum PromptAttachments {
    ...
    ///
    /// Inside, rather than in Swarm's application support directory, because the agent has to be
    /// able to read it. Claude Code will not read a path outside its working directory without
    /// asking, Swarm has nothing that can answer that question, and a turn that stalls on an
    /// unanswerable permission prompt is worse than no attachments at all. Inside the worktree the
    /// read is ordinary.
    ///
    /// Under `.swarm`, which is already Swarm's corner of a repository, and made invisible to git
