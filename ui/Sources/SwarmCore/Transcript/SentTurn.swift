...
/// Instructions Swarm put into a turn it composed, in the words the agent was actually handed.
///
/// Swarm writes four kinds of instruction into the turns it sends on the owner's behalf, and
...
///
/// Three runs, and the last two are one thing said two ways. A file Swarm named in the sentence is
/// a path the reader can point at; a block of instructions Swarm put in the message itself is
...
/// was that he could hover one and not the other. Two drawings of "here is what Swarm added to
...
///
/// The parsing is a pure function of the stored text and of the constants Swarm appends, so it
...
public enum SentTurn {
    ...
    /// The title a file Swarm named in its own turn is drawn under, or nil for any other file.
    ///
    ...
    ///
    /// Matched on the exact paths Swarm writes into a turn, and nothing looser, so a file somebody
    ...
    /// Every block Swarm ever appends whole, as the constant it is appended from.
    ///
    ...
    ///
    /// `retiredDefaults` is in here for the reason it exists at all: a turn sent by an older Swarm
    ...
    private static func firstBlock(in text: String, from start: String.Index) -> Found? {
        ...
        // A project's own words are not a constant, so what is recognised is the sentence Swarm
    ...
    ///
    /// Swarm joins the parts of a turn with a blank line and never with anything else, so a match
    /// that starts mid sentence is somebody quoting rather than Swarm appending. It is the same
