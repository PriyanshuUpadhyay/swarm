...
/// A few lines the owner wrote and keeps typing again, kept by Swarm so they can be put back in
/// the box with two keystrokes.
///
/// Swarm's own row rather than a file in `.claude/commands`, and that is the decision this type
...
/// two lists they came out of. The price of the row is that a quick prompt is Swarm's alone: it
...
public struct QuickPrompt: Identifiable, Sendable, Hashable {
    ...
    /// stored row must not change what it does because Swarm learned a new column.
