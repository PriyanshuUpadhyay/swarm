...
public struct Subagent: Sendable, Hashable, Identifiable {
    ...
    /// When `task_started` arrived, by Swarm's own clock, so a row can count its own seconds when
    ...
    /// and Swarm's own otherwise, because a fan-out that produces no ticks at all is a real thing
...
/// and running ones are kept, and only the process going away ends one Swarm was not told about.
///
...
/// already and Swarm does not own that file; and a table would mean a migration, a delete pass and
