...
///
/// The settings a repository has are already stated in files: `<repo>/.swarm/settings.toml` and
/// its `.local` sibling, plus the `.conductor` pair of the same shape, which is where a
/// repository already configured for Conductor keeps them. Swarm reads all four. The obvious way
/// to make the screen editable is to keep overrides in Swarm's own database and layer them on
...
///   A value only Swarm's database knows is a value nothing else can honour.
/// * "Edit it in the app, then look at the file" has to show the change. That is only true if the
///   app edits the file.
///
/// So the app edits the files, and the only question left is which one. The answer is: **Swarm's
...
///
/// The one value that is not edited where it was found is one that came from `.conductor`. Swarm
/// reads that folder so a repository already set up for Conductor works here with nothing to
/// configure; it does not follow that Swarm should maintain another product's file. So the edit
/// goes to `.swarm`, which outranks it, and the old line is left exactly as the team committed
...
public enum SettingsWriter {
    ...
    ///
    /// Always one of Swarm's own two files. A value read out of `.conductor/settings.toml` is not
    /// edited where it was found: Swarm reads that folder so an existing Conductor repository
    /// works with nothing to configure, which is a courtesy on the way in and not a claim on the
    /// folder. Writing there would make Swarm quietly maintain another product's configuration.
    ///
    /// A value only a machine-wide file states is not edited in place either, for the older
    /// reason: this screen is about one repository, and rewriting `~/.swarm/settings.toml` from it
    /// would change every other repository on the machine.
    ///
    /// The tier is kept. A value that came from a `.local` file is written to Swarm's `.local`
    ...
    /// `.swarm/settings.toml` would leave the old value still winning, and the edit would look
    /// like it did nothing.
    public static func destination(for key: SettingsKey, in settings: RepoSettings, repo: String) -> String {
        ...
        return swarmFile(matching: origin, repo: repo)
    }

    /// Swarm's file at the same tier as `path`: shared for shared, `.local` for `.local`.
    static func swarmFile(matching path: String, repo: String) -> String {
        let name = (path as NSString).lastPathComponent
        return (repo as NSString).appendingPathComponent(".swarm/\(name)")
    }
    ...
    public static func defaultFile(repo: String) -> String {
        (repo as NSString).appendingPathComponent(".swarm/settings.toml")
    }

    /// Swarm's folder in a repository, made ready the first time something is written into it.
    ///
    /// `.swarm` is meant to be committed, and that is the whole point of it. `settings.toml` and
    ...
    ///
    /// The distinction matters because `.swarm` is routinely brought into existence by something
    /// that has no business writing the team's ignore rules: `WorktreeScratch` makes
    /// `.swarm/attachments` and `.swarm/scratch` inside it, and those folders must add nothing to
    ...
    static func prepareFolder(for path: String, repo: String) {
        ...
        guard folder == (repo as NSString).appendingPathComponent(".swarm") else { return }
    ...
    ///
    /// A program does; one command does not. `npm run dev` inside `.swarm/run-dev.sh`, under a
    ...
    /// `.swarm/.gitignore` keeps `*.local.sh` out of `git` for the same reason it keeps
    ...
    static func defaultScriptPath(for location: ScriptLocation, local: Bool) -> String {
        ...
        return ".swarm/\(name)\(local ? ".local" : "").sh"
    }
    ...
    ///    it is not quietly repointed at Swarm's own name for it, and a file that has gone missing
    ...
    public static func scriptFile(
        ...
        // not Swarm's call to make from a cleared text box.
    ...
    ///
    /// Only ever true because Swarm writes to `.swarm` and reads `.conductor` as well. Deleting a
    /// line from `.swarm/settings.toml` would leave the `.conductor` one showing through, so
    ...
    static func apply(
        ...
        // keep it in a file of its own already has `.swarm/merge-instructions.md`, which wins.
    ...
    private static func writeRunScripts(
        ...
        for script in scripts {
            ...
            // Only when it says something the id does not. `name` is Swarm's own key, and writing
