...
public actor Store {
    ...
    /// paragraph of prose.** The directory was the constant "Swarm", so every process that
    /// reached `defaultPath` without `SWARM_UI_DB_PATH` opened the real database: the owner's
    /// projects, the real worktree paths, the tmux socket derived from that path. CLAUDE.md and
    /// `Tools/dev-build.sh` both warned about one route into that, `Swarm Dev.app/Contents/MacOS/
    /// Swarm` started by hand, since `LSEnvironment` is applied by LaunchServices and not by a
    /// shell. Nothing warned about the other one, which is `swift run` or `.build/debug/Swarm`,
    ...
    /// points `SWARM_UI_DB_PATH` at, so a hand started dev binary lands where it was always meant to
    ...
    public static func databaseDirectoryName(forBundleIdentifier identifier: String?) -> String {
        ...
        // An executable that is not inside a bundle at all: `swift run`, `.build/debug/Swarm`, or
    ...
    public static func defaultPath() throws -> String {
        ...
        let override = [environment["SWARM_UI_DB_PATH"]].compactMap { $0 }
            .first { !$0.isEmpty }
        if let override {
        ...
        let destination = directory.appendingPathComponent("swarm.sqlite")
        ...
        // A dev copy has never adopted it either, because `SWARM_UI_DB_PATH` returns above this line.
    ...
    private nonisolated static func migrate(_ db: SQLiteDatabase) throws {
        let migrations: [Migration] = [
            ...
            // to survive: quitting Swarm with three of them waiting, a turn that fails instead of
            ...
            //
            // **This is the `deliveries` table in `swarm-handover/mcp-design.md`, laid down here
            ...
            // Swift of Swarm's is on the stack for. There is no UPDATE trigger because a message
            ...
            // setup script put up, because it is what makes `$SWARM_UI_PORT` the same number in both.
            //
            // Zero rather than NULL, and no backfill. Zero already means "no block yet" in the
            // Swift value and in `$SWARM_UI_PORT`, so every row that existed before this reads as a
            ...
            // `PromptOverrides` get away with a key each because their set is closed and Swarm
            // wrote it; this list grows, is renamed and is deleted from, and every growing list in
            // Swarm is a row. It hangs off nothing: there is one flat global list, so there is no
            ...
            // The chat that belongs to no workspace: Ask Swarm, which has a transcript, a cost and
    ...
    /// lost the icon Swarm had just found for it, or got its old name back, or its old colour.
    ///
    ...
    /// three files and the two beside `swarm.sqlite` are a log that gets checkpointed away. The
    ...
    public func compactDatabase() throws {
        ...
        // `swarm.sqlite-wal` and the main file only shrinks when a checkpoint moves it across.
    ...
    /// than counted in memory, so the answer survives Swarm being reopened while children are
    ...
    ///
    /// Ask Swarm's, and nothing else today. It is a separate method rather than a nil argument to
    ...
    /// Swarm died would come back claiming to be waiting on a question whose process is long gone:
    ...
    /// Quitting Swarm with a crew working and reopening it used to bring back a set of dead rows,
    ...
    /// would draw live buttons that write into a closed pipe. Swarm denies explicitly on the way
    ...
    /// Nil when the stored bytes will not decode. A row Swarm cannot read is a question it cannot
    ...
    private static func session(from row: Row) -> Session {
        Session(
            ...
            // A null here is a chat with no worktree, which is Ask Swarm. See `Session.workspaceID`.
