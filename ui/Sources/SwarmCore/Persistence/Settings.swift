...
public struct RepoSettings: Sendable, Hashable {
    ...
    /// What this project adds to the turn Swarm sends when someone presses Merge, and the one it
    /// sends for Fix merge conflicts. Swarm's own words are not here and never were: for merging
    /// they are in the message it composes, and for conflicts they are the file
    /// `ConflictInstructions` writes. These are the project's, they go after Swarm's, they win
    ...
    /// `.swarm/merge-instructions.md` beat these.
    public var mergeInstructions: String?
    public var conflictInstructions: String?
    /// What a browser pane opens on, before expansion, or nil for the port Swarm allocated.
    ///
    /// Stated with the variables a script is handed, so one line covers every workspace:
    /// `http://localhost:$SWARM_UI_PORT/admin`. A workspace whose setup script wrote an address of
    ...
    /// Set by a machine-wide file (`~/.conductor/settings.toml`, `~/.swarm/settings.toml`).
    ///
...
public enum SettingsLoader {
    ...
    public static func repoPaths(repo: String) -> [String] {
        [
            ...
            "\(repo)/.swarm/settings.toml",
            "\(repo)/.conductor/settings.local.toml",
            "\(repo)/.swarm/settings.local.toml",
        ]
    }
    ...
    ///
    /// Relative to the repository rather than to the settings file, so `.swarm/setup.sh` reads
    /// the same whichever of the four files names it, and so a path stays meaningful when a value
    /// is promoted from `.conductor` to `.swarm`.
    ...
    static func apply(
        ...
        // states. That matters now that Swarm writes to `.swarm` and never to `.conductor`, so
        ...
        // missing here: a repository configured in Conductor copied nothing in Swarm, silently,
...
public struct AppDefaults: Sendable, Hashable {
    ...
    /// What a copy of Swarm with no row for `defaults.backend` runs on, which is every copy that
    ...
    /// and Swarm exists to run several at once.
    ///
    /// This is a fallback, not an override. The moment `defaults.permissionMode` holds anything
    /// at all, the Settings, Models picker wins, which is why a copy of Swarm whose Models tab
    ...
    public static func load(from store: Store) async -> AppDefaults {
        ...
        // so the pair a copy of Swarm was already carrying stays one coherent choice.
