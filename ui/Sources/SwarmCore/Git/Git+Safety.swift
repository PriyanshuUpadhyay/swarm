...
/// files and `git worktree remove` deletes them without a word. Swarm copies `.env*` into every
...
/// - `.swarm` is ours. The app creates it to hold prompt attachments, so listing it as work at
///   risk is Swarm warning the user about Swarm.
...
public struct WorkspaceSafetyReport: Sendable, Hashable {
    ...
    /// without a word, so these were invisible. Swarm copies `.env*` into every worktree, which
...
extension Git {
    ...
    /// An ignored file that matches the main checkout byte for byte is not a loss: Swarm copied it
