...
/// writes to a folder the user chose rather than to a worktree Swarm made.
public extension Git {
    ...
    /// Whether the repository's own ignore rules already cover this path. Asked so Swarm only
    ...
    /// and that reads as not ignored, which is the same way round `isIgnored` had it: Swarm writes
