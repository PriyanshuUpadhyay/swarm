...
///
/// Three cases rather than a family name and a flag, because two of the faces Swarm offers have
...
public struct ChatFontFace: Identifiable, Hashable, Sendable {
    ...
    /// one, so the stored setting names a font on the machine rather than a word only Swarm knows.
...
/// The faces the conversation can be set in: four Swarm recommends, then everything installed.
///
/// **What changed and why.** The setting used to be four cases, and two people asked for the font
/// they read in rather than the font Swarm picked for them. A closed enum cannot answer that, so
...
public enum ChatFontCatalogue {
    ...
    /// weight in the bundle and a redistribution question to answer before Swarm can be signed and
