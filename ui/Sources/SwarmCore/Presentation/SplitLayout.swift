...
public struct SplitLayout: Codable, Sendable, Hashable {
    ...
    /// Moves a pane next to another one while its id and live shell stay fixed.
    /// The pane id set before and after the move must be equal.
...
extension SplitNode {
    ...
    /// `before` puts the new pane on the leading or top side.
    func splitting(
