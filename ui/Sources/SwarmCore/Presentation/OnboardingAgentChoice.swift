...
/// `DefaultBackend`). So this picks that agent's own default model: Swarm's fallback for Claude
...
public enum OnboardingAgentChoice {
    ...
    public static func defaults(
        ...
        let effort: String
        if kind == .claudeCode {
            // Swarm's own fallback effort rather than the one in force, which came from the other
