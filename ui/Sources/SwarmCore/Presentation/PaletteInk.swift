...
/// Every colour Swarm states as a number, in the one place a test can read them.
///
/// `Palette` in the app target is what the window draws with, and it is `Color` and `NSColor`,
/// which is exactly why the values could not be checked: `Tests/SwarmCoreTests` depends on
/// `SwarmCore` alone, so a ratio written in a doc comment beside a `Color` was a claim nothing
...
public enum PaletteInk {
    ...
    /// than chosen.
    ///
    /// This is the one pair here Swarm does not draw. `Palette.textSelection` is
    ...
    /// was Swarm's, which is the two-blues complaint at its most visible.
    ///
