...
/// test: `Tests/SwarmCoreTests` depends on the core alone. Three states for where a size comes from
/// and two for whether terminals can outlive a quit, and getting one of them wrong means telling
/// somebody Swarm is following a configuration it has stopped following.
public enum TerminalSettingsCopy {
    /// Where the terminal text size is coming from, and nothing else.
    ///
    /// "13 pt" on its own cannot say whether Swarm is following the user's own terminal or has
