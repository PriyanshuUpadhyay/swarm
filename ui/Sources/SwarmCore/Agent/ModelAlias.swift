...
///
/// Swarm reads Conductor's settings files verbatim so an existing repository needs no new config,
...
public enum ModelAlias {
    ...
    public static func cliValue(for model: String) -> String {
        ...
        if let colon = source.firstIndex(of: ":"),
           ["claude", "claudecode"].contains(source[..<colon].lowercased().filter(\.isLetter)) {
            named = String(source[source.index(after: colon)...])
        } else {
            named = source
        }
