...
/// that died from a row Swarm could not write, and inside a process exit the shape of a runtime
...
public enum AgentExitCause: Sendable, Hashable {
    ...
    case endedMidTurn
    /// Swarm could not write the row. The agent is not what failed.
    case storage(String)
    /// The turn never reached a process. Nothing was launched and nothing was said to the agent,
    /// so there is no exit status and no output: the string is Swarm's own account of why.
...
public struct AgentExit: Sendable, Hashable {
    ...
    /// The resolved path of what Swarm launched, when the payload carried one.
    ///
    ...
    /// write of Swarm's own that failed, which is not about the agent's binary at all.
    ...
    public static func read(_ json: JSONValue?) -> AgentExit {
        ...
        // carries a sentence of Swarm's and no stderr, so reading it as a process exit would draw
    ...
    public static func cause(status: Int?, stderr: String) -> AgentExitCause {
        ...
        // 127 with Swarm's own sentence beside it. `StreamingProcess` writes that line when it
        // cannot resolve the executable, so this is Swarm reading its own words rather than
    ...
    /// A sentence Swarm writes itself ends in a stop. The CLI's own text never gets one added:
