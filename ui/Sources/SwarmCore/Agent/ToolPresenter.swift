...
public enum ToolPresenter {
    ...
    /// Swarm's own four: an agent putting a second agent in the worktree it is already in, and
    /// talking to it. `Crew` argues the feature and `CrewTools` holds the wire contract.
    ///
    /// They are answered here rather than by the generic MCP row, which drew "Swarm: agent
    ...
    private static func mcp(name: String, input: JSONValue) -> ToolPresentation {
        ...
        // Swarm's own bridge, said as Swarm.
        //
        ...
        // it, so a server the owner had called `swarm` produced Swarm's binary running under the
        ...
        let isSwarm = server == BridgeRegistration.serverName
        ...
        if isSwarm, let row = crew(tool: bare, input: input) { return row }

        let label = tool.isEmpty
            ? (isSwarm ? "Swarm" : server)
            : "\(isSwarm ? "Swarm" : server): \(tool)"
        let glyph = isSwarm ? "square.stack.3d.up" : "puzzlepiece.extension"
    ...
    /// A tool Swarm has never heard of still has to read as a sentence, so the name goes in the
