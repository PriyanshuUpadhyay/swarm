...
///
/// **Standing in `~` is not free, because the child is not Swarm.** macOS attributes what a
/// subprocess touches to the responsible application, so anything the CLI reads there is asked for
/// in Swarm's name, and the user is asked about it by a system prompt naming Swarm. Both agent CLIs
...
/// Swarm did, before it had been shown a single project, was name the user's home directory as the
