...
/// Grok's `session/request_permission`, in the vocabulary Swarm's permission prompt already speaks.
///
/// ACP sends a tool call plus a list of options (`allow_once`, `allow_always`, `reject_once`,
/// `reject_always`). Swarm's prompt is allow once / session / project and deny, so the options
...
/// prompt will not offer a rule Swarm could not honour on the wire.
public enum GrokPermission {
    ...
    /// The option Swarm should send back for this decision, or nil when the agent did not offer
