...
///
/// Headless `-p` is NDJSON in the Messages API shape Swarm already stores, and looks like a
...
///     with `--resume`, which is not how Swarm holds a chat open.
...
/// is load-bearing: without it a Swarm chat can attach to the owner's interactive TUI leader.
///
...
/// numbers Swarm assigns, and the server's permission requests use the same numbering, so a
