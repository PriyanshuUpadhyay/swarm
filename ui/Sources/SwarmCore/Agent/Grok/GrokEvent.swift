...
/// One ACP event Swarm cares about, decoded from `grok agent stdio`.
///
...
public struct GrokToolCall: Sendable, Hashable {
    ...
    /// one string so a result row has something to show without Swarm having to understand every
    /// ACP content kind.
    static func contentText(_ json: JSONValue?) -> String {
