...
public struct OutputStyle: Identifiable, Hashable, Sendable {
    ...
    /// What the CLI calls "no style", and what Swarm stores when the picker is left alone.
    ///
    /// The binary's own table of styles has this as a key mapping to nothing, so it is a real
    /// value rather than an invention here. Swarm still sends no setting at all for it rather than
