...
///
/// Keyed by **repository**, not by workspace. A Swarm workspace is a git worktree, so anything
...
///
/// Nothing here is a settings file. Swarm writes none, and answers matching asks itself.
public struct PermissionGrant: Identifiable, Sendable, Hashable, Codable {
    ...
    ///
    /// Here rather than in each runner because it is Swarm's own bookkeeping rather than either
...
/// whitespace, no resolving of paths. If any of those were done here, Swarm would be deciding
/// that two rules mean the same thing, and Swarm is not the component that knows that: a rule is
/// a string in a syntax the CLI owns and evolves.
///
/// The consequence is deliberate and worth stating. Swarm under-matches rather than over-matches.
/// A rule Swarm cannot recognise means one more question, which costs a click. The opposite
/// mistake means a command running because Swarm decided it resembled something the user once
...
/// without Swarm knowing what `*` means.
public enum PermissionGrantIndex {
    ...
    /// The sentence the transcript prints when Swarm answered instead of the user.
    ///
...
/// for as long as it takes, and there is no timer on the other end, so a Swarm that forgot the
...
/// later Swarm has to be readable by this one: an unrecognised word means "decided, somehow",
