...
/// Codex's five approval requests, in the vocabulary Swarm's permission prompt already speaks.
///
/// ## What is the same
///
...
/// reopened mid question can still draw it, and a project-wide allow is stored in Swarm's own
...
///     nothing of the kind. Swarm will not invent a pattern on its behalf, because a guessed rule
...
///   * **`acceptForSession` is remembered by the server, not by Swarm.** So "allow for this
///     session" costs Swarm nothing to keep, and dies with the app-server process rather than with
///     a stored row.
...
public enum CodexPermission {
    ...
    /// What Swarm's answer becomes on the wire.
    ///
    /// Project scope sends `acceptForSession`, exactly as the Claude Code side does for the same
    /// case: the agent is told to stop asking for the rest of this session, Swarm keeps the durable
