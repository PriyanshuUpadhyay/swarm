...
///
/// The eighteen types are the ones `ThreadItem` names in the generated schema. The ten Swarm will
...
public struct CodexUserMessage: Sendable, Hashable {
    ...
    /// Swarm's own attachment drafts already produce, so nothing about them has to change.
...
/// and a Swarm row needs them: one app-server connection can carry several threads.
...
public struct CodexTokenUsage: Sendable, Hashable {
    ...
    /// drive either the context gauge or the per-turn usage Swarm stores.
    ...
    /// Poured into the shape the rest of Swarm already stores and draws.
    ///
...
/// One thing the server said, in the vocabulary Swarm's transcript can eventually draw.
///
...
public enum CodexEvent: Sendable {
    ...
    /// addressed to, and it is the server's own numbering, not Swarm's.
...
/// One of the five questions the server can ask, held open until Swarm answers it.
///
...
/// server to remember rather than asking Swarm to.
...
/// Claude Code there is no rule for Swarm to store and replay.
