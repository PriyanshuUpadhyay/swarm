...
/// cannot use it.
///
/// **The efforts belong to the model, not to Swarm.** Claude Code takes the same five levels for
...
/// each. Not one line of Swarm changed to offer any of it, which is the whole case for fetching:
...
/// keeping: it is what a picker can put under the name instead of Swarm inventing one.
...
public actor CodexModelCatalog {
    ...
    /// `cwd` is an empty folder Swarm owns rather than the home directory: listing models opens no
