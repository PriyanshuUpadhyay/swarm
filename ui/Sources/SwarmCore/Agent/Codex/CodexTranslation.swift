...
/// Turns Codex's typed items into the vocabulary Swarm already stores and draws.
///
...
/// pairs them draws what Swarm already draws for Claude Code, and the result row is where the
...
public struct CodexTranslation: Sendable {
    ...
    /// under the name Swarm already looks for (`command`, `file_path`, `url`), so
    ...
    /// The item as JSON, which for anything Swarm decoded means rebuilding it rather than keeping
    ...
    /// One Codex event, as zero or more Swarm events.
    ///
