...
///     is finished. Swarm's transcript types as the model does, and `exec` cannot do that.
...
///     There is no wire on which a question could arrive, which is the same failure Swarm already
...
/// also what Conductor drives. So Swarm takes app-server, and `exec` is never used.
///
...
///     decoder that requires `"jsonrpc": "2.0"` drops every line. Swarm still sends it, because a
...
/// structured detail there that no Swarm release has to understand in advance.
...
/// Builds the lines Swarm writes to the server's stdin.
///
