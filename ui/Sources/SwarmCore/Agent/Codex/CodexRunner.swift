...
/// Supervises one `codex app-server` connection for one Swarm chat.
///
...
public actor CodexRunner: SessionRunner {
    ...
    /// "allow for this session" live in the app-server process rather than in Swarm's database,
    ...
    /// as "Swarm was not running when this was asked", which is untrue and is not what the person
    ...
    /// moment Swarm concluded the agent was gone, `codex app-server` and the app-server binary
    ...
    public func answer(requestID: String, decision: PermissionDecision) async {
        ...
        // Swarm's own bookkeeping, and it happens after the agent has been unblocked, so a
    ...
    /// answer any more, and the next launch's sweep files it as "Swarm was not running when this
    ...
    /// How Swarm's five modes reach a protocol that has no modes.
    ///
    ...
    ///
    /// | Swarm | Codex preset | policy | sandbox | reviewer |
    /// | --- | --- | --- | --- | --- |
    ...
    private func handle(_ event: CodexEvent) async {
        ...
        // A connection that closed because Swarm closed it is not news, and it must not be drawn
    ...
    /// the sentence a person typed, and no field for the sentence Swarm sends by default either,
