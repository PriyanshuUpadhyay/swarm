...
/// the type to `AgentQuotaAdapters.all`. Nothing else in Swarm changes: the store is keyed by
...
/// of the CLIs Swarm detects are exactly that, and they contribute nothing rather than
...
/// Every adapter Swarm has, and the one call the app makes.
...
/// second window, so what Swarm knows is whatever has been mentioned since the last reset, which
...
///
/// **There is no monthly window.** `five_hour` and `seven_day` are what this protocol emits. Swarm
...
public enum ClaudeCodeUsageAdapter: AgentQuotaAdapter {
    ...
    /// is the CLI's own rather than `NumberFormatter`'s fraction digits, so Swarm and `/usage`
