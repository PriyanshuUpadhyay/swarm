...
///
/// **The panel used to say "not reported" because Swarm only ever listened.** Both CLIs volunteer
...
/// and neither runs a model or spends anything. That is two process launches a minute while Swarm
...
/// One provider Swarm can ask about its own allowance.
///
...
///
/// ```json
/// {"type":"control_request","request_id":"swarm-usage-1","request":{"subtype":"get_usage"}}
/// ```
///
/// and the answer came back in 0.39 seconds, before the CLI had printed its own `system/init`:
///
/// ```json
/// {"type":"control_response","response":{"subtype":"success","request_id":"swarm-usage-1",
...
/// stdin Swarm already holds open for a running chat, and it works, but it is the wrong one. It
...
public struct ClaudeCodeQuotaSource: AgentQuotaSource {
    ...
    /// `cwd` is an empty folder Swarm owns and never the home directory. Nothing here reads a
    ...
    public func read() async -> Data? {
        let id = "swarm-usage-\(UUID().uuidString)"
...
/// `experimentalApi` and its shape will not move under Swarm. It needs no thread: the connection
...
/// adapter Swarm already had rather than through a second reader.
///
...
public struct CodexQuotaSource: AgentQuotaSource {
    ...
    public init(
        ...
        // MCP server to be offered to, and registering one would put a Swarm tool in front of a
...
/// Every source Swarm has, and the one call the app makes.
///
