...
///   on this protocol, and there is no way to ask for the others. So the windows Swarm knows about
...
/// Claude's names are the only thing it gives and a name Swarm has never seen carries no length.
public struct QuotaWindow: Sendable, Hashable, Codable, Identifiable {
    ...
    /// must be whatever the payload said and never a Swarm invention.
    ...
    /// name is not one Swarm can read.
...
public struct AgentQuota: Sendable, Hashable, Identifiable {
    ...
    /// When Swarm last heard this. Kept so a late arriving report cannot overwrite a fresher one,
    ...
    /// was refilled while nobody was watching and the only honest reading is that Swarm does not
...
/// Everything Swarm knows about every provider's allowances, arranged for reading.
///
