...
/// run, and what Swarm says to the orchestrator when one of them stops.
///
...
/// what it split the work into and Swarm does not, so `cascade-read` and `media-suite` are as
/// valid as anything we could have listed. All Swarm asks is that a name is short enough to draw
...
public enum Crew {
    ...
    /// A name Swarm will accept, or nil when there is nothing left of it.
    ///
    ...
    /// The line Swarm puts in the orchestrator's chat when one of its subagents stops.
    ///
    ...
    /// end of a subagent's turn is an event Swarm delivers, carrying what the agent last said
    ...
    ///
    /// **A hint rather than a rule, and this is the whole cleanup design.** Swarm sweeps nothing:
