...
/// A `result` line that closes a turn nobody in Swarm asked for.
///
...
///
/// Swarm read that `result` because every `result` used to end whatever turn was open. It wrote
...
public enum StrayResult {
    /// Whether this line closes a turn Swarm never started.
    ///
    ...
    ///
    /// The emptiness is the net under that reading. No turn Swarm sent comes back with no API
