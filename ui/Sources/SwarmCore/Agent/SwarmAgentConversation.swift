...
public enum SwarmAgentTabs {
    public enum WorkerAnchor: Sendable, Equatable {
        case chat
        case worker(Int)
    }

    public static func placement(index: Int) -> (anchor: WorkerAnchor, axis: SplitAxis) {
        if index == 0 { return (.chat, .horizontal) }
        if index == 1 { return (.worker(0), .vertical) }
        return (.worker(index - 2), .horizontal)
    }

    public static func agentsToAdd(
        from agents: some Sequence<SwarmAgent>,
        attachable: Bool?,
        chairID: SwarmAgentID,
        existing: some Sequence<SwarmAgentID>
    ) -> [SwarmAgentID] {
        guard attachable == true else { return [] }
        var known = Set(existing)
        return agents.compactMap { agent in
            guard agent.id != chairID, agent.alive == true,
                  agent.pane?.isEmpty == false, known.insert(agent.id).inserted
            else { return nil }
            return agent.id
        }
    }
