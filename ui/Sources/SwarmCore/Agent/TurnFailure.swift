...
/// "API Error: Connection lost mid-response. The response above may be incomplete." Swarm read
...
///
/// # Swarm's sentence over the CLI's, with the CLI's kept
///
...
///
/// What is not defensible is passing it through as though Swarm had written it, because then the
/// one time a person most needs the app to be plain, the app is quoting. So a terminal reason
/// Swarm recognises gets Swarm's own lead sentence in Swarm's own register, saying what happened
/// and whether anything is asked of the reader; the CLI's words follow, marked as the CLI's. A
/// reason Swarm does not recognise gets no lead at all, and the CLI's words stand alone rather
/// than being wrapped in a sentence that pretends to a diagnosis nobody made.
public struct TurnFailure: Sendable, Hashable {
    /// Swarm's own account, or nothing when Swarm has no reading of this ending.
    ...
    /// is the honest answer to an ending Swarm has never seen.
