...
///
/// Model ids and permission modes reach Swarm verbatim from a settings file or from a session's
/// own init line, so the ones Swarm ships a label for are a list that is always behind. Everything
...
/// branch in it and the test target cannot reach `Sources/Swarm`. It got there by being wrong:
...
public enum ModelLabel {
    ...
    ///
    /// The vendor prefix goes: inside Swarm every model is a Claude model, so the word carries no
