...
///
/// `ruleContent` is the CLI's, never Swarm's. A rule for one shell command comes back as
/// `Bash` + `sudo -n true`, and a rule for a family of them as `Bash` + `bin/test:*`; which of
/// those the CLI thinks is right is a judgement Swarm is in no position to second-guess, so it is
...
public struct PermissionRule: Sendable, Hashable, Codable {
    ...
    /// broadest thing a single button can grant, and the ask carries a flag asking Swarm not to
...
/// `setMode`, `addDirectories`, `removeDirectories`) and only the first is one Swarm knows how to
...
/// Swarm sends is then the CLI's own words, which is the only way to be sure Swarm never widens a
...
public struct PermissionSuggestion: Sendable, Hashable {
    ...
    /// The one shape Swarm offers as a button: rules that would let this call through.
...
/// Where the CLI would put a rule it was handed. Swarm only ever uses `session`; the rest are here
/// because they are what can arrive on a suggestion, and a value Swarm did not expect must survive
...
/// working directory**, which for Swarm is a git worktree: gitignored, not shared with the
/// repository, and deleted when the workspace is archived. A rule granted "for this project
/// forever" would evaporate at exactly the moment it was supposed to matter, and Swarm would have
/// caused a file write it never told anybody about. Project scope lives in Swarm's own database
...
///
/// This only exists because Swarm launches the CLI with `--permission-prompt-tool stdio`. Without
...
/// call is aborted, or until stdin closes. Every part of the waiting policy is Swarm's.
public struct PermissionAsk: Sendable, Hashable, Identifiable {
    ...
    /// The CLI asking Swarm not to offer a persistent "do not ask again" button for this one,
    ...
    /// Swarm's implementation choice, attached before this plan approval reaches the transcript.
    ...
    /// The suggestion Swarm would act on, and the only one it ever offers as a button.
    ///
    ...
    /// companions widen tools Swarm was not asked about, so they are neither offered nor ever sent
    ...
    /// outside the worktree, so Swarm does not invent one and the button is not drawn.
    ...
    ///
    /// Three of the four ways it can be false are the CLI's own judgement rather than Swarm's
    ...
    ///
    /// The fourth is Swarm's, and it is not a preference. **A question can never be answered by a
    ...
    /// request subtypes, which Swarm has no business answering.
    ///
    ...
    public static func decode(_ json: JSONValue, raw: Data) -> PermissionAsk? {
        ...
        return PermissionAsk(
            ...
            implementationMode: json["swarm_implementation_mode"]?.stringValue.flatMap(PermissionMode.init(rawValue:)),
            raw: raw
        )
    }
...
public enum PermissionDecision: Sendable, Hashable {
    ...
    /// What Swarm says on the way out, when it is closing the session rather than answering.
    ///
...
public enum PermissionAnswer {
    ...
    public static func encode(ask: PermissionAsk, decision: PermissionDecision) throws -> String {
        ...
        case .allow(let scope):
            response["behavior"] = .string("allow")
            // Unedited. Swarm offers no way to change a command before allowing it, and sending
            ...
            // What the decision was, for the CLI's own telemetry. Its enum, not Swarm's.
