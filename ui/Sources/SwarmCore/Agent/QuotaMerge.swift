...
/// Folds a fresh report about an allowance into what Swarm already knew about it.
///
...
///
/// Swarm read that notification as the whole truth. Everything on Codex's `RateLimitWindow` is
...
public enum QuotaMerge {
    ...
    /// Swarm know now", but writing every untouched row back would restamp its `observedAt` and
