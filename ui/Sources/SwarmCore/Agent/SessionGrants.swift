...
/// carried a `cachedRepoID` property, a `repoID()` under the same comment about Ask Swarm having
...
/// the same paragraph about doing Swarm's bookkeeping after the agent has been unblocked. That is
...
/// none of this is protocol. It is Swarm deciding what it already approved, in Swarm's own tables,
...
final class SessionGrants: Sendable {
    ...
    func repoID() async -> RepoID? {
        ...
        // Nil for a chat with no worktree, which is Ask Swarm: there is no project behind it, so
