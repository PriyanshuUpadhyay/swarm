...
/// The chat that belongs to Swarm rather than to a workspace: what it is called, where it runs,
...
public enum AskConversation {
    ...
    /// Without host context, Ask Swarm picked an installed Conductor skill to create workspaces.
    ...
    /// gives: `Store.databaseDirectoryName` is the one rule that decides which copy of Swarm a
    /// process is, so Swarm and Swarm Dev get their own without a second rule to keep in step.
    ///
    ...
    /// under `.swarm/attachments` here, exactly as it would in a worktree, and the agent can then
    ...
    /// worktree is Swarm's word for a thing the reader has not met yet, and an empty pane that
    ...
    ///
    /// Every conversation is made as "Ask Swarm", so a rail of them read "Ask Swarm" down the whole
    ...
    /// panel keeps a composer to type into, and a rail with one "Ask Swarm" row in it after you
