...
/// Supervises one `claude` process for one Swarm session.
///
...
public actor AgentRunner {
    ...
    /// global on Homebrew's PATH and a current native install in `~/.local/bin`, and Swarm
    ...
    ///
    /// An effort Swarm does not know about is passed through rather than filtered. Effort is an
    ...
    func ingest(_ event: AgentEvent) async {
        ...
        // A result for a turn Swarm never asked for closes nothing, and it is dropped here rather
        ...
        if case .result(let result) = event, StrayResult.isStray(result) {
            Self.log.info("ignored a result for a turn Swarm did not start: \(result.origin, privacy: .public)")
        ...
        case .initialized(let info):
            ...
            // Swarm sent.** The CLI runs a background task's completion notification as a turn of
            ...
            // `turnStarted` is `unchanged` from `running`, which is what a turn Swarm sent is by
    ...
    private func handle(_ ask: PermissionAsk) async {
        // Swarm's own bridge tools answer themselves, and they do it before anything is stored or
        ...
        // settled question saying Swarm allowed Swarm is a second row about a call the transcript
    ...
    public func answer(requestID: String, decision: PermissionDecision) async {
        ...
        // Granting is Swarm's own bookkeeping and happens after the CLI has been unblocked, so a
...
public extension PermissionMode {
    /// What `--permission-mode` expects. Swarm's names happen to line up with the CLI's today,
