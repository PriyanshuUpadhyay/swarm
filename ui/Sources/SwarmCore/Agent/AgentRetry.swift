...
///
/// The CLI announces every one of these and Swarm used to drop the lot. During an evening of 529s
...
/// is that the difference between waiting and hung is information Swarm already had.
///
...
public struct AgentRetry: Sendable, Hashable {
    ...
    /// The line in the row's label column: what went wrong, in Swarm's words.
...
public enum RetryTrouble: Sendable, Hashable {
    ...
    case unreachable
    /// A status Swarm has no reading of, kept as a number rather than guessed at.
    ...
    public static func diagnose(status: Int?, category: String?) -> RetryTrouble {
        guard let status else {
            ...
            // anything Swarm can say a sentence about.
