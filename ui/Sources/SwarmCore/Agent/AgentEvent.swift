...
///
/// Tool input is arbitrary JSON that Swarm neither controls nor fully understands, so it has to
...
public enum JSONValue: Sendable, Hashable, Codable {
    ...
    /// payload. It is still the fallback for a decoder Swarm did not build itself.
    ...
    /// Parse one JSON document. Returns nil instead of throwing, because every caller in Swarm is
    ...
    /// A JSON `null` reads as a missing key, because for every field Swarm cares about the two
...
extension AgentError {
    ...
    /// The `.error` row Swarm writes when a turn never reached a process at all.
    ///
...
public enum AgentEvent: Sendable {
    ...
    case hook(AgentHook)
    /// The agent asking to run something, with its turn held open until Swarm answers. Only ever
    ...
    public static func decode(line: String) -> AgentEvent? {
        ...
        // exactly what "Swarm never asks" looked like from the inside: the CLI was willing to ask
        // and nobody was reading the line. Only `can_use_tool` is lifted out; the other control
        // subtypes are the CLI answering Swarm, or asking something Swarm has no business
    ...
    private static func decodeSystem(_ json: JSONValue, raw: Data) -> AgentEvent {
        ...
        // and were dropped, which is what "Swarm cannot see subagents" looked like from the
