...
/// What a model came back with, once Swarm has decided it is usable.
///
...
public enum WorkspaceNaming {
    ...
    ///
    /// Plants, because the app is called Swarm and because a workspace called Foxglove sitting in
    ...
    /// overwrite decision compares against the exact string Swarm wrote, which is what
    ...
    /// also carries fields Swarm must never read back out, so nothing here touches anything but
    /// those two keys.
    ...
    ///
    /// The comparison is against the exact string Swarm wrote at creation, not against the shape
...
/// Whether Swarm names new workspaces for you.
///
