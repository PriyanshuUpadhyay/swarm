...
///
/// **The bug this exists for.** `ExistingBranch.inUseBy` was filled from Swarm's own workspace
/// rows and from nothing else, so a worktree cut by Conductor, by another agent runner, or by
/// hand was invisible to it. The picker drew pull request #362 as a free row, Swarm cut a
...
/// which names a folder Swarm never mentions anywhere else and reads as a crash rather than as a
/// refusal.
///
...
/// the **name**. A branch held by one of Swarm's own workspaces is worth saying by workspace name,
...
public enum BranchHolder: Sendable, Hashable, Codable {
    /// One of Swarm's own live workspaces, by name.
    ...
    /// A worktree Swarm did not make: Conductor's, another runner's, or one cut by hand.
    case otherWorktree(path: String)

    /// Whether the app can take somebody to the holder. Only its own workspaces.
    public var isSwarmWorkspace: Bool {
    ...
    /// intentions, and Swarm does not get to pick between them on his behalf.
...
public extension BranchHolder {
    ...
    ///
    /// A workspace name wins over the path of the same worktree. Swarm's own worktrees are in the
    ...
    /// inside a function whose whole value is that it is pure. Git prints absolute paths and Swarm
