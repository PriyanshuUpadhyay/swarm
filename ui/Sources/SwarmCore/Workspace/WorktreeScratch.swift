...
/// The folders Swarm writes into inside somebody else's checkout, and the one rule about them:
/// git must be blind to every one.
///
/// `.swarm` itself is meant to be committed. `settings.toml`, `setup.sh` and the rest are what a
/// team shares, and that is the whole point of the folder. But Swarm also has to put working
/// files where the agent can read them, which on this platform means inside the worktree: an
/// agent will not read a path outside its working directory without asking, and Swarm has nothing
/// that can answer that question. So Swarm's scratch lives in the user's checkout by necessity,
...
/// what it finds. Swarm's own pull request instructions say "Run `git status`. If anything is
/// uncommitted, review it and commit it", and an untracked file in the worktree is something an
/// agent obeying that will pick up. It did: `.swarm/pr-instructions.md` was written next to the
...
/// Swarm's own changed file list comes from, does not list it. An agent cannot commit what git
/// will not name.
///
/// Deliberately NOT `.git/info/exclude`: a linked worktree shares that file with the repository it
/// was forked from, so writing there would edit the user's main checkout to hide Swarm's scratch.
/// Deliberately NOT a line appended to `.swarm/.gitignore` either: in a repository that already
/// commits `.swarm`, that file is tracked, and editing it would show up as a modified tracked
...
public enum WorktreeScratch {
    ...
    public static let attachments = ".swarm/attachments"

    /// Files Swarm generates for its own use, that no project asked for and no reviewer wants to
    /// read: the default pull request instructions, and whatever comes after them.
    public static let generated = ".swarm/scratch"
    ...
    ///
    /// Pure, and asserted on directly. Anything Swarm writes into a worktree that this says no
    ...
    public static func shield(_ folder: String = attachments, in worktree: String) {
        ...
        // `.swarm` itself is created as a side effect and deliberately left bare. Its own
        ...
        // a setting, whether or not `.swarm` already exists by then.
