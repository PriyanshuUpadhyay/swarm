...
public struct WorkspaceManager: Sendable {
    ...
    ///
    /// `WorkspacesRoot` owns the path and its `.noindex` suffix, which is the part measured to keep
    /// Spotlight out of dependency and build folders inside each worktree.
    ...
    /// owner that Swarm has just undone something they chose.
    @discardableResult
    func bringProjectBack(_ repoID: RepoID) async -> Bool {
        ...
        // A line in Console because the owner hid this project on purpose and Swarm has just
    ...
    private func cut(
        ...
        // why this can say it and nothing in `Sources/Swarm` can. A workspace with no setup script
    ...
    /// starting work, rather than the local `main`, which Swarm never moves and which is weeks
    ...
    private func open(
        ...
        //
        // Git allows one worktree per branch, and Swarm used to find that out from git: it cut a
    ...
    /// The prefix Swarm's own interface uses. Everything the app documents, and everything a
    /// script written for it should bind, is `SWARM_UI_*`.
    public static let environmentPrefix = "SWARM_UI"

    /// Kept alongside it, and deprecated.
    ///
    /// `CONDUCTOR_*` is not a second interface and nothing here should be read as Swarm being
    ...
    ///
    /// Conductor has no equivalent, so this one is Swarm's and the deprecated prefix carries it
    ...
    /// `CREATE DATABASE $SWARM_UI_WORKSPACE_NAME` has one project's worktree writing into the
    /// other's database, and the archive script then drops it out from under them. Every script
    /// author could derive this from `basename $SWARM_UI_ROOT_PATH`, and the ones who do not think
    ...
    public func environment(for workspace: Workspace, repo: Repo, port: Int) -> [String: String] {
        // The one place `$SWARM_UI_URL_FILE` is handed out, and therefore the place its folder has to
        // be made to exist: a script told to write to a path whose directory is missing fails with
        // a redirection error, in the middle of setup, over a folder that is Swarm's business
    ...
    public func runSetup(
        ...
        // A setup script is the one place where a PATH that arrives a moment late is a failed
        // `composer install` rather than a lookup that was slightly worse. Usually free: the probe
        // was started at launch and finished long before anyone created a workspace. See
        // `LoginShellPath`.
    ...
    public func archive(
        ...
        if let archiveLaunch, archiveRuns,
           FileManager.default.fileExists(atPath: workspace.path) {
            ...
            // `$SWARM_UI_PORT`: the container publishing it, the process still listening on it, the
            ...
            // The same reason the setup run waits for it: a teardown that cannot find `docker`
            // leaves the containers the setup started behind. See `LoginShellPath`.
            await LoginShellPath.ready()
        ...
        // the repository, so archiving in Swarm Dev with `delete_branch_on_archive` set deleted a
        ...
        // branch belonging to a worktree Swarm has just removed. A worktree Swarm never found is a
        // state Swarm did not make, and a branch left behind can be deleted by hand later, where a
...
/// can bind `$SWARM_UI_PORT` without colliding with a sibling workspace.
