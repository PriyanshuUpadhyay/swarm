...
// One field, and everything Swarm works out from the line typed into it.
//
...
public extension ProjectTarget {
    ...
    /// rule is asked, so "dev/sparkline" is now read as a place that Swarm cannot find rather than
...
/// Why Swarm will not take the target that was typed.
///
...
public enum ProjectTargetRefusal: Sendable, Equatable {
    ...
    /// is what tells the reader Swarm understood the target and will not take it. Which verb is
...
/// What Swarm will do about the target that was typed. One of these is named on the button before
...
public enum ProjectTargetVerdict: Sendable, Equatable {
    /// Nothing is there. Swarm makes the folder, and the location above it when `makesLocation`.
    case create(makesLocation: Bool)
    /// A folder is there and it is empty, so Swarm uses it as it is.
    case adopt
    /// A git repository. Swarm registers it and writes nothing.
...
public extension ProjectTargetVerdict {
    ...
    ///
    /// Only where Swarm made the repository. A project with an empty first commit is not something
...
public enum ProjectConsequenceTone: Sendable, Equatable {
    /// Nothing typed yet.
    case waiting
    /// Swarm will do it, and there is nothing to weigh up.
    case going
    /// Swarm will do it, and this is the one to read before pressing: somebody's existing files
...
public extension ProjectConsequence {
    ...
    /// The whole content of this sentence is that Swarm wrote none of it. A language-guessed
