...
/// Why something Swarm did for a workspace failed, said to the owner rather than to an agent.
///
...
/// owner's own folder and the whole remedy is about it; a Swarm worktree's path is not, because it
/// is a directory inside Swarm's workspaces root that the owner did not choose and cannot usefully
/// act on. The workspace's name is named instead. A worktree that is **not** one of Swarm's is the
...
public enum WorkspaceTrouble: Sendable, Equatable {
    ...
    /// `BranchHolder`: that something is not always one of Swarm's own workspaces, and the two
    /// read differently because only one of them is somewhere Swarm can take you.
    ...
    /// Swarm could not write a turn into its own database, and the transcript it would have
    ...
    /// Swarm could not write a review comment, so the list on screen is what the row still holds.
    ...
    /// argv taken off, and Swarm does not know enough about it to say where a break belongs.
