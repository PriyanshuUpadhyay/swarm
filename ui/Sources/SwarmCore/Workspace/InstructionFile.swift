...
/// The two things every instruction file Swarm sends to an agent has to do: prove it is really
...
public enum InstructionFile {
    ...
    ///
    /// Swarm writes this message rather than the user, so there is no caret to put the file at and
    ...
    /// a turn Swarm composed and in one somebody typed, so a transcript can draw both the same way.
