...
/// not an API key, and the token that represents that sign-in lives in `~/.claude.json`. Swarm
/// does not read it, does not hold it and does not send it anywhere: the CLI authenticates, the
/// same way it does for every other agent turn this app runs. An HTTP client inside Swarm would
/// mean Swarm handling a live credential, and would not work at all for the majority of users who
