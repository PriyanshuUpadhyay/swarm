...
// the path that makes is one Swarm is willing to create.
//
...
// apply (Swarm's own worktrees, a repository above, the folders macOS owns) are asked here of the
...
extension NewProjectFacts {
    ...
    /// in front of it: `~/dev/code/swarm/Tools` is refused for being inside a repository, with the
...
/// Why Swarm will not make a project at the path that has been typed.
///
...
public enum NewProjectVerdict: Sendable, Equatable {
    /// Swarm makes the folder. `makesLocation` is true when the location above it has to be made
    /// as well, which is the ordinary first run: nobody has a `~/Developer` until something makes
    /// one.
    case create(makesLocation: Bool)
    /// The folder is already there and empty, so Swarm takes it as it is.
    ///
    ...
    /// send them to the Finder to delete a folder so that Swarm could make the same one back.
    case adopt
...
public extension NewProjectVerdict {
    ...
    /// they make somewhere Swarm is willing to work at all, and only then what is already there.
    ...
    static func of(_ facts: NewProjectFacts) -> NewProjectVerdict {
        ...
        if FolderPath.isInside(facts.path, of: facts.workspacesRoot) {
            return .refuse(.insideSwarmsWorkspaces(shown(facts.path)))
        }
    ...
    /// The line under the location field. It says the whole path and then what Swarm will do to
...
// MARK: - The two answers Swarm fills in for you

public enum NewProjectPlan {
    /// Where a first project goes when there is nothing to learn it from.
    ///
    /// `~/Developer` rather than a name of Swarm's own choosing. It is the one folder name macOS
    ...
    ///
    /// Counted rather than guessed, and counted off the projects Swarm already has, because the
    /// honest default for somebody who has been using Swarm for a week is the folder they have
    ...
    /// How many of the projects Swarm already has live in this folder.
    ///
    ...
    /// Location and name as one absolute path, or nil when the location is not one Swarm can place.
