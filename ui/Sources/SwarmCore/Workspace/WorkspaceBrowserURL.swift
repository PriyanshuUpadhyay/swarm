...
///
/// `http://localhost:$SWARM_UI_PORT` was the only answer there was, and it is the right answer only
...
/// variables the scripts are handed, so `http://localhost:$SWARM_UI_PORT/admin` is a line somebody
/// can commit. The file at `$SWARM_UI_URL_FILE` is for an address only the setup script can know: a
...
/// address bar says where it came from; one quietly swapped for `localhost` looks like Swarm
...
public enum WorkspaceBrowserURL {
    ...
    /// The absolute path handed to every script as `$SWARM_UI_URL_FILE`.
    ...
    ///
    /// A name nothing sets is left exactly as it was typed rather than blanked. `$SWARM_UI_PROT` is
    /// a typo, and an address bar showing it is a person's answer in one glance; the same typo
    /// blanked produces `http://localhost:` and a question about Swarm.
