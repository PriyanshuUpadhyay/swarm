...
public enum RepositoryDestination: Sendable, Equatable {
    /// `git init` and a first commit. No network, no account, no gh. Everything Swarm does with a
...
public enum RepositoryStartStep: String, Sendable, CaseIterable, Comparable {
    ...
    /// be worse than the silence it replaced. What runs out is Swarm's willingness to keep
...
/// failure never does.
///
/// The rule is that Swarm cleans up exactly what it made and nothing else. A `git init` with no
/// commit after it is a folder that is a repository and cannot be used as one: Swarm cannot cut a
/// worktree from it, git tools behave differently inside it, and nobody asked for it. So it goes.
/// The moment there is a commit it stops being Swarm's to remove: that commit holds the user's
/// files, it is a project Swarm can run, and deleting it would be throwing away the only half of
/// the job that worked.
public enum RepositoryStartAbandonment: Sendable, Equatable {
    /// No repository was made, so there is nothing to undo.
    case nothingToUndo
    /// The `git init` Swarm ran was undone. The folder is a plain folder again.
    case repositoryRemoved
    /// There is a commit, so the folder is a project Swarm can run and it is left exactly as it is.
    case projectKept
    ...
    /// Whether the folder is a repository Swarm can already run workspaces in, which is what makes
...
public struct RepositoryStartFailure: Error, Sendable, Equatable {
    ...
    /// Whether the folder is a repository Swarm can already run workspaces in. True the moment
...
public enum RepositoryStarter {
    ...
    ///
    /// "Initial commit" and nothing else, deliberately. Swarm has not read this code and cannot
    /// say what it does, and a generated sentence describing somebody's project would be a claim
    /// about work Swarm did not do, permanently attached to their history by an author who is
    ...
    public static func start(
        ...
        // An origin that is already set is proof that both of the next two steps have run: Swarm
    ...
    ///
    /// Only ever correct for a folder Swarm itself initialised, which is the only kind the setup
    ...
    private static func makeFirstCommit(
        ...
        // Swarm editing a file it had no reason to touch.
