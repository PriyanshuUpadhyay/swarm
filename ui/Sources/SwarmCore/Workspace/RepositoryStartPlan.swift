...
// What Swarm does about a folder it has been handed, decided before anything is done to it.
//
...
// added anything git recognised, which meant it would register one of Swarm's own worktrees or
...
public struct FolderFacts: Sendable, Equatable {
    ...
    ///
    /// Where the location rules are applied, because `~/swarm/workspaces/x/src` is inside one of
    /// Swarm's worktrees exactly as much as `~/swarm/workspaces/x` is, and only the top level
    ...
    /// only from a caller that typed a path, and it matters there because Swarm's working
    ...
    /// Where Swarm cuts its own worktrees, passed in for the same reason.
...
/// Why Swarm will not take a particular folder, whether it was asked to register one or to make
/// one.
///
...
public enum FolderRefusal: Sendable, Equatable {
    /// A path that means nothing on its own. Swarm's working directory is not the caller's.
    case notAbsolute(String)
    /// Nothing at that path.
    case nothingThere(String)
    case notADirectory(String)

    /// A worktree Swarm cut itself. Registering one would make Swarm cut worktrees of a worktree.
    case insideSwarmsWorkspaces(String)
    ...
    /// git would not answer, so Swarm cannot tell what the folder is. Refused rather than offered,
...
public extension FolderRefusal {
    ...
    var sentence: String {
        ...
        case .insideSwarmsWorkspaces(let path):
            """
...
/// What Swarm will do about a folder that was handed to it.
...
public extension FolderVerdict {
    ...
    /// would register one of Swarm's own worktrees, or a home directory somebody had run
    /// `git init` in, while `project_add` refused both. They are asked of the resolved top level
    /// rather than of the path handed in, because `~/swarm/workspaces/x/src` is inside a worktree
    /// exactly as much as `~/swarm/workspaces/x` is and git resolves either.
    static func of(_ facts: FolderFacts) -> FolderVerdict {
        ...
        if facts.isRepository {
            ...
            if FolderPath.isInside(root, of: facts.workspacesRoot) {
                return .refuse(.insideSwarmsWorkspaces(root))
            }
...
public enum FolderPath {
    ...
    /// string prefix, so `~/swarm/workspaces-old` is not read as being inside `~/swarm/workspaces`.
    ///
...
///
/// Swarm already knows this hazard from two directions. `WorkspaceSafetyReport` singles out a
...
/// in the repository is still copied into every worktree Swarm creates, so agents keep working.
///
