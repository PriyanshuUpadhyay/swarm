...
/// session already lives and the id does not change. Nothing in Swarm reads those files, so no
...
///
/// **What is not carried, and why the opening turn says so.** Swarm's own transcript rows are
...
public enum CarryOnRefusal: Sendable, Hashable {
    ...
    case stillLooking
    /// The project this workspace belonged to is no longer in Swarm, so there is no repository to
    ...
    case neverRan
    /// The chat is on a backend Swarm has no runner for, so nothing could pick the thread up. See
...
public struct ArchivedCarryOn: Sendable, Hashable {
    /// The archived workspace's name, which the new one keeps. Swarm does not invent a name for
