...
public struct WorkspaceStartRequest: Sendable {
    ...
    ///
    /// Nil is the route Swarm has always had: a branch named after the prompt, cut from
...
extension WorkspaceManager {
    ...
    public func start(
        ...
        if request.setupPolicy == .run {
            ...
            // wanted to stop whatever holds `$SWARM_UI_PORT` had nothing to look for while the setup
