...
/// creating a folder that is not there yet, plus knowing whether Swarm made that folder so that
...
public enum NewProjectStarter {
    ...
    /// Swarm is about to make on somebody's behalf and an existing repository's is not. Saying
    ...
    /// Undoes exactly what Swarm made, and nothing else.
    ///
    ...
    /// same rule one level up: Swarm removes the folder only if Swarm made it, only if the
...
public struct NewProjectCreation: Sendable, Equatable {
    ...
    /// Whether the folder itself is Swarm's doing. False when the person pointed at an empty
...
/// without the other: a folder Swarm did not make keeps its `git init` undone and stays, and a
/// folder Swarm made that now has a commit in it stays with the repository intact.
public struct NewProjectAbandonment: Sendable, Equatable {
    ...
    /// Whether what is left is a project Swarm could still run, which is what makes "add it
