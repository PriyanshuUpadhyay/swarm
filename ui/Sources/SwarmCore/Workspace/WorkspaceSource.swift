...
public enum WorkspaceSource: Sendable, Hashable, Identifiable {
    /// Cut a fresh branch off a named ref. What Swarm could always do.
    ...
    /// not exist.
    ///
    /// Only one of the two answers is somewhere Swarm can take you. A workspace is selected; a
...
public struct WorkspaceSourceOffering: Sendable, Hashable {
    ...
    /// `branches` above: that one has the pull request heads taken out of it, and they are
    /// perfectly good things to cut a branch from.
    ...
    /// The "New branch from" section: Swarm's original and still ordinary route.
    ...
    /// from: it is in `branches` now, and a window that opens asking "open main instead?" whenever
    /// the project happens to be on another branch is second-guessing the ordinary case. Nor for a
    /// branch git would refuse: one held by the project's own checkout or by a worktree Swarm did
    /// not make. A branch one of Swarm's own workspaces
    ...
    public func carryOn(
        ...
        guard base != defaultBranch else { return nil }
