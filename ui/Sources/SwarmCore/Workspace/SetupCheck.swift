...
public enum SetupTool: String, Sendable, Hashable, CaseIterable, Identifiable, Codable {
    ...
    /// Why Swarm wants it, in one sentence, from the user's side of the screen.
    ///
...
/// retyped from prose is a command that fails on a hyphen. `isInteractive` is what says Swarm can
...
public struct SetupFix: Sendable, Hashable {
    ...
    /// the command assumes.
    public var url: URL?
    /// True when the command asks questions, and therefore when Swarm can host it in a terminal
...
public enum SetupVerdict: Sendable, Hashable {
    ...
    case readyWithNotes
    /// Swarm cannot start a workspace on this machine yet.
    case blocked
...
public struct SetupReport: Sendable, Hashable {
    ...
    /// True when at least one agent Swarm can actually drive is installed and signed in.
    ///
    ...
    /// The rows standing between this machine and a working Swarm, in reading order.
...
public extension SetupReport {
    ...
    private var readyWithNotesSentence: String {
        ...
        // to be alone in this list, and "Swarm has what it needs. the GitHub CLI is not set up"
...
public enum OnboardingTrigger: Sendable, Hashable {
    /// Nobody has finished it on this copy of Swarm yet.
    case firstRun
...
public enum OnboardingGate {
    ...
    ///
    /// **The report this answers: "I don't want this popup to pop out every time I open Swarm."**
