...
///
/// Continuing is deliberately NOT a rename. `BranchRenameGate` decides whether the branch Swarm
...
public struct ContinuationFacts: Sendable, Hashable {
    ...
    /// where Swarm put the worktree, and an agent that cuts a branch of its own never touches it.
    ///
...
///
/// Swarm normally names a branch from the first prompt, using a model (`WorkspaceNamer`). That is
...
///
/// A workspace named after the task that just merged keeps that name, for the same reason. Swarm
...
public enum ContinuationBase: String, Sendable, Hashable {
    ...
    /// with no remote, which is a perfectly ordinary thing for Swarm to be pointed at.
    case localBranch
}
...
public extension WorkspaceManager {
    ...
    func continueOnNewBranch(
        ...
        // for the reason `ContinuationFacts.mergedBranch` gives at length: the row is where Swarm
