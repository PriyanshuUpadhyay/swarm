...
/// Which shell Swarm starts, and what it calls it.
///
/// Two places decided this independently and identically: the pty `TerminalView` launches, and
/// the tmux configuration. Each read `SHELL`, each fell back to
...
/// is invisible until somebody's `.zprofile` stops running in one of the two terminals Swarm
