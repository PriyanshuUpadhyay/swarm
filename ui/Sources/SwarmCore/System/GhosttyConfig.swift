...
///
/// Deliberately not an AppKit colour: SwarmCore has no UI, and the whole point of reading Ghostty
...
public struct GhosttyColor: Codable, Sendable, Hashable {
    ...
    /// `#rrggbb`, `rrggbb`, `#rgb` or `rgb`, read by `HexColor`, which Swarm's own palette reads
    /// its stored accents with too.
    ///
    /// X11 colour names are valid in Ghostty but are not understood here: an unparseable value
    /// leaves the key untouched rather than resetting it, so a name falls back to Swarm's own
...
/// The slice of Ghostty's configuration Swarm's terminal can honour.
///
/// Every colour is optional and `nil` means "Ghostty said nothing about this", which is what lets
/// a machine with no Ghostty config keep Swarm's own appearance untouched.
public struct GhosttyTheme: Codable, Sendable, Hashable {
    ...
    /// What a terminal draws with when it follows this configuration inside a Swarm theme.
    ///
    /// **A config that sets its own background or foreground is a whole terminal**, and gets
    /// Ghostty's defaults for everything else, because a dark ground of the user's with Swarm's
    ...
    ///
    /// Filling the gaps from Ghostty rather than from Swarm's palette matters: a user who
...
public enum GhosttyConfigLoader {
    ...
    /// machine at all. Nil is the signal to leave Swarm's own terminal colours alone.
