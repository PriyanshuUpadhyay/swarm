...
///     `.zshrc` that blocks forever costs five seconds once and then Swarm carries on with the
...
public enum LoginShellPath {
    ...
    static func discover(
        ...
        guard environment["SWARM_UI_LOGIN_SHELL_PATH"] != "0" else { return [] }
    ...
    /// resolving it against Swarm's working directory rather than the user's would mean the same
    ...
    /// `/usr/bin/ruby` meant it. What Swarm itself was launched with comes next, and the guessed
