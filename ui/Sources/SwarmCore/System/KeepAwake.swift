...
/// relaunch, and one that ran out while Swarm was closed is simply gone.
...
public enum KeepAwake {
    ...
    /// sandboxed App Store app, it holds the same two IOKit assertions Swarm does, and its
    ...
    ///
    /// So Swarm does the same thing through the door Apple opened for it, `SMAppService`, which a
    /// sandboxed app cannot use and Swarm can: see `SleepSwitch`. The flag is cleared when the
    /// session ends, when Swarm quits, and by the helper itself if Swarm dies, which is the one
