...
public final class StreamingProcess: Sendable {
    ...
    public init(
        ...
        // SIGPIPE, whose default disposition kills the process, and Swarm does not turn it off.
        //
        ...
        // `SO_NOSIGPIPE` on every socket it owns and its comment is that without it Swarm would be
    ...
    /// pgid, so either one would have Swarm signal itself. Both fall back to the single pid.
