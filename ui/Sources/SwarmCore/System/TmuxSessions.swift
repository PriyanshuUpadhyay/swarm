...
public enum TmuxSessions {
    ...
    /// is not loaded into shells Swarm drew, so their prefix key and status line stay theirs.
    ///
    /// The socket is per database, so an instance pointed at `SWARM_UI_DB_PATH` cannot adopt or sweep
    ...
    /// Sessions that nothing in Swarm can ever reach again.
    ///
    ...
    /// edit outside Swarm left behind.
    ...
    /// killed. Nothing running is allowed to depend on Swarm having remembered it.
    ...
    /// of a tmux-backed pane is a child of the server rather than of Swarm, so this is the only
