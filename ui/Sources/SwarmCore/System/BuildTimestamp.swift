...
/// When this copy of Swarm was built, for the one place that has a use for it: the line under the
...
/// `SwarmBuildDate` into the assembled bundle beside `SwarmBuildChannel`, which is the same place
...
public enum BuildTimestamp {
    /// The Info.plist key `Tools/build.sh` writes. In the `Swarm` prefix the other two build facts
    /// use, so the three read as one family in a plist dump.
    public static let infoKey = "SwarmBuildDate"
