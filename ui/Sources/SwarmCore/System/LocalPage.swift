...
/// A page that lives in the worktree rather than on a server, and what Swarm's browser may do
/// with one.
///
/// The request was "when an HTML file is selected, offer to open it in a browser tab or pane",
/// and the awkward half of it is that **Swarm serves nothing**. A workspace's port block is a set
...
/// that goes away with it. There is no route by which Swarm could hand a file of its own to a
...
public enum LocalPage {
    ...
    ///
    /// Not `md`, and not `markdown`: Swarm has a Markdown viewer of its own, and WebKit handed a
    ...
    public static func fileURL(from address: String, root: String) -> URL? {
        ...
        // The separator goes on before the comparison, so a worktree at `/w/swarm` cannot claim
        // the file at `/w/swarm-old/index.html`. Canonical paths also keep a symlink inside the
