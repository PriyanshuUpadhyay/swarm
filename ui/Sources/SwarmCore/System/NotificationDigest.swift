...
///
/// Swarm's whole premise is several agents at once, which means they finish together: kick six off
    ...
    public static func prepare(_ batch: [NotificationDraft]) -> PreparedNotification? {
        ...
        guard batch.count > 1 else {
            ...
            return PreparedNotification(
                identifier: "swarm.\(first.event.rawValue).\(first.workspaceID)",
        ...
        return PreparedNotification(
            identifier: "swarm.\(first.event.rawValue).digest",
            threadIdentifier: "swarm.\(first.event.rawValue)",
