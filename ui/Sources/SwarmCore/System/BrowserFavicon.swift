...
/// Which icon a browser tab wears, and what Swarm will accept as one.
///
...
/// and it is Swarm opening a connection to whichever host a page happens to be on for a file
/// nobody asked for. An icon a page does not declare is one Swarm does not go looking for.
///
/// **What comes back is never the page's own bytes.** The page rasterises its icon into a canvas
/// Swarm sized and hands back a PNG of that, so no ICO parser, no SVG renderer and no image of a
...
public enum BrowserFavicon {
    ...
    private struct Ranked {
        ...
        /// Nil for a declaration Swarm will not ask for: a `rel` naming no icon, an href that will
    ...
    /// capped before any decoding, so a page cannot make Swarm allocate its way through a
