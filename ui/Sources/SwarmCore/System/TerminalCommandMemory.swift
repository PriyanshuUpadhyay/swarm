...
/// What a terminal pane was running when Swarm last saw it, and whether that is worth offering
/// back on the next launch.
///
/// The problem this exists for: a dev server is started in a terminal tab, Swarm is quit, and the
...
/// Swarm executing an agent's words with nobody in the loop. Shown and pressed, not executed.
public enum TerminalCommandMemory {
    ...
    /// What to write down for a pane, from the two things known about it: the last line Swarm sent
    ...
    ///
    /// When something IS running, the text Swarm sent wins over what the machine calls it. They
    ...
    /// the fallback, because a command typed into the shell by hand never passed through Swarm.
