...
/// What this copy of Swarm actually is: a release, the copy `Tools/master.sh` installs, or a build
...
/// `SWARM_UI_VERSION` and `SWARM_UI_BUILD`, which only the release workflow does. So every build made on
...
///
/// `SwarmBuildChannel` is the key that knows, and `SwarmMasterCommit` takes precedence because it
/// is the more specific fact about the same bundle.
public enum BuildIdentity: Equatable, Sendable {
    private static let buildChannelKey = "SwarmBuildChannel"
    private static let masterCommitKey = "SwarmMasterCommit"
