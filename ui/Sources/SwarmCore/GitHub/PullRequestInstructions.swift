...
/// instead of Swarm guessing one, and the whole request lands in the transcript where it can be
...
/// scripts became `.swarm/setup.sh`: a program belongs somewhere it can be read, edited, diffed
...
///
/// - `.swarm/pr-instructions.md` is the PROJECT's. If it exists it wins, tracked or not, and
///   nothing here ever writes it, edits it or deletes it. Once it exists it belongs to the
///   project rather than to this app.
/// - `.swarm/scratch/pr-instructions.md` is SWARM's, written on demand from the default below so
...
/// instructions say to commit anything uncommitted. The agent obeyed, and Swarm's scratch file
/// went out in a user's pull request and was merged. A file only Swarm writes has no business
...
public enum PullRequestInstructions {
    ...
    public static let projectPath = ".swarm/pr-instructions.md"

    /// Where Swarm writes the default when the project has no copy of its own.
    ...
    /// look taken for the one attachment Swarm makes for itself.
    ...
    /// Which of the two files this worktree is going to use, writing Swarm's own if it has to.
    ...
    /// Moves a copy an older Swarm left lying in `.swarm/pr-instructions.md` into the scratch
    ...
    /// file has to be byte for byte the default this app ships, which is what proves Swarm wrote
    ...
    /// committed by the next agent told to commit what it finds. Swarm does not undo what is
    ...
    /// Whether this text is one Swarm wrote and nobody has touched since.
    ///
    ...
    /// older Swarm wrote into `.swarm/pr-instructions.md` before there was a scratch folder to
    ...
    is invisible to git. To make it this project's, move it to `.swarm/pr-instructions.md`, edit it
