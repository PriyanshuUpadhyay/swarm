...
/// How Swarm resolves a merge conflict, as a file in the worktree rather than as eight paragraphs
/// in the chat.
///
...
///
/// **The file is Swarm's, and it is rewritten on every press.** It sits in the shielded folder
...
/// and `.swarm/conflict-instructions.md` or the project settings field, which is `ProjectInstructions`
...
public enum ConflictInstructions {
    ...
    /// That is the contract every path Swarm writes into a turn holds: a path in a turn is a
    ...
    what this project does about conflicts, write it in `.swarm/conflict-instructions.md` and commit
