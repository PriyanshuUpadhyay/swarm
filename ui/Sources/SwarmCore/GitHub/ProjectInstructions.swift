...
/// What a project has to say about one of Swarm's own turns, over and above what Swarm says.
///
/// Swarm's own words used to be a file as well. Pressing Merge wrote `MergeInstructions` into
/// `.swarm/scratch/merge-instructions.md` and attached it straight back, so the turn could say
...
/// turn arrives here already carrying a file of Swarm's, and what this type adds goes after it and
/// outranks it.
///
/// **Two places a project can say it, and the file wins.**
///
/// - `.swarm/merge-instructions.md`, or `.swarm/conflict-instructions.md`, in the worktree. This
...
public enum ProjectInstructions {
    /// One turn Swarm composes itself and that a project may add to.
    ...
    public static func projectPath(for subject: Subject) -> String {
        ".swarm/\(fileStem(for: subject))-instructions.md"
    }
    ...
    /// `.swarm/merge-instructions.md`, or one somebody emptied and left behind, has said nothing,
    /// and a turn that names it would send the agent to read nothing. Judging by the words in it
    /// stops there: a file with a heading and nothing else is somebody starting, and Swarm is in
    ...
    /// moment of answering, which is the contract every path Swarm writes into a turn holds: a
    ...
    /// The whole turn: what the prompt rendered, Swarm's own steps for this subject, and the
    ...
    /// Swarm's own steps, which no override can edit away.
    ///
    ...
    ///
    /// Resolving a conflict has steps of Swarm's own too, and they are not nil because there are
    ...
    /// file Swarm names in a turn goes and how a person typing one writes a path. See
