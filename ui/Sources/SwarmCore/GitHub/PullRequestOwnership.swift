...
/// reused constantly, by people and by Swarm itself, which derives one from the task and from a
...
///   branch's own git config, so a review workspace can say which one is its without Swarm
...
public extension Git {
    ...
    /// Reading it back costs one `git config` and needs no column of Swarm's own: the association
...
public extension GitHub {
    ...
    /// both sides, the worktree moved to `main`, and every route Swarm had ran out at once. `gh pr
    ...
    /// deleted is the state Swarm's own merge leaves behind, and the only state route 3 helps in.
