...
public struct Repo: Identifiable, Sendable, Hashable, Codable {
    ...
    /// `collapsed` and `sortOrder` already made. Swarm's database is per machine already: there
    /// is one row per project per copy of Swarm, so a per-machine view preference and a
    ...
    /// Where `iconPath` came from, and whether Swarm has looked at all. See `RepoIconSource`.
...
/// picture Swarm guessed at should not.
public enum RepoIconSource: String, Sendable, Codable, CaseIterable, Hashable {
    /// Added before Swarm looked for icons, or added while detection was refused. Draws the
...
public struct Workspace: Identifiable, Sendable, Hashable, Codable {
    ...
    ///
    /// `internal(set)` so that is a compile error in `Sources/Swarm` rather than a convention. See
    ...
    /// put up, since both are handed the same `$SWARM_UI_PORT` for the same workspace.
    ///
    ...
    /// reason this column exists.** Every lookup Swarm had went through `gh pr view <branch>`,
    ...
    /// doc comment says that needs no column of Swarm's. That holds right up until the branch is
    ...
    /// `Sources/Swarm` could build a fresh `Workspace` carrying an existing id and whatever state
...
public enum SessionState: String, Sendable, Codable, CaseIterable, Hashable {
    ...
    /// state ends when a person ends it. A session in it is the one thing in Swarm that gets
...
///
/// **The cases are Swarm's slots, and the words over them belong to whichever CLI is about to
...
public enum PermissionMode: String, Sendable, Codable, CaseIterable {
    ...
    ///
    /// **Added because Swarm had no row for it and a user said so.** Codex's four presets are
    /// `read-only`, `workspace`, `auto` and `full-access`, and Swarm offered three of them:
    ...
    /// `approvalsReviewer: auto_review`, which nothing in Swarm had ever sent.
    ///
...
public struct Session: Identifiable, Sendable, Hashable, Codable {
    ...
    ///
    /// **Nil is Ask Swarm and nothing else today.** That chat sits above every project, so there
...
public enum MessageKind: String, Sendable, Codable, CaseIterable {
    ...
    case system
    case notice
    /// Something an agent said to another agent, or Swarm's own word about one of them. The
...
/// The coding agent CLIs Swarm knows how to talk about.
///
...
public enum AgentKind: String, Sendable, Codable, CaseIterable, Identifiable {
    ...
    /// Whether Swarm can actually drive a chat with it.
    ///
    ...
    /// as the next turn instead, announcing it with a second `init`. Both landings are ones Swarm
