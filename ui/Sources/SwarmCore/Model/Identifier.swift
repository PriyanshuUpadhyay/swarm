...
///
/// Every id in Swarm used to be a bare `String`, so `store.update(workspaceID: session.id)`
...
/// One of the owner's saved model presets, the named sets the composer's model menu lists.
...
///
/// The raw value is `task_id` off `system/task_started`, and it is the CLI's, not Swarm's: it is
...
///
/// swarm's, not Swarm's: an integer in swarm's own database, kept here as its decimal text. Stored
