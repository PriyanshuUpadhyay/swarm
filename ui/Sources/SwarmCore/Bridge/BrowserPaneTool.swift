...
///
/// **Why the line between them falls where it does.** Two report Swarm's own chrome, which is the
...
/// `browser_read`: what Swarm's own toolbar says about one browser pane.
///
...
/// Swarm answering its own question there would be Swarm deciding that what is in the reader's
...
public struct BrowserGoTool: BridgeToolHandling {
    ...
    /// the two doors into a browser pane cannot come to disagree about what Swarm will open.
...
/// administrator is his data, and the difference is not something Swarm can tell from here. So
