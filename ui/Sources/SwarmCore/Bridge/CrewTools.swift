...
public enum CrewToolTrouble: Error, Sendable, Equatable {
    ...
    /// Swarm still has its row, because "talk to your orchestrator" is worth less than its name.
...
public struct AgentListTool: BridgeToolHandling {
    ...
    /// Swarm sweeps no finished agent away itself, for the reason `Crew.tidyHint` gives, so this
...
/// description has to carry that because nothing else can. Swarm sweeps no finished agent away on
