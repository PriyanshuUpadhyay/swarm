...
/// Whether Swarm answers a permission question about one of its own tools without asking anybody.
///
/// ## Why this exists
///
/// Swarm's bridge tools reach the agent as MCP tools, and an MCP tool call goes through the CLI's
...
///
/// Swarm is on both ends of this question. It wrote the tool, it minted the token the caller is
...
/// weigh that Swarm has not already decided, and the ask carries no information a person could
/// act on beyond "an agent would like to use Swarm".
///
/// That is emphatically not true of the tools the agent brings with it. `Bash`, `Write` and
/// `Edit` reach outside anything Swarm knows about, and nothing here touches them.
///
/// ## What is deliberately not on the list
///
/// Anything that destroys work. `workspace_archive` removes a worktree, and the whole reason Swarm
...
/// Swarm keeps no copy of what was there before, and both are reachable only from the owner's own
...
/// the owner wrote; a workspace name is a label Swarm proposed and the owner accepted, it is one
...
/// and a page he is signed into is his data. Swarm cannot tell a dev server's front page from an
...
/// is nothing on the far side of it to restore. Swarm answering its own permission question there
/// would be Swarm deciding to publish, which is the one decision it has never had.
public enum BridgeToolApproval {
    ...
    /// The tools Swarm answers for itself, by their bare names.
    ///
    ...
    public static let selfApproved: Set<String> = [
        ...
        // that Swarm has not already decided.
        "agent_start",
        "agent_say",
        "agent_list",
        "agent_stop",
        ...
        "workspace_say",
        // The projects Swarm has, read from its own database. It changes nothing, and it is the
        // call a workspace agent makes before naming another project to `workspace_start`, so an
        // ask here would hang the turn that is about to hand work on, with nothing for a person
        // to weigh: the names are the owner's own sidebar. `project_add` and `project_hide` stay
        // off the list and off the workspace role, because they change that sidebar.
        "project_list",
    ]

    /// Whether this ask is Swarm answering itself.
    ...
    /// Swarm had allowed Swarm, and a turn that opens a pane, splits it, renames a tab and lists
