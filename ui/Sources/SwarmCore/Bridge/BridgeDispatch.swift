...
/// The MCP server, on Swarm's side of the socket.
///
...
public struct BridgeDispatch: Sendable {
    ...
    /// What Swarm calls itself in `serverInfo`. The version is the socket protocol's, not the
    ...
    /// point of the handshake that already ran on this socket is that version skew is Swarm's
