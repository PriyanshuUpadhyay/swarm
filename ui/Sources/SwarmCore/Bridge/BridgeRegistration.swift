...
public enum BridgeRegistration {
    ...
    /// user's own server called `swarm`, overriding `command` and `env` produced Swarm's binary
    ...
    /// The name the owner's own client registers Swarm under, **derived per copy of the app** and
    ...
    /// scope, which Claude Code applies to every session on the machine, and Swarm's own
    /// `--mcp-config` is documented above as additive over exactly that file rather than replacing
    /// it. So an agent Swarm launched inside a workspace would meet two entries called the same
    ...
    /// `swarm-owner-bridge` for every copy of the app at once, and `claude mcp add` replaces an
    /// existing entry of the same name **without saying so**. User scope is one file for the whole
    /// machine, so Swarm and Swarm Dev each offered a command claiming that one entry and whichever
    ...
    ///
    /// Every value is single quoted because a path can hold a space, and Swarm Dev's does.
    ...
    /// `CODEX_HOME`. Swarm already runs one app-server process per chat, so a per-process override
    ...
    /// `--mcp-config` file is. Nil attachment means no Swarm bridge, which is every test that
    /// did not ask for one.
    ...
    /// `Tools/test-core.sh` mirrors only SwarmCore into a package with no executable targets, so
    ...
    public static func shimPath(
        ...
        if let override = environment["SWARM_UI_BRIDGE_SHIM"], !override.isEmpty {
            return FileManager.default.isExecutableFile(atPath: override) ? override : nil
        }
        guard let executable else { return nil }
        let candidate = (executable as NSString).deletingLastPathComponent + "/swarm-bridge"
