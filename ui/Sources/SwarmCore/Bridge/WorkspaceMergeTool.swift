...
/// What Swarm could find out about a workspace's pull request, in one value.
///
...
/// against the template the owner may have edited in Settings, carrying Swarm's own merge rules,
...
///
/// A merge tool was designed and turned down once, and the reasoning was right at the time. Swarm
...
/// is merged", "Swarm cannot undo this". A tool reading `can_merge: true` skipped the only
/// protective part of the whole arrangement.
///
/// Swarm does not merge any more. `GitHub.merge` and `deleteRemoteBranch` are gone. Merging is a
...
///
/// That list is for tools Swarm answers a permission question about on its own behalf, and its own
...
public struct WorkspaceMergeTool: BridgeToolHandling {
    ...
    /// gh's complaint without the command line Swarm built to provoke it.
    ///
