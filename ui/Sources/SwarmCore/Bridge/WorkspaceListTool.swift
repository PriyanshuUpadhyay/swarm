...
/// `workspace_list`: the workspaces Swarm has, for a client that is sitting in none.
///
...
///
/// Every field except the GitHub block comes out of Swarm's own database, so the default call is a
...
/// loop, and the answer Swarm gives to that question is `notify_when_done`, a fact put in the
...
public struct WorkspaceListTool: BridgeToolHandling {
    ...
    /// over, so the workspace falls back to what Swarm's own database can say about it.
