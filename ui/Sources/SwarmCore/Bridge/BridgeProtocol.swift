...
/// The contract between `swarm-bridge`, the stdio shim Swarm ships inside its own bundle, and the
...
/// share between Swarm and Swarm Dev, which are a documented permanent pair. So the registered
...
/// replaced while Swarm is running and the CLI launches whatever binary is at the path its config
...
public enum BridgeProtocol {
    ...
    ///
    /// The skew to design for is a NEW shim meeting an OLDER running Swarm: the bundle is replaced
    ...
    public static let socketVariable = "SWARM_UI_BRIDGE_SOCKET"
    ...
    public static let tokenVariable = "SWARM_UI_BRIDGE_TOKEN"
    ...
    /// more.
    public static let roleVariable = "SWARM_UI_BRIDGE_ROLE"
    ...
    public static func problem(with hello: BridgeHello) -> String? {
        ...
        return """
            This copy of Swarm speaks bridge protocol \(version) and swarm-bridge \
    ...
    /// sentence cannot serve both. A session token is minted for a process Swarm is about to
    /// launch and is held in memory only, so a quit really does retire it and reopening Swarm
    ...
    /// the owner regenerates it. Sending that caller away to quit and reopen Swarm is sending it
