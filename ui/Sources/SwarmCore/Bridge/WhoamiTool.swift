...
/// Swarm and carried in the shim's environment, so the chain the bridge has to get right is token
...
public struct WhoamiTool: BridgeToolHandling {
    ...
    public func call(
        ...
        do {
            ...
            case .ownerClient:
                ...
                // person; which door they came through is Swarm's business, not the agent's.
                answer["created_by"] = .string("owner")
            }
    ...
    /// wants is which one. A client outside Swarm asks it to find out whether it reached anything
    /// at all, and if so which copy: the owner runs Swarm and Swarm Dev at once, both listening on
