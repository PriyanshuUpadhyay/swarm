...
/// ends by saying Swarm will not start a workspace in a repository it does not know about, which
...
public enum ProjectHideTrouble: Sendable, Equatable {
    ...
    /// Swarm has no projects, so there is nothing to hide or to bring back.
    case nothingRegistered(tool: String)
    /// A name, path or id that matches nothing. Carries what Swarm does have, so the next call can
    ...
    ///
    /// Diagnosed by asking Swarm's own project list rather than by reading the words off an error,
