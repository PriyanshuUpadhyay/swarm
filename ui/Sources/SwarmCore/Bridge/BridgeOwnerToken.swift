...
/// session token. A session token is minted for a process Swarm is about to launch, the argv
...
///
/// The standalone token identifies no process Swarm starts. It identifies a client configuration
/// that Swarm hands to a person once and then never sees again: it is pasted into the user's own
...
/// readable by anything running as the user. Keeping Swarm's copy in the Keychain would protect
...
///
/// Third, a Keychain item is access-controlled by signing identity, and Swarm is re-signed ad hoc
...
public struct BridgeOwnerToken: Sendable {
    /// Where it lives: beside the database, so the separation between Swarm and Swarm Dev that
    ...
    /// nothing to lose by minting again, and the alternative is a Swarm that cannot be coupled to
    ...
    ///
    /// This is revocation. The old token is not written anywhere else by Swarm, so overwriting the
