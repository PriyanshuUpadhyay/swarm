...
/// One browser pane, as much of it as Swarm's own toolbar knows.
///
...
public struct BrowserPaneReport: Sendable, Equatable {
    ...
    /// Why this pane is showing Swarm's own did-not-load state instead of a page, or nil when it
    /// is showing a page.
    ///
    ...
    /// nothing and `browser_screenshot` with Swarm's error card, which a model reads as a blank
    ...
    /// so. The rest is Swarm's toolbar read out loud.
    public var json: JSONValue {
        .object([
