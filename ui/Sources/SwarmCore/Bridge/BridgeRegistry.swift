...
///
/// Minted by Swarm and handed to the CLI through the shim's environment, never claimed by the
...
public final class BridgeRegistry: Sendable {
    private struct State {
        ...
        ///
        /// Ask Swarm, and nothing else today. It needs to be here for one reason: the config sweep
    ...
    /// because it is a different kind of act. `mint` invents a token for a process Swarm is about
    /// to start; this accepts one Swarm stored the first time somebody asked to couple a client of
