...
/// the test target cannot see `Sources/Swarm`, so a refusal composed there is a refusal nothing
...
///
/// `Plan` rather than the bare noun for two reasons. `Sources/Swarm` already has a `Reveal`, which
