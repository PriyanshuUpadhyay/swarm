...
/// Turning the name a client typed into a project Swarm already has.
///
...
/// Swarm printed, the path on disk, the name in the sidebar. The id and the path are unique in the
...
public enum BridgeProjectLookup: Sendable {
    ...
    ///
    /// `FolderPath.sameFolder` on both sides, so `~/dev/swarm`, `/Users/x/dev/swarm` and a path
