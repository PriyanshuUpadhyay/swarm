...
/// What is in a tab, one case per kind, each carrying only what Swarm already knows.
///
...
/// A swarm agent tab, using only the bus state Swarm already polls for the tab itself.
...
/// A terminal tab, as much of one as Swarm knows without asking a shell anything.
public struct WorkspaceTabTerminal: Sendable, Equatable {
    /// Where its shell was started, which is the workspace's worktree. Swarm does not follow a
    /// `cd`: reading a live working directory means asking tmux or walking `/proc`, and neither
    /// belongs in a listing.
    public var directory: String
    /// Whether a shell has actually been forked for it in this run of Swarm.
    ///
