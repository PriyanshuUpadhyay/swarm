...
/// Whether the owner's own Claude Code has already been told about this copy of Swarm.
///
...
/// and two values Swarm itself minted.
///
/// **It never writes.** Swarm does not compose a person's own configuration file, which is the
...
/// from under an entry this app minted, and which says at length why that is repairing Swarm's own
/// record rather than editing somebody's setup.
public enum BridgeUserRegistration {
    /// What the user scope says about this copy of Swarm.
    ///
    ...
    public enum State: String, Sendable, Hashable {
        /// An entry under this Swarm's name, pointing at this Swarm's shim, with this Swarm's
        /// socket and token in it.
        case registered
        /// No entry, or one that names this Swarm and points somewhere else. Those two are one
        /// case on purpose: an entry left behind by a copy of Swarm that has been replaced is a
    ...
    ///   - name: the server name this copy of Swarm registers under. Always
    ///     `BridgeRegistration.ownerServerName` in the app; passed in so a test can name one.
    ///   - attachment: what this copy of Swarm would hand out today, or nil when it has no bridge
    ...
    /// Whether an entry is this Swarm's rather than something else wearing its name.
    ///
