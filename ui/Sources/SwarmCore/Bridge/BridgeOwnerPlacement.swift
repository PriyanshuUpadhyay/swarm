...
///
/// The owner registers Swarm in their own `~/.claude.json` at user scope, and Claude Code applies
/// user scope to every session on the machine, including every session Swarm launches. So every
/// workspace agent ran two shims: its own, on its session token, and the owner's, on the standalone
/// token. Measured with `lsof` on 15 September 2026: each Swarm-launched `claude` had one of each,
