...
public struct BridgeToolbox: Sendable {
    ...
    /// a bridge tool unattended therefore needs an answer of its own, either a grant Swarm makes
    /// for its own tools or an allow rule in that agent's settings. `BridgeToolApproval` is
    /// where that was answered, and its head says which tools Swarm answers for and why the rest
