...
///
/// **They run in a content world of Swarm's own, not the page's.** A content world shares the DOM
/// and nothing else, so the elements are the real ones while `document.querySelectorAll`,
/// `HTMLInputElement.prototype` and the map of references are Swarm's. A page cannot replace the
...
public enum BrowserAgentScript {
    /// The name of Swarm's content world. One per web view, and WebKit clears its globals when a
    /// document is replaced, which is exactly when every reference ought to stop meaning anything.
    public static let worldName = "swarm-agent"
    ...
    private static let prelude = """
        const swarm = globalThis.__swarmAgent || (globalThis.__swarmAgent = { refs: new Map(), next: 1 });
    ...
    public static let snapshot = prelude + """
        ...
        for (let node = walker.currentNode; node; node = walker.nextNode()) {
          ...
          const ref = swarm.next;
          swarm.next += 1;
          swarm.refs.set(ref, node);
    ...
    private static let lookup = """
        const element = swarm.refs.get(ref);
...
///
/// A content world has its own `console` object, so wrapping Swarm's would hear nothing the page
...
public enum BrowserConsoleScript {
    ...
    public static let handlerName = "swarmConsole"

    public static let source = """
        (function () {
          if (window.__swarmConsole) { return; }
          Object.defineProperty(window, "__swarmConsole", { value: true });
