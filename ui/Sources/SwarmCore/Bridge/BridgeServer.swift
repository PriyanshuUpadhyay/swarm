...
/// Swarm's end of the bridge: one listening socket for the whole app, one connection per running
...
public final class BridgeServer: Sendable {
    ...
    public func register(session: Session, workspace: Workspace) -> BridgeHandle? {
        guard let shimPath = BridgeRegistration.shimPath() else {
            note("no swarm-bridge beside the running executable, so \(session.id) gets no bridge")
            return nil
        }
    ...
    /// The same, for the conversation that belongs to Swarm rather than to a workspace.
    ///
    ...
    /// names a shim binary and a socket that will answer it with "Swarm does not recognise this
    ...
    /// the same fingerprint as the socket beside it, so Swarm and Swarm Dev cannot read each
    ...
    /// running Swarm is told about the version rather than about an unknown token, which is what
