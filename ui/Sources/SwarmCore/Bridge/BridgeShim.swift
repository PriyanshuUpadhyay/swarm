...
/// The whole of `swarm-bridge`, the stdio shim Swarm ships in its own bundle.
///
/// It speaks no MCP. After a hello it copies lines from stdin to the socket and from the socket to
/// stdout, and Swarm answers `initialize`, `tools/list` and `tools/call` at the other end. That
...
///
/// It lives in SwarmCore rather than in the executable target because `Tools/test-core.sh` mirrors
/// only SwarmCore and its tests into a throwaway package: an executable target is invisible to the
...
public enum BridgeShim {
    ...
    /// exited 1" says nothing that "Swarm is not running" does not say better.
    public enum Exit {
        public static let ok: Int32 = 0
        public static let notConfigured: Int32 = 64
        public static let cannotReachSwarm: Int32 = 69
    ...
    public static func run(
        ...
        do {
            ...
            // The ordinary case, and it deserves a sentence rather than an errno: Swarm has quit
            // and somebody is running the CLI by hand from a config file Swarm left behind.
            complain("swarm-bridge could not reach Swarm on \(socketPath). Is Swarm running?")
            return Exit.cannotReachSwarm
        }
        ...
        guard let data = try? JSONEncoder().encode(hello) else {
            complain("swarm-bridge could not build its hello frame.")
            return Exit.notConfigured
        }
        ...
        // the handler below closed the socket, which is an ordinary shutdown, or Swarm went away
        // while the CLI was still talking to it. The shim could not tell them apart and reported
        // both as success, so a `tools/call` sent to a Swarm that quit mid-answer left the CLI
    ...
    /// stdin to the socket, off the same readability handler the rest of Swarm uses for a pipe.
    ///
    ...
    static func missingEnvironment(_ environment: [String: String]) -> String? {
        ...
        return """
            swarm-bridge is launched by Swarm and takes \(BridgeProtocol.socketVariable) and \
