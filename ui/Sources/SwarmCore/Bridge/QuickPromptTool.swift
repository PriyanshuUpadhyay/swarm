...
///
/// The panel seeds Swarm's built-ins the first time it is opened, not at launch, so on a copy of
/// Swarm whose composer panel has never been opened the table is genuinely empty. A tool reading
/// `quickPrompts()` there would report an empty library, an agent asked to add "Explain changes"
/// would write a second copy of a prompt Swarm is about to insert, and the owner would open the
...
enum QuickPromptCall {
    ...
    /// cleared, and a blank symbol is Swarm's default rather than a hole down the left of the row.
...
///
/// The one thing it can write is Swarm's own built-in, on a database whose panel has never been
...
public struct QuickPromptCreateTool: BridgeToolHandling {
    ...
    ///
    /// `.workspace` is the case worth defending. The owner mostly talks to Swarm from inside Swarm,
    ...
    public func call(
        ...
        do {
            ...
            // does not sit above Swarm's own built-in when the panel finally inserts it.
...
/// library that belongs to no workspace, and Swarm keeps no copy of what was there before. Two
...
/// the whole reason Swarm asks before archiving is that the answer is sometimes no and there is
