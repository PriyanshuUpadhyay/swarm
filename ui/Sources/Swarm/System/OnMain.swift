...
/// queue if we are not.
///
/// **This exists for the place where the main-actor guarantee is somebody else's.** Every
/// other `MainActor.assumeIsolated` in this app is backed by a guarantee the app itself makes: a
/// notification it posted, a callback it registered on a queue it chose. The exception is:
///
...
/// SwiftTerm is pinned open-ended in `Package.swift`, so a resolve can move it. And the failure
/// mode of `assumeIsolated` being wrong is not a wrong pixel, it is a **fatal trap**: a terminal
/// exiting would take the app down with every agent's unsaved work in it. That is a large
/// consequence for a guarantee the library does not write down.
///
