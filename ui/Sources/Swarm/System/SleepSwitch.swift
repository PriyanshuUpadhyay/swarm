...
/// Swarm's half of the conversation with `swarm-sleep-helper`: the privileged daemon that turns the
...
/// best effort and says so: a build that is not signed with Swarm's certificate cannot register a
...
final class SleepSwitch {
    ...
    /// Puts the switch back, whatever a session thought. Called on the way out, so quitting Swarm
    ...
    /// was unreachable, the error handler ran on XPC's queue and Swarm stopped at launch in
...
/// helper is a hundred lines with no dependencies, and linking the whole of `SwarmCore` into a
