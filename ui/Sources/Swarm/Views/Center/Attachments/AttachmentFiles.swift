...
enum AttachmentFiles {
    ...
    /// The most Swarm will take off a clipboard before it has looked at it.
    ///
    ...
    /// Bytes Swarm made itself, written straight out under the name it chose.
    ///
    ...
    /// Takes a copy back off the disk. Only ever called for a copy Swarm made and that has not
    ...
    /// Makes the attachments folder invisible to git, which is not a nicety: Swarm's whole premise
    ...
    ///
    /// The arrangement itself is `WorktreeScratch`, in SwarmCore, because attachments are not the
    /// only thing Swarm writes into somebody else's checkout and the rule has to be one rule. See
