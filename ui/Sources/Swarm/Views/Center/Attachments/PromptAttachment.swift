...
/// Swarm runs is spawned with the worktree as its working directory, so a relative path inside it
...
struct PromptAttachment: Identifiable, Hashable, Codable, Sendable {
    ...
    var source: String = ""
    /// True when Swarm made this copy under `.swarm/attachments`, which is what makes removing the
    ...
    /// are the honest answers for something Swarm is only reading back: it is not a copy this
...
/// Where an attachment came from, before it is a file Swarm can point at.
///
...
enum AttachmentSource: Hashable, Sendable {
    ...
    /// Words Swarm generated that the agent is meant to read as a file: a CI log fetched for a
