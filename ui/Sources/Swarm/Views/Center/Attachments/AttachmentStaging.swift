...
/// one: `.swarm/attachments/<id>/<name>`, relative to the staging root. That is what makes the
...
enum AttachmentStaging {
    /// Under the system temporary directory rather than in Swarm's application support, because
    ...
    static func directory(draftID: String) -> String {
        let root = (NSTemporaryDirectory() as NSString).appendingPathComponent("swarm-drafts")
