...
struct ComposerPlansView: View {
    ...
    private func load() async {
        ...
        do {
            ...
            app.notice = SwarmNotice(message: "Could not load saved plans: \(error.localizedDescription)")
        }
    }
        ...
        Task { @MainActor in
            ...
            if inNewConversation {
                guard let model, let session = await model.createSession(
                    ...
                    app.notice = SwarmNotice(message: "Could not create the implementation conversation.")
                    return
                }
