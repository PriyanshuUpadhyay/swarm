...
struct ComposerView: View {
    ...
    /// What the empty box says. The default is what every chat in a worktree says; Ask Swarm
    ...
    private func fork(onto kind: AgentKind, with controls: ComposerControls) {
        ...
        // Ask Swarm has no workspace and therefore no tab beside this one to fork into. Its
        ...
        Task { @MainActor in
            ...
            guard let session = await model.createSession(
                ...
                app.notice = SwarmNotice(message: BackendChange.forkFailureNotice(to: kind))
                return
            }
            ...
            app.notice = SwarmNotice(message: BackendChange.forkNotice(title: title, from: from))
        }
    }
    ...
    private func send() {
        ...
        if let question = SideConversation.question(in: transcript.draft) {
            guard canOpenSideConversation, let model else {
                app.notice = SwarmNotice(message: "Use /btw in a workspace chat to open a side conversation.")
                return
            }
            // The command itself belongs to Swarm. Leave review comments on the main chat.
        ...
        if onDismiss != nil,
           ChatClearCommand.matches(transcript.draft) || ChatCloseCommand.matches(transcript.draft) {
            app.notice = SwarmNotice(message: "Use Keep and start new in the side conversation menu.")
            return
        }
        ...
        // path that is not there any more only teaches the agent that Swarm lies about paths. The
    ...
    /// A chip opens the file where every other file in Swarm opens: the review tab, through
    ...
    private func prepare() async {
        ...
        // Swarm that sits above every project. See `ComposerDefaults.resolve`.
        ...
        // here, because it has no key for an output style: Swarm's TOML schema does not carry one
        // and Claude Code's own `.claude/settings.json` is already read by the CLI itself, so
        // copying its value up into this picker would be Swarm claiming to have chosen it.
