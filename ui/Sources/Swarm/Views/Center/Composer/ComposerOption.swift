...
struct ComposerOption: Identifiable, Hashable {
    ...
    /// a word on a scale, and a sentence under either would be Swarm inventing copy about
    ...
    /// Models screen can pin one Swarm has no name for, and `opus-5-1m` in a settings file is a
    ...
    /// Falls back to a tidied form of the raw value, so a model set in a settings file that Swarm
