...
final class ComposerTextView: NSTextView, HoverQuickLookSource {
    ...
    ///
    /// The rules are `PastedAttachment.plan`, in SwarmCore, where they can be asserted on. This is
...
extension ComposerTextView {
    ...
    private func chip(at point: CGPoint) -> HoveredChip? {
        ...
        // Swarm injected is added when a turn is composed, which is after this box has let go of
        // it. See `InlineChip`.
...
extension ComposerTextView {
    ...
    ///
    /// **The copy under `.swarm/attachments` deliberately stays.** It is deleted when the turn
