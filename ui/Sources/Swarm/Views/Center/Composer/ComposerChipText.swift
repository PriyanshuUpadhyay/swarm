...
/// turns Swarm composes itself. A pull request turn names a file in its sentence and the reader can
...
enum InlineChip: Equatable, Sendable {
    ...
    /// Words Swarm put in the message itself. Carried whole rather than as a key into a lookup, so
    ...
    /// What the chip reads. Swarm's own instruction files read as the button that sent them, the
    ...
    /// Whether this chip stands for instructions Swarm gave the agent, by either road.
...
final class AttachmentChipCell: NSTextAttachmentCell {
    ...
    /// Swarm's own files: the same `doc.text` the button under the bubble used to carry, kept so
