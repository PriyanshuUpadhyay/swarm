...
struct BrowserTabView: View {
    ...
    private var host: BrowserPaneHost {
        BrowserPaneHost(
            ...
            report: { [weak app] notice in
                app?.alert = SwarmAlert(title: notice.title, message: notice.message)
            }
        )
    }
    ...
    private func prepareRegion() async {
        ...
        do {
            ...
            cancelRegion()
            app.alert = SwarmAlert(title: "That page could not be captured", message: error.readableMessage)
        }
    }
    ...
    private func capture() {
        ...
        Task {
            ...
            app.alert = SwarmAlert(title: "That screenshot was not attached", message: failure)
        }
    }

    /// The pane's own menu with what Swarm can do with this page on top of it, which is what a
    ...
    /// here that hands the page to something outside Swarm.
    ///
