...
final class BrowserSession {
    ...
    /// comes back is what the reader can see, and editing it further would be Swarm deciding which
    ...
    /// Runs one of Swarm's own scripts in the page.
    ///
    ...
    /// are, and the head of `BrowserPaneCommand` argues why Swarm offers no tool that would want
    /// an arbitrary one.
    ///
    ...
    private func findIcon(for origin: String) async {
        for delay in BrowserFavicon.attempts {
            ...
            // The page moved off this origin while Swarm was waiting, so any answer now would be
    ...
    /// answer is a Swarm tab rather than a panel of WebKit's own.
    ...
    ///
    /// Whether it goes up at all, what Swarm's own line above it says and how much of the page's
    ...
    /// index, and the only way to get one would be to run a script of Swarm's own over somebody
...
/// One of Swarm's own scripts answered with something that is not what it returns.
///
