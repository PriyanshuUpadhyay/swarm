...
/// other is what used to leave this pane with a title and a single sentence in it. Swarm does not
...
/// `parent_tool_use_id` and Swarm stored every one of them, so the pane falls back to those. See
    ...
    private func load() async {
        ...
        let result = await Task.detached { () -> Result<SubagentReading, SubagentOutput.Failure> in
            ...
            case .failure(let reason):
                // **What Swarm saw, when the CLI has not written anything to read.** This is the
