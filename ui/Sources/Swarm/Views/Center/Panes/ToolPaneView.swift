...
struct ToolPaneView: View {
    ...
    /// straight after a relaunch would be handed `SWARM_UI_PORT=0`.
    ///
    ...
    @State private var terminalToggleEpoch = 0
        ...
        return { excerpt in
            Task { @MainActor in
                if let failure = await TerminalExcerptHandoff.attach(excerpt, to: destination, sessionID: sessionID) {
                    app.notice = SwarmNotice(message: failure)
                }
            }
        }
    }
