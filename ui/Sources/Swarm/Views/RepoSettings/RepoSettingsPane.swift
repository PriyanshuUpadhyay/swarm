...
enum RepoSettingsPane: String, CaseIterable, Hashable {
    ...
    /// Which pane a window opens on, from `SWARM_UI_PANE=workspaces|scripts|instructions`.
    ///
    ...
    static var requested: RepoSettingsPane? {
        guard let named = ProcessInfo.processInfo.environment["SWARM_UI_PANE"] else { return nil }
