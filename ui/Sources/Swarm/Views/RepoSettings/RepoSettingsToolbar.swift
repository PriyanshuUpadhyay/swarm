...
/// else in Swarm, and it did not look like the Mac either.
///
/// **Why the toolbar and not `PanelTabs`**, which is Swarm's own strip and would have been three
/// lines. `PanelTabs` is a control for the inside of a panel: it divides a width it is handed
/// between two or three words, and it is drawn in Swarm's own greys precisely because the things
/// around it are Swarm's. This row is not inside anything. It is the chrome at the top of a
...
/// Swarm's strip here would have made the two settings windows differ in a second way rather than
/// stop them differing in the first, and it would have put a hand drawn control in the one row of
/// this window that belongs to the system. The place Swarm draws its own tabs is where it owns
/// what is around them.
///
