...
/// created, what it tells an agent when Swarm asks for something, and how to stop tracking it.
///
...
/// Swarm RUNS in it, and what it SAYS on the project's behalf. Which one is showing is chosen in
...
/// explicit about. The name, mark and colour are Swarm's own record of a folder and live in its
...
struct RepoSettingsView: View {
    ...
    /// change it.
    ///
    /// Swarm looks for artwork once, when a project is added, at the places a favicon and an
    ...
    /// name, or have the letters back. A project added before Swarm knew how to look has never been searched, and its button
    ...
    /// Swarm searched, and that sentence arrived middle-truncated, so the one message a user
    ...
    /// merely looks accent coloured is `Palette.accent`, which belongs to Swarm and not to any
...
struct SettingsDestinationLabel: View {
    ...
    ///
    /// Only ever a `.conductor` file: Swarm reads those so an existing repository works with
    /// nothing to configure, and writes its own. Both files then state the setting, Swarm's wins
...
/// The ten colours Swarm hands out, and a picker for any other.
///
