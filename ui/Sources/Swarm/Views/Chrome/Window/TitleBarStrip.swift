...
/// as well and only SwiftUI knew how wide that control came out. That menu is the workspace row's
/// right click menu in the sidebar now, so the accessory is the band and nothing else, and the
/// second number went with it rather than being left at zero for the next reader to wonder about.
/// See `WorkspaceMenuItems`.
///
...
/// `SwarmWindowToolbar`'s flexible spacer, so a frame set in one step moved the field 379 points
...
/// a workspace with a state in it that changes without anybody touching Swarm, so it took the top
...
final class TitleBarStripController: NSTitlebarAccessoryViewController {
    ...
    /// out in what the trailing accessory leaves it, and `SwarmWindowToolbar`'s flexible spacer
