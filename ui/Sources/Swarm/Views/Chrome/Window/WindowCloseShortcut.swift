...
///
/// Swarm's Cmd+W closes the tab, deliberately: the window lists every workspace and every agent in
...
/// carry. `SwarmCommands` retired Close Session: a conversation IS a tab in the centre strip, and
/// the old item closed the workspace's active chat whatever was actually in front of you.
///
/// It is done in AppKit rather than in `SwarmCommands` because SwiftUI has no command group that
...
/// group. Adding a second Close of Swarm's own would have meant two items called Close in one
