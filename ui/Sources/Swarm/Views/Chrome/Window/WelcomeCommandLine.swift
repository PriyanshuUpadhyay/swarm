...
/// The welcome window's third screen: the one command that lets the owner drive Swarm from a
/// terminal they already had open.
///
/// **It is an offer, and everything on it is arranged to read as one.** Swarm works perfectly
/// without the bridge, so there is no control here that has to be satisfied before anybody can
/// leave: the primary button in the footer says "Start using Swarm" and does exactly that whether
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Metrics.pane - Metrics.spacingSmall) {
            ...
            VStack(alignment: .leading, spacing: Metrics.spacing) {
                ...
                // rather than a second button. A "Skip" next to a "Start using Swarm" would be two
