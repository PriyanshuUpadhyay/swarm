...
/// between them, not the plate, and it is `SwarmWindowToolbar`'s `ToolbarSpacer`.
///
...
struct WindowTitleControl: View {
    /// Handed in rather than read from the environment, as `SwarmWindowToolbar` takes it, because
    ...
    var body: some View {
        Group {
            if let workspace = renaming {
                ...
                label(workspace)
            } else if app.selection != .home {
                // Ask Swarm's full pane. There is nothing to rename, so it is text and only text.
                //
                // Home draws no title at all. It said "Swarm", which names the application in its
                // own window, and the owner found a title there odd: the search field and the
                // sidebar already say where you are. The window keeps its title for the Window
                // menu and Mission Control. See `WindowTitle`.
                titleText
