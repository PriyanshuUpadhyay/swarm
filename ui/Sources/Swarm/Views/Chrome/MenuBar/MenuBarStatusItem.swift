...
final class MenuBarStatusItem: NSObject, NSMenuDelegate {
    ...
    /// Swarm's own mark, out of the bundle, as a template image. Drawn when no starred metric has a
    /// figure yet, when figures are switched off, and when every provider is.
    ///
    /// `Resources/SwarmMenuBar.pdf`, drawn by `Tools/icon/menubar.py` from the same eased lane the
    ...
    private static let mark: NSImage? = {
        guard let url = Bundle.main.url(forResource: "SwarmMenuBar", withExtension: "pdf"),
    ...
    /// Swarm speaks from while it is behind another window cannot disagree.
