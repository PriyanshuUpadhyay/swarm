...
struct SwarmCommands: Commands {
    ...
    /// Which shape the sidebar is in, read from the preference the pane itself writes. See the
    /// View menu's own group for why the item is up here as well as down there.
    ...
    var body: some Commands {
        // Swarm's own About window rather than AppKit's panel. `orderFrontStandardAboutPanel` can
        // be handed the bundle's keys and a paragraph of credits and nothing else: no mark, no
        // typeface, no ground. Everything that makes this app recognisable as the one runswarm.app
        ...
        CommandGroup(replacing: .newItem) {
            MenuCommand(.newWorkspace) {
                ...
                NotificationCenter.default.post(name: .swarmNewWorkspace, object: nil)
            }
            ...
            MenuCommand(.newWorkspaceFromPullRequest) {
                NotificationCenter.default.post(
                    name: .swarmNewWorkspace, object: nil,
                    userInfo: [Notification.swarmPullRequestKey: true]
                )
            }
            .disabled(model.repos.isEmpty)
            ...
            // Reached from the menu as well as from the toolbar glyph that appears while Ask Swarm
            // is open. See the note beside this item in `MenuBarCatalogue`: the toolbar button is
            // a control you have to be looking at Ask Swarm to see, so it could not answer
            ...
            MenuCommand(.newAskConversation) {
                NotificationCenter.default.post(name: .swarmNewAskConversation, object: nil)
            }
            ...
            .disabled(model.selectedModel == nil && !model.isAskInFront)
            ...
            // Shift+Cmd+N, which nothing in Swarm held. It is the initial of the thing, which is
            ...
            .disabled(isMainWindowFocused != true || (model.isAskInFront ? !canCloseAskTab : closableTab == nil))
        ...
        CommandGroup(after: .pasteboard) {
            ...
            MenuCommand(.search) {
                NotificationCenter.default.post(name: .swarmFocusSearch, object: nil)
            }
        ...
        // How the sidebar is grouped, at the head of the View menu, because what the pane IS comes
        // before what is shown beside it: the items under this split the centre column and toggle
        // the two side columns, all of which are about the window's furniture rather than about
        // the list somebody reads.
        //
        // The same preference the sidebar's own filter menu binds, so the two controls can never
        // disagree. A menu bar is a map of what the app can do, which is the whole reason this is
        // here as well: a shape you can only reach by opening a control at the foot of one pane is
        // a shape most people never find. See `SidebarGrouping`.
        ...
        //
        // **The grouping picker is the first thing in this group rather than a group of its own,
        // and that is a limit rather than a preference.** `CommandsBuilder` takes ten members, this
        // body already had ten, and an eleventh compiles locally against the cached build and fails
        // on a clean one with "extra argument in call" pointing at the closing brace of `body`.
        ...
        CommandGroup(after: .sidebar) {
            ...
            // the row itself now.
            // Opens and closes the panel rather than going anywhere.
            MenuCommand(.goToAsk) {
                AskPanelModel.shared.toggle(app: model)
            }
        }
        ...
        CommandMenu("Workspace") {
            ...
            MenuCommand(.renameWorkspace) {
                ...
                NotificationCenter.default.post(
                    name: .swarmRenameWorkspace, object: nil,
                    userInfo: [Notification.swarmWorkspaceIDKey: workspace.id.rawValue]
                )
            }
    ...
    private var canCycleCentreTabs: Bool {
        if model.isAskInFront { return model.ask.sessions.count > 1 }
    ...
    /// keyboard.
    ///
    /// Swarm bound neither these nor Ctrl+Tab: the full shortcut grep found 74 bindings and not one
    ...
    private func renameSelectedTab() {
        ...
        NotificationCenter.default.post(name: .swarmRenameTab, object: nil)
    }
    ...
    /// The last one closes too, and is replaced by a blank conversation. See `AskModel.close`.
    private var canCloseAskTab: Bool { model.isAskInFront && model.ask.selectedID != nil }

    private func closeSelectedTab() {
        if model.isAskInFront {
            if canCloseAskTab, let id = model.ask.selectedID { model.ask.requestClose(id) }
            return
        }
