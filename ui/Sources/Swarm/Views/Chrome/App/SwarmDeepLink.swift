...
/// Preserves Conductor deep links so existing scripts can hand work to Swarm unchanged.
@MainActor
enum SwarmDeepLink {
    /// The schemes this copy actually answers to, read off its own bundle rather than written out.
    ///
    /// It was the literal `"swarm"`, and `Tools/dev-build.sh` rebrands the registered scheme to
    /// `swarmdev` so the dev copy cannot swallow a link meant for the real one. The result was a
    /// dev build that registered `swarmdev://`, received them, and then refused every one with
