...
extension Notification.Name {
    ...
    static let swarmHandleURL = Notification.Name("swarmHandleURL")
}

/// Bridges macOS lifecycle callbacks into the observation-driven app without introducing a second state owner.
@MainActor
final class SwarmAppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    /// Handed over by `SwarmApp` once the scene exists. Explicit rather than a global, because the
    ...
    private let servicesProvider = SwarmServicesProvider()
    ...
    func attach(_ model: AppModel) {
        ...
        // And the welcome window needs it for one thing: whether this copy of Swarm has a bridge
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        ...
        // Here, at the earliest point there is, because everything that pays for it pays only for
        // what is left of it. A GUI app inherits a PATH with no homebrew and no version manager in
        // it, and the login shell is the only thing that knows what the user's real one is. It
        // runs in the background under a timeout and blocks nothing. See `LoginShellPath`.
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // bundle, which `swift run` and `.build/debug/Swarm` are not. See
        ...
        // Last, and after the main window exists, so the welcome window opens in front of Swarm
    ...
    /// restorable state" on every launch and quietly declines to restore the window. Swarm's
    ...
    /// False, because Swarm runs agents. Closing the only window must never be a silent way to end
    ...
    /// with Swarm behind three other windows, and a sheet nobody can see is the same failure again.
    @MainActor
    private func askBeforeQuitting(running: Int) {
        ...
        guard let window = NSApp.keyWindow ?? NSApp.mainWindow ?? NSApp.windows.first(where: \.isVisible) else {
            // No window to hang it on, which is a Swarm with its window closed and agents still
    ...
    /// is where somebody looks while Swarm is behind three other windows, and the question they
    ...
    /// Without this, macOS silently swallows every banner while Swarm is the frontmost app, and
    /// "frontmost, but looking at a different workspace" is precisely the case the suppression rule
    /// deliberately lets through. Whether to interrupt is Swarm's decision and it has already been
    ...
    /// show the answer being sent undoes that. Everything else brings Swarm forward.
    ///
