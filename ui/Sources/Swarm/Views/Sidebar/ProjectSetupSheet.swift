...
/// What Swarm offers instead of refusing a folder that is not a git repository.
///
/// Two answers, not one. Conductor's version of this dialog assumes a GitHub repository is what
/// you want, and for Swarm that is simply not true: worktrees, agents, diffs and merging back into
...
struct ProjectSetupSheet: View {
    ...
    /// Called with the folder once it is a repository Swarm can add, and with nil when the user
        ...
        HStack(spacing: Metrics.spacingWide) {
            ...
            case .failed(let failure):
                if failure.isUsableProject {
                    ...
                    // in it, so it is a project Swarm can run, and refusing to add it over a
    ...
    /// is then worked out and, where Swarm made it, undone. See `RepositoryStarter.abandon`.
    private func stop() {
        startTask?.cancel()
        startTask = nil
        isStopping = true
