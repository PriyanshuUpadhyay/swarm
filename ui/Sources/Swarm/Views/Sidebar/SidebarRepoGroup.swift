...
enum SidebarPaneRow: Identifiable {
    ...
    var id: String {
        ...
        // sessions table and its id is Swarm's own, unique across the app.
    ...
    /// The key a row under a workspace fades in by, and nil for every other kind of row.
    ///
    /// The row's own id, which already scopes a subagent by its workspace. See `SidebarView`'s
    /// `childArrival` for what it is fed to, and `id` above for why a subagent needs the scope.
    var childKey: String? {
        switch self {
        ...
        for group in groups {
            ...
            if group.workspaces.isEmpty, waiting.isEmpty, group.sessions.isEmpty {
                ...
                for workspace in group.workspaces {
                    ...
                    rows.append(contentsOf: SwarmSessionListing.workspaceChats(
                        group.sessions, workspaceID: workspace.id
                    ).dropFirst().map { .swarmSession($0, repoID: group.id) })
