...
/// accent colour whenever the table is first responder in the key window, and Swarm's accent is
/// its house blue, so switching away from Swarm and back with the pane holding the keyboard turned
...
/// accent (see `HomeRowBackground`), but that is an inset list, and the sidebar is glass. Replacing
/// it was not reliable on its own, so the table's own drawing is switched off as well: see
...
extension View {
    /// Puts Swarm's selection under a selectable sidebar row, and keeps the row's ink ordinary.
    ///
