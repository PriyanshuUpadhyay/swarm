...
struct TranscriptListView: View {
    ...
    private var entries: [TranscriptTableEntry] {
        ...
        // workspace for a pane to hang off, which is Ask Swarm. See `SubagentRunActions`.
        ...
        // things a worktree has. Ask Swarm uses this opening entry only for the space below the
        ...
        if let workspaceID = home.workspaceID {
            ...
            // Ask Swarm has no setup row to supply the space below its title bar. Keep that
        ...
        .overlay {
            if showsPlaceholder {
                TranscriptPlaceholderView(
                    ...
                    onSuggestion: { prompt in Task { await transcript.submit(prompt) } }
                )
    ...
    private func showSetupLogEnd(wasAsked: Bool) {
        guard transcript.rows.isEmpty else {
            ...
            // whenever setup is the last thing Swarm did to this workspace, which is the case a
