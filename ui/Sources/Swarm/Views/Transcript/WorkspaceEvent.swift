...
/// Something Swarm itself did to a workspace, as the transcript shows it.
///
...
///
/// One value for all of it, rather than a bespoke row per thing Swarm does. Setup and merge are
...
/// transcript's own directory, rather than in `SwarmCore` next to the stored model, because being
...
struct WorkspaceEvent: Identifiable, Equatable {
    ...
    /// transcript rather than something Swarm did to a workspace behind its back: an agent
    /// running `gh pr merge` in front of the reader needs no row from Swarm saying it happened.
