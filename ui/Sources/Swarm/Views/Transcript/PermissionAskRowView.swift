...
/// on Full access and never take it off, and Swarm ends up less safe than the refusal it replaced.
...
///
/// The rule text is the CLI's own `ruleContent`. Swarm composes none of it, and when the CLI
/// offers no rule (a path outside the worktree is the case that prompted this) there is no rule
/// button at all rather than one Swarm invented. The same is true of the CLI's two undocumented
...
struct PermissionAskRowView: View {
    ...
    /// stored against a project; a chat that has none cannot store one. Ask Swarm is that chat.
    ...
    private var header: some View {
        HStack(alignment: .firstTextBaseline, spacing: TranscriptLayout.glyphGap) {
            ...
            if !ask.reason.isEmpty {
                // The CLI's own sentence, not Swarm's paraphrase of it.
                Text(ask.reason)
                    .font(Typo.caption)
        ...
        VStack(alignment: .leading, spacing: Metrics.spacing) {
            ...
            HStack(spacing: Metrics.gutter) {
                ...
                HStack(spacing: Metrics.spacing) {
                    ...
                    // arbitration is AppKit's rather than anything Swarm asked for and it should
