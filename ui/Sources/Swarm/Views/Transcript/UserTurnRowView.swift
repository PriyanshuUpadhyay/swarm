...
/// `.swarm/attachments` tells them nothing they did not know. See `AttachmentTrailer` for the one
/// place that format is written and read.
///
/// Instructions Swarm appended to a turn it composed itself are drawn the same way, as a chip
...
struct UserTurnRowView: View {
    ...
    /// A chip opens where every other file in Swarm opens, which is the review tab, and it is the
    ...
    private func open(_ path: String) {
        // No workspace is Ask Swarm, which has no review pane for a file to open into. The chip
    ...
    ///
    /// For a block Swarm put in the message there is nothing to resolve and nothing to ask the disk:
