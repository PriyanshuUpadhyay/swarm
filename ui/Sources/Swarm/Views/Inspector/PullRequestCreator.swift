...
/// whether Swarm itself can talk to GitHub has no bearing on it. See `WorkspaceModel`.
///
...
struct PullRequestCreator: View {
    ...
    /// to open a pull request for, and Swarm knows that for free, so it says so here rather than
    ...
    /// Whether Swarm itself can talk to GitHub. It has no bearing on the button, which goes to the
    /// agent, and every bearing on whether this strip can be trusted when it says there is no pull
    /// request: signed out, Swarm simply never found out.
    ///
    ...
    var body: some View {
        HStack(spacing: InspectorLayout.gap) {
            ...
            if isWorking {
                ...
                // re-read when a turn ends and every six seconds while Swarm is frontmost, which
    ...
    private var helpText: String {
        ...
        // No path named. There are two, the project's own and Swarm's copy of the default, and
