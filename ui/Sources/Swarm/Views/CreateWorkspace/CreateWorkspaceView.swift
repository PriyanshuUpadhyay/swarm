...
struct CreateWorkspaceView: View {
    ...
    /// the project a workspace was last started in, or the repo of whatever is selected. Either way the project arrives decided, so the control
    ...
    /// The picker's Recent section, newest first. See `RecentProjects`.
    @State private var recentRepoIDs: [RepoID] = []
    ...
    /// The pull request or branch this workspace opens on, or nil for the route Swarm has always
    ...
    /// meeting Swarm read "New workspace" and had to infer that a workspace is a worktree on disk.
    ...
    var body: some View {
        ...
        .task(id: [app.workspaces.count, app.archivedRevision]) {
            recentRepoIDs = await app.recentProjects().map(\.id)
        }
    ...
    private var header: some View {
        HStack(spacing: Metrics.spacingSmall) {
            ...
            //
            // What the band is now is the sentence that was already after the title: "in swarm,
    ...
    private var projectControl: some View {
        ...
        if app.repos.count > 1 {
            Menu {
                ...
                // `RepoIconImage`.
                //
                // One picker keeps a single selection across both sections. A project can be in
                // both, so rows are tagged with their section too, and only one of them is ticked.
                // See `ProjectMenuGroup.Choice`.
                let groups = ProjectMenuGroup.grouped(app.repos, recent: recentRepoIDs)
                Picker("Project", selection: Binding(
                    get: { ProjectMenuGroup.choice(for: repoID, in: groups) },
                    set: { repoID = $0.repoID.rawValue.isEmpty ? nil : $0.repoID }
                )) {
                    ForEach(groups) { group in
                        Section(group.title) {
                            ForEach(group.repos) { candidate in
                                ...
                                .tag(ProjectMenuGroup.Choice(kind: group.kind, repoID: candidate.id))
                            }
                        }
                    }
                }
    ...
    private var hint: some View {
        if let checkout {
            ...
            // Swarm will name it, and the branch follows the name. A second sentence eight points
    ...
    /// two are different questions: what may be cut from includes every head a pull request
    /// speaks for, and what may be opened does not. See `WorkspaceCheckoutPlan.offeredBranches`.
    ...
    private func load() async {
        if repoID == nil {
            ...
            let recent = await app.recentProjects().first
    ...
    ///
    /// A branch one of Swarm's own workspaces holds is not a refusal at all: the window has nothing
    ...
    private func offer(_ chosen: WorkspaceCheckout) {
        ...
        if holder.isSwarmWorkspace, let repo, let held = WorkspaceCheckoutPlan.workspaceHolding(
    ...
    /// goes to the workspace that has it when it is one of Swarm's. They used to be left out,
    ...
    /// cut by hand, is as visible as one of Swarm's own. See `BranchHolder`.
    ...
    private func create() {
        ...
        // naming a path that is not there only teaches the agent that Swarm lies about paths.
        ...
        openWindow(id: SwarmApp.mainWindowID)
