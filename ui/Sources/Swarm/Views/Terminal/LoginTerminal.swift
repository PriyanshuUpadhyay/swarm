...
/// Keeping the process in Swarm avoids the Automation permission needed to control Terminal.app.
...
final class LoginTerminalSession {
    ...
    @ObservationIgnored let terminal: SwarmTerminalView
    ...
    init?(
        ...
        launch = TerminalLaunch(
            ...
            // An empty folder avoids macOS asking about home-directory access in Swarm's name.
            directory: FileManager.default.fileExists(atPath: directory)
                ? directory
                : AgentScratchDirectory.current()
        )

        terminal = SwarmTerminalView(frame: .zero)
