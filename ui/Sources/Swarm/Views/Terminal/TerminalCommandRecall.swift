...
final class TerminalCommandRecall {
    ...
    /// What Swarm itself last sent into a pane, which is preferred over what `ps` reports.
    ///
    ...
    /// the command was `npm run dev`. So the exact text wins whenever Swarm has it.
    ///
    ...
    // MARK: - What Swarm typed

    /// A line Swarm put into a pane itself.
    ///
    ...
    /// what Swarm last sent into this pane, so the next poll records it as the pane's own text
    ...
    /// script being typed into its tab by Swarm or started by hand. The offer is no longer true, so
    ...
    /// it is the wrong one to rely on alone: Swarm is developed in Swarm, it gets killed, force
