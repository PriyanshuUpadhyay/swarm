...
final class SwarmTerminalView: LocalProcessTerminalView {
    ...
    /// Set when Swarm is the one ending this shell rather than the shell ending by itself. The two
    ...
    private var terminalScheme: TerminalScheme = .swarm
    ...
    /// user reads everywhere else rather than at Swarm's own body size.
    ...
    /// Tells the terminal that Swarm is about to end this shell, ahead of whatever signal does it.
    ...
    fileprivate func handleProcessExit(_ status: Int32?) {
        ...
        hasExited = true
        // Swarm ended this one, so there is nobody to tell and nothing to close.
    ...
    /// SwiftTerm ships a palette that looks nothing like the rest of Swarm, so both the sixteen
...
private final class TerminalProcessObserver: LocalProcessTerminalViewDelegate {
    weak var owner: SwarmTerminalView?
...
final class TerminalHostView: NSView {
    ...
    func attach(_ view: SwarmTerminalView) {
...
struct TerminalView: NSViewRepresentable {
    ...
    var repo: Repo?
    var port: Int
    /// Columns to draw whatever the pane is worth, and 0 to follow the pane.
    var fixedColumns = 0
