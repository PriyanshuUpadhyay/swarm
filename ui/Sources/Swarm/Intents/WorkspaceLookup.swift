...
enum WorkspaceLookup {
    ...
    /// outside the app is the session row the runner marks `running`. Swarm resets those rows on
    ...
    /// somebody removed from under Swarm. None of those is worth failing an intent over, so the
