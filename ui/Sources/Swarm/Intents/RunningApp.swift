...
///
/// An intent is performed inside Swarm's own process, but not necessarily inside a Swarm that has
...
/// intent is one more caller of Swarm's existing entry points either way, never a second owner of
/// its state.
@MainActor
enum RunningApp {
    /// The state the window is running on, handed over by `SwarmAppDelegate` once the scene
    /// exists.
    ///
    /// Weak and explicit, the same bargain `SwarmServicesProvider`, `SwitchProbe` and
