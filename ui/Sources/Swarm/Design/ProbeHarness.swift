...
/// disk. A report is a `JSONValue` now (`SwarmCore`, `Sendable`, `Codable`, and with no case that
...
struct ProbeHarness {
    ...
    var outputPath: String {
        Self.value(for: flag) ?? (NSTemporaryDirectory() + "swarm-\(subject)-probe.json")
    }
    ...
    /// owner is using his own copy of Swarm, and an activation would take his keyboard. The two
    ...
    /// `SwarmAppDelegate` calls `attach`, and the two moments are not ordered against each other,
