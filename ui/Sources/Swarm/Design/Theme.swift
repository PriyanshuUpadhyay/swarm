...
///
/// **Ground is Swarm's own, and it is what a theme changes.** The surfaces and the rule between
...
/// step the other way, and a raised control. The Swarm theme's light members carry a slight cool
...
enum Palette {
    ...
    // steps of Swarm's ramp, for the reasons written on each. Never write any of the three as
    ...
    /// Ink on anything Swarm has filled with a colour of its own.
    ///
    ...
    /// of Swarm's own. That note says the system's third rung is what the system means for a
    /// disabled control and that Swarm's tertiary is deliberately not it, which leaves nothing in
    ...
    // `Palette.controlAccent`. Swarm's AccentColor asset supplies the default under Multicolour,
    ...
    /// Swarm's brand accent for identity, links and status ink.
    ///
    ...
    /// and its top half for light. Swarm `#4FD8C4` measures 10.6 to 1 on the dark surface and 1.7
    /// to 1 on the light one, so it can never be text on light; the ramp's answer for that is Swarm
    ...
    /// on it measures 5.2 to 1. Swarm itself cannot do this job: white on
    ...
    /// system blue here would be the one colour in Swarm that came from somewhere else. It is
    ...
    ///
    /// `accent` cannot do this job. Swarm `#4FD8C4` on Spatie Blue measures 3.0 to 1, under AA for
    ...
    /// numbers are somewhere `Tests/SwarmCoreTests` can walk them: see `PaletteInk`.
...
extension View {
    /// A `Button` that reads as a link, in Swarm's teal rather than the system accent.
    ///
...
extension ControlActiveState {
    ...
    ///
    /// AppKit draws a focus ring only in the key window, and every hand-drawn ring in Swarm was
