...
///
///     Swarm --menu-probe /tmp/menu.png [--menu-part menu|kinds|terminal|terminalKinds|browser|row|colour|worktree|project|projectHidden|filter|diffScope] [--menu-project <path>] [--menu-base <branch>]
///
...
/// through a real `SwarmTerminalView`'s right mouse handling rather than through `NSMenu.popUp`,
...
/// want `SWARM_UI_DB_PATH` set at a seeded scratch copy rather than the owner's.
///
...
enum MenuProbe {
    ...
    private static var outputPath: String {
        value(for: "--menu-probe") ?? (NSTemporaryDirectory() + "swarm-menu.png")
    }
    ...
    private enum Part: String {
        ...
        case terminalKinds
        /// A browser page's own menu, which is WebKit's and not Swarm's.
        case browser
    ...
    /// it up, through `SwarmTerminalView`, because AppKit adds items of its own on that path and
    ...
    private static let terminalPane: SwarmTerminalView? = {
        guard MenuProbe.part == .terminal else { return nil }
        return SwarmTerminalView(frame: NSRect(x: 0, y: 0, width: 600, height: 400))
    }()
    ...
    private static func build(model: AppModel?) -> NSMenu {
        ...
        case .filter:
            NSHostingMenu(rootView: SidebarFilterMenuItems(
                filter: .constant(.all),
                showsHiddenProjects: .constant(true),
                hiddenCount: 2,
                grouping: .constant(SidebarGrouping.status.rawValue)
            ))
        case .worktree:
