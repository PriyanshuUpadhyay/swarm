...
enum Snapshot {
    ...
    private static var directory: String {
        ...
        guard let index = arguments.firstIndex(of: "--snapshot"), index + 1 < arguments.count else {
            return NSTemporaryDirectory() + "swarm-shots"
        }
    ...
    static func refuseWithoutDatabase(flag: String) {
        guard ProcessInfo.processInfo.environment["SWARM_UI_DB_PATH"] == nil else { return }
        FileHandle.standardError.write(Data(
            "\(flag) captures against a database named by SWARM_UI_DB_PATH, and refuses to run without one.\n".utf8
        ))
        exit(1)
    }
    ...
    /// no such button, and the button had been in the source the whole time. `.build/debug/Swarm`
    ...
    /// which is `.build/<configuration>/Swarm` in a checkout and nothing at all in a shipped copy.
    ...
    private static var packageRoot: URL? {
        ...
        // .build/<configuration>/Swarm, so three levels up is the package.
        let root = executable
    ...
    /// the app it feels like and does not carry `SWARM_UI_DB_PATH`, so a test that drives a deep link
    ...
    ///
    ///     Swarm --open-url "swarm-ui://prompt=...&path=..."
    static func scheduleURLIfRequested() {
        ...
        Task { @MainActor in
            ...
            NotificationCenter.default.post(name: .swarmHandleURL, object: url)
        }
    }
    ...
    ///
    ///     Swarm --select w1 --expand-setup-log 8
    ///
    ...
    static func scheduleSetupLogExpansionIfRequested() {
        ...
        Task { @MainActor in
            ...
            NotificationCenter.default.post(name: .swarmExpandSetupLog, object: nil)
        }
        #endif
    }
    ...
    ///
    ///     Swarm --terminal-workspace plumage
    ///
    ...
    static func scheduleTerminalWorkspaceIfRequested() {
        ...
        Task { @MainActor in
            ...
            NotificationCenter.default.post(
                name: .swarmStartTerminalWorkspace, object: project
            )
        }
        #endif
    }
    ...
    ///
    ///     Swarm --notice 4 "Swarm named this workspace X. Its branch is still `y`, because ..."
    ///
    ...
    /// Swarm did something it did not do.
    static func scheduleNoticeIfRequested() {
        ...
        Task { @MainActor in
            ...
            NotificationCenter.default.post(name: .swarmShowNotice, object: message)

            guard let hold else { return }
            try? await Task.sleep(for: .seconds(hold.0))
            NotificationCenter.default.post(name: .swarmHoldNotice, object: true)
            try? await Task.sleep(for: .seconds(hold.1))
            NotificationCenter.default.post(name: .swarmHoldNotice, object: false)
        }
        #endif
    }
    ...
    ///
    ///     Swarm --select w1 --running w1,w2,w3
    ///     Swarm --select w1 --running w1,w2,w3/w1/
    ///
    ...
    static func scheduleRunningStateIfRequested() {
        ...
        Task { @MainActor in
            ...
            for (index, ids) in stages.enumerated() {
                ...
                NotificationCenter.default.post(name: .swarmCaptureRunning, object: ids)
            }
        }
        #endif
    }
    ...
    private static var windowCapturePath: String {
        ...
        guard let index = arguments.firstIndex(of: "--snapshot-window"), index + 1 < arguments.count else {
            return NSTemporaryDirectory() + "swarm-window.png"
        }
        return arguments[index + 1]
    }
    ...
    ///
    ///     Swarm --snapshot-window /tmp/shots/window.png [--window-size 900x700]
    ...
    static func scheduleWindowCapture() {
        ...
        Task { @MainActor in
            ...
            if let index = arguments.firstIndex(of: "--project-setup"), index + 1 < arguments.count {
                ...
                NotificationCenter.default.post(
                    name: .swarmOfferProjectSetup, object: arguments[index + 1]
                )
                try? await Task.sleep(for: .seconds(3))
            }
            ...
            // comparison against "Swarm" matched neither and every settings capture silently
            ...
            if wantsSettings || wantsRepoSettings || wantsAbout || wantsWelcome || wantsCreateWindow
                ...
                for _ in 0..<40 {
                    ...
                    if wantsCreateWindow {
                        ...
                        NotificationCenter.default.post(
                            name: .swarmOpenRepoSettings, object: repoSettingsProject
                        )
                    } else if wantsAbout {
    ...
    ///
    ///     Swarm --snapshot-gallery /tmp/shots [--gallery review-comments|inspector-tabs|diff-scope]
    ///
    ...
    /// item is in the same position: `SwarmCommands` replaces the `.appInfo` group, so the action
    ...
    /// to start without `SWARM_UI_DB_PATH`, so the `bootstrap` here can never touch the user's real
    /// workspaces.
...
extension Notification.Name {
    ...
    static let swarmCaptureRunning = Notification.Name("swarm.captureRunning")
    ...
    static let swarmExpandSetupLog = Notification.Name("swarm.expandSetupLog")
    ...
    static let swarmShowNotice = Notification.Name("swarm.showNotice")

    /// Carries a `Bool` saying whether the corner notice's countdown is on hold. Posted only by
    /// `Snapshot.scheduleNoticeIfRequested`, and only in a debug build. See
    /// `View.acceptsCaptureNoticeHold`.
    static let swarmHoldNotice = Notification.Name("swarm.holdNotice")
}
...
extension View {
    ...
    func acceptsCaptureRunningState(_ app: AppModel) -> some View {
        #if DEBUG
        return onReceive(NotificationCenter.default.publisher(for: .swarmCaptureRunning)) { note in
    ...
    func acceptsCaptureSetupLogExpansion(_ expand: @escaping @MainActor () -> Void) -> some View {
        #if DEBUG
        return onReceive(NotificationCenter.default.publisher(for: .swarmExpandSetupLog)) { _ in
            expand()
        }
        #else
        return self
    ...
    func acceptsCaptureNotice(_ app: AppModel) -> some View {
        #if DEBUG
        return onReceive(NotificationCenter.default.publisher(for: .swarmShowNotice)) { note in
            guard let message = note.object as? String else { return }
            app.notice = SwarmNotice(message: message)
        }
        #else
        return self
        #endif
    ...
    func acceptsCaptureNoticeHold(_ hold: @escaping @MainActor (Bool) -> Void) -> some View {
        #if DEBUG
        return onReceive(NotificationCenter.default.publisher(for: .swarmHoldNotice)) { note in
