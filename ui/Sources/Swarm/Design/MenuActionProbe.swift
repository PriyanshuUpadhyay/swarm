...
/// after.
///
///     Swarm --menu-action "Project settings…" [--menu-action-row 1]
///     Swarm --menu-action "Project Settings…" --menu-action-scope main
///     Swarm --menu-action "Project settings…" --menu-action-close commandW|escape
///     Swarm --menu-action "Project settings…" --menu-action-reopen
///     Swarm --menu-action "Project settings…" --menu-action-hide miniaturize|orderOut
///
/// **It exists because "the window did not open" is the one report the test suite cannot answer.**
/// `Tests/SwarmCoreTests` links the core alone, so nothing there knows a scene exists;
...
///
/// Debug builds only, and it wants `SWARM_UI_DB_PATH` pointed at a scratch database with a project in
