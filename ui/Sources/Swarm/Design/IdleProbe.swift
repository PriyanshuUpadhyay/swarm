...
/// Measures what Swarm costs while nothing is happening.
///
...
/// that waited for it would have to bring Swarm to the front, and taking the front from somebody
...
/// story here: Swarm's own thread barely moves while it burns a second of `git` every six.
...
///
///     Swarm --idle-probe /tmp/idle.json --idle-worktrees /tmp/trees.txt
///           [--idle-base main] [--idle-passes 5]
///
/// `--idle-worktrees` is a file with one worktree path per line, which is how a run needs no
/// database and cannot touch the owner's. `ls -d ~/swarm/workspaces/*/*/ > /tmp/trees.txt` is the
