...
/// `Swarm --snapshot-gallery <dir> --gallery inspector-tabs`. Deliberately not a `--snapshot`
    ...
    private func model(pullRequest: PullRequest?) -> WorkspaceModel {
        let model = WorkspaceModel(
            workspace: Workspace(
                ...
                name: "checks-tab",
                branch: "fix/checks-tab-gate",
                path: "/tmp/swarm-snapshot/checks-tab",
                baseBranch: "main"
            ),
            app: app
        )
        ...
        model.changedFiles = [
            ChangedFile(path: "Sources/SwarmCore/InspectorTab.swift", change: .added, additions: 63),
            ChangedFile(path: "Sources/Swarm/State/WorkspaceModel.swift", change: .modified, additions: 12, deletions: 6),
            ChangedFile(path: "Sources/Swarm/Views/Inspector/InspectorToolbar.swift", change: .modified, additions: 6, deletions: 2),
            ChangedFile(path: "Sources/Swarm/Design/InspectorTabStripGallery.swift", change: .added, additions: 104),
            ChangedFile(path: "Tests/SwarmCoreTests/InspectorTabTests.swift", change: .added, additions: 98),
        ]
        return model
    }
}
