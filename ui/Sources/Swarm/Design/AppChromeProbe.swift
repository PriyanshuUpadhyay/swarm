...
enum AppChromeProbe {
    ...
    static func runAndExit() -> Never {
        guard Bundle.main.bundleIdentifier != "io.github.priyanshuupadhyay.swarm",
              ProcessInfo.processInfo.environment["SWARM_UI_DB_PATH"] != nil else { exit(1) }
    ...
    private static func checkThemeSettings() async -> [String] {
        ...
        let domain = "swarm-theme-probe-\(UUID())"
        ...
        state.choice = .swarm
        check(state.glass == ColourTheme.swarm.glass && state.codeScheme == .swarm, "Theme overrides leaked")
        ...
        check(restored.codeScheme == .swarm && restored.terminalScheme == .charcoal, "Theme settings did not persist")
        ...
        preference.overrides.codeScheme = "swarm"
        ...
        let terminal = SwarmTerminalView(frame: CGRect(x: 0, y: 0, width: 600, height: 240))
        ...
        preference.overrides.terminalSource = .builtin("swarm")
    ...
    private static func checkColourThemes() async -> [String] {
        ...
        for theme in ColourTheme.allCases {
            ...
            for scheme in [ColorScheme.light, .dark] {
                ...
                await render(UserTurnRowView(text: "Please review Sources/Swarm/Design/Theme.swift", home: TranscriptHome())
    ...
    private static func render(
        ...
        try? bitmap.representation(using: .png, properties: [:])?.write(to: URL(filePath: "/tmp/swarm-chrome-\(name).png"))
        return aligned
    }
}
