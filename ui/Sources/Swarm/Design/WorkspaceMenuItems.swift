...
///
/// ## Four groups, and the rule is what each one acts on
///
/// The first group is about the ROW and changes nothing outside Swarm: what it is called, whether
/// it floats, whether it is shouting at you, what colour it is. The unread mark leads, because the
/// owner reaches for it most, straight after a turn finishes; Rename follows, which it used to
/// lead, having once sat fourth under a submenu where it was hunted for.
/// The second is about the CHECKOUT ON DISK: it opens the worktree, in the editor and in anything
/// else that can take it, shows it, puts its branch on the clipboard, and runs the repository's
/// setup script in it. The third is the PROJECT the workspace belongs to, and it is a group of one
/// because nothing else in this menu acts on anything that wide. The last destroys the workspace,
/// behind a rule of its own.
///
/// Project Settings is here because the row lost its hover menu. The row's archive button is the
/// only control left under the pointer, so the right click is where everything else lives, and
/// the owner went looking for the project's settings there. It opens the project this row is in,
/// not the selected workspace's, which is why it is a plain button rather than `MenuCommand`: that
/// would show File's Shift Command comma beside it, a shortcut that acts on a different project
/// whenever the row right clicked is not the selected one.
///
...
/// things that change nothing outside Swarm, and it is not down beside Archive, which is a group
/// of one on purpose.
///
...
struct WorkspaceMenuItems: View {
    ...
    /// Everything else that can open this worktree, under the item that opens it in the editor.
    ///
    /// **It was reachable from one place in the app, and that place was an overflow button in the
    /// inspector's toolbar.** Settings promises that an application you add "joins every Open in
    /// menu, including Open Worktree in", and a user who added iTerm and PHPStorm wrote in to say
    /// he could not find that menu anywhere, having looked where anybody would look: the right
    /// click menu on the workspace. He was right. `Open in Editor` above goes through
    /// `Reveal.inEditor`, which asks for the preferred editor and never offers a terminal, a git
    /// client or anything else the user added, so on this menu there was no route to them at all.
    ///
    /// Both, rather than the submenu replacing the item. `OpenInMenu` explains why the parent row
    /// of a submenu cannot carry an action of its own: AppKit never delivers it. Opening the
    /// worktree in the editor you always use is the common case and it stays one click.
    ///
    /// The repository is put in the environment here because the sidebar has none to inherit. It
    /// is what makes the submenu remember a choice per project rather than globally, which is the
    /// whole point of `EnvironmentValues.openInRepoID`.
    ...
    /// The project's settings window, for the project this workspace is in. See the type's note
    /// on why it is not `MenuCommand`.
    private var projectSettingsItem: some View {
        Button(MenuBarCatalogue[.projectSettings].title) {
            openWindow(id: RepoSettingsWindow.id, value: workspace.repoID)
        }
    }
