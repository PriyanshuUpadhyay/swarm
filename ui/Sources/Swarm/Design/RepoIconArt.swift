...
enum RepoIconArt {
    ...
    /// a path Swarm is already showing can become a different picture. A file edited on disk
    /// outside Swarm is picked up at the next launch: watching every project's icon for changes
    ...
    /// shape underneath is `#000`. Swarm's own document is exactly that shape; the bug was hidden
