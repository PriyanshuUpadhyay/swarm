...
enum TranscriptSentMotionTrace {
    static func record(content: NSView, cell: NSView, arrival: MessageArrival, distance: CGFloat) {
        guard FileManager.default.fileExists(atPath: "/tmp/swarm-record-sent-motion") else { return }
        var geometry: [[Double]] = []
        ...
        Task { @MainActor in
            ...
            let url = URL(fileURLWithPath: "/tmp/swarm-sent-motion-\(UUID().uuidString).json")
            do {
