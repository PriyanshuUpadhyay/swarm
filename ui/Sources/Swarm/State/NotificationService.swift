...
/// in `NotificationPolicy` and `NotificationDigest`, in SwarmCore, where it can be tested without
...
final class NotificationService {
    ...
    /// a morning of crash reports named Swarm: `.build/debug/Swarm` run as a bare executable died
    ...
    nonisolated private static let workspaceCategory = "swarm.workspace"
    /// Open only, for a digest and for the test banner.
    nonisolated private static let summaryCategory = "swarm.summary"
    nonisolated private static let replyAction = "swarm.reply"
    nonisolated private static let openAction = "swarm.open"
    ...
    /// permission in System Settings while Swarm is running, and a toggle that keeps claiming to
    ...
    /// Handed the app state once it exists, the same way `SwarmAppDelegate` is. The suppression
    ...
    /// Whether macOS is currently throwing away everything Swarm sends. Drives the warning in the
    /// settings pane.
    ...
    func openSystemSettings() {
        // The Ventura-and-later pane id. The `id` query selects Swarm's own row, so the switch the
    ...
    func sendTestNotification() {
        deliver(PreparedNotification(
            identifier: "swarm.test",
            threadIdentifier: "swarm.test",
            title: "Swarm",
    ...
    /// Watches CI on the pull requests Swarm already knows about.
    ///
