...
final class WorkspaceModel {
    ...
    /// Things Swarm did to this workspace that are worth a line in the transcript: a merge, and
    /// whatever follows it.
    ///
    ...
    /// stored state, then everything Swarm has recorded since, in the order it happened.
    ///
    ...
    func clearConversation(_ previous: Session, controls: ComposerControls) async -> Session? {
        ...
        do {
            ...
            app.notice = SwarmNotice(message: "Could not clear the conversation: \(error.readableMessage)")
            return nil
        }
    }
    ...
    func replaceSession(_ session: Session, controls: ComposerControls) async -> Session? {
        ...
        do {
            ...
            app.alert = SwarmAlert(title: "Could not start a fresh chat", message: error.readableMessage)
            return nil
        }
    }
    ...
    func closeSession(_ session: Session) async {
        ...
        do {
            ...
            app.notice = SwarmNotice(message: "Could not close the conversation: \(error.readableMessage)")
            return
        }
    ...
    /// Every line this subagent has produced, as Swarm stored it off the parent's own stream.
    ///
    ...
    /// Gives up the start hold once setup and the first delivery are behind it.
    ///
    /// A chat turn has already set the running flag by the time `drain` returns, so the hold has
    /// either been settled by it or there is no turn coming. A CLI agent in a terminal is only
    /// seen running once its own status file says so, which is a poll away, so that one keeps
    /// the hold a few seconds longer rather than dropping the row to Idle in between.
    private func releaseStartHold(afterCLILaunch: Bool) {
        let id = workspace.id
        guard afterCLILaunch else {
            app.endStart(id)
            return
        }
    ...
    /// browser opened on one block while the shell exported the other's `SWARM_UI_PORT`. The task
    ...
    private func stream(
        ...
        guard let lease = operationLease ?? WorkspaceOperationLease.acquire(in: workspace.path),
            ...
            app.notice = SwarmNotice(message: "Setup is already running in this workspace.")
            return false
        }
        isRunningSetup = true
    ...
    /// trade by a long way.
    ///
    /// The whole row, not the one column. `setupState` is `internal(set)` in SwarmCore now, so
    ...
    func refreshChanges(_ reason: ChangesRefresh = .requested) async {
        ...
        // its worktree path is a directory inside Swarm's workspaces root that the reader neither
        ...
        let task = Task.detached(priority: .userInitiated) { () -> Result<ChangesAnswer, GitFailure> in
            ...
            do {
                ...
                // worktree deleted underneath Swarm used to surface here as "`git rev-parse
    ...
    ///
    /// The agent already holds the things a pull request needs and Swarm does not: this project's
    ...
    ///
    /// The agent rather than Swarm, and the reasoning is the same one that put pull request
    /// creation here: a commit needs a message, and Swarm knows only that a file changed. The
    ...
    /// Swarm used to run `gh pr merge` and then `git push --delete` behind it, catch a `ShellError`
    ...
    /// One of Swarm's own turns about landing a branch, with Swarm's rules under it and the
    ...
    /// them in. `ConflictInstructions` puts Swarm's own steps in a file and names it, which is what
    ...
    /// nothing to say still gets Swarm's file, which is the difference from Merge, where Swarm's
    ...
    func requestFixConflicts(
        ...
        let context = FixConflictsPromptContext(
            ...
            // branch the agent is standing on, and this is the one fact here Swarm holds itself.
            branch: workspace.branch,
            baseBranch: workspace.baseBranch
        )
    ...
    ///
    /// The path goes in the sentence that asks for it, which is where every other file Swarm sends
    ...
    /// pull request, and a scratch path under `.swarm/attachments` is invisible to git, means
