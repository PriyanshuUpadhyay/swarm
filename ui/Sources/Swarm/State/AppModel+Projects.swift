...
///
/// A project is a git repository the owner pointed Swarm at. It owns workspaces and nothing else,
...
extension AppModel {
    ...
    func addRepository(at path: String, presentedIn surface: ProjectSetupSurface = .main) async {
        ...
        case .refuse(let refusal):
            alert = SwarmAlert(title: "Swarm will not add this folder", message: refusal.sentence)
    ...
    private func addKnownRepository(at path: String) async -> Repo? {
        ...
        do {
            ...
            alert = SwarmAlert(title: "Could not add that folder", message: error.readableMessage)
            return nil
        }
    }
    ...
    /// folder is a git repository with a commit in it, whether Swarm made it a moment ago or found
