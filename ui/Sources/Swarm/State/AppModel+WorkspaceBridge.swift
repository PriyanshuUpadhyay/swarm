...
extension AppModel {
    ...
    /// Swarm's own merge rules, whatever the project adds to them and the guard that refuses mid
    ...
    /// client always names one, both from the list Swarm already has. Two callers, one answer,
    ...
    private func startWorkspaceForBridge(
        ...
        // the owner's own client, which has nothing to inherit and gets Swarm's defaults.
