...
/// across. The decisions are `ContinuationGate` and `CarryOnGate` in SwarmCore, and they are the
...
extension AppModel {
    ...
    func createWorkspace(
        ...
        do {
            ...
            alert = SwarmAlert(title: "Could not create the workspace", message: trouble.sentence)
            return nil
        }
    }
    ...
    func startWorkspace(
        ...
        /// Overrides the name Swarm would derive. Only the bridge passes one; the sheet lets the
        /// namer do its job.
        name: String? = nil,
        ...
        if let notice = pick?.notice {
            self.notice = SwarmNotice(message: notice)
        }

        // Said out loud, because this is a preference the owner set that Swarm has just undone,
        // and the workspace that undid it can have been asked for by an agent on the bridge with
        // nobody watching. A banner is exactly what `SwarmNotice` is for: something the app did
    ...
    /// standing in front of the strip. A refusal is Swarm declining on purpose and naming the
    ...
    /// `ContinuationGate.decide` in SwarmCore and is the only thing here allowed to say yes; the
    ...
    /// come across is Swarm's own transcript, whose rows belong to the chat they were written
