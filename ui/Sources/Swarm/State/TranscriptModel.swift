...
final class TranscriptModel {
    ...
    ///
    /// **Nil is Ask Swarm and nothing else today.** Its id and path are stable and are what most
    ...
    /// the queue, because a message somebody typed has to survive quitting Swarm. See `Delivery`.
    ...
    /// The conversation that belongs to Swarm rather than to a workspace.
    ///
    ...
    init(
        swarmSession session: Session, chairLog: TranscriptLogReader?, directory: String,
        workspace: Workspace? = nil, app: AppModel
    ) {
        self.session = session
        self.workspace = workspace
        self.cwd = directory
    ...
    private func absorb(_ message: Message, decisions: [String: String] = [:]) {
        ...
        // because Swarm writes some crew rows directly, and one of those landing mid send must not
    ...
    /// into `messages` for the reason `WorkspaceEvent`'s header sets out: a row Swarm writes about
    ...
    func refreshSession() async {
        // `app.store` rather than `store`: a chat read from a CLI's log still has a row, and the
        // provider session id it resumes from is written on that row by the activity poll.
        guard let store = app.store, let fresh = try? await store.session(id: session.id) else { return }
        session = fresh
    ...
    func updatePreferences(
        ...
        // `app.store` rather than `store`, for the reason `refreshSession` says: the pickers
        // belong to the row, and a chat read from a CLI's log has one.
    ...
    func submit(_ text: String, clearingDraft sourceDraft: String? = nil,
        ...
        // Ask Swarm is the only transcript with no workspace, and the only one named this way.
        if workspace == nil {
            let id = session.id
            Task { await app.ask.nameFromFirstMessage(id, prompt: body) }
        }
    ...
    func confirmDiscard(_ delivery: Delivery) async {
        ...
        guard removed else {
            app.notice = SwarmNotice(message: PendingMessageDiscard.alreadySentSentence)
            return
        }
    ...
    func editPending(_ delivery: Delivery) async {
        ...
        guard removed else {
            app.notice = SwarmNotice(message: PendingMessageEdit.alreadySentSentence)
            return
        }
    ...
    private func abandon(_ delivery: Delivery, saying complaint: String) async {
        ...
        guard let store else {
            app.alert = SwarmAlert(title: "Could not start the agent", message: complaint)
            return
        }
    ...
    /// payload are not. Guarded by `isRunning` at the call site so a turn Swarm sent, whose `init`
    ...
    private func ensureRunner() -> (any SessionRunner)? {
        ...
        // minted for that workspace and the role its origin says; Ask Swarm gets the owner's own,
    ...
    /// the probe has to measure is what Swarm does with a delta, and the only faithful way to
    ...
    private func handle(_ event: AgentEvent) async {
        switch event {
        case .initialized:
            // **A turn Swarm did not send starts here and nowhere else.** The CLI runs a
            // background task's completion notification as a turn of its own, and it announces
            // each one with an `init` exactly as it announces the ones Swarm asks for. Until this
    ...
    /// "Always allow ... in Swarm" is a promise about a place, and the place has to be named.
    ...
    /// to ask either polls or forgets, so the end of a crew member's turn is an event Swarm
