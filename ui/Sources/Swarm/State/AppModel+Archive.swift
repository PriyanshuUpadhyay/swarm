...
/// `ArchiveRequest` in SwarmCore turn the answer into a question worth asking, `offerUndo` puts
...
extension AppModel {
    ...
    private func performArchive(
        ...
        do {
            ...
            let archiveNotice: SwarmNotice?
            if let path = report?.preservedFolderPath {
                archiveNotice = SwarmNotice(
                ...
                archiveNotice = SwarmNotice(message: stopped)
            } else {
                archiveNotice = nil
            }
    ...
    private func reportArchiveFailure(_ error: any Error, workspace: Workspace) async -> String {
        ...
        alert = SwarmAlert(title: "Could not archive the workspace", message: trouble.sentence)
    ...
    /// - An archive script has already wound the workspace down, and Swarm has no idea what it
