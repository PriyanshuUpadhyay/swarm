...
struct SwarmAlert: Identifiable {
    let id = UUID()
    var title: String
    var message: String
}
...
final class AppModel {
    ...
    var alert: SwarmAlert?
    /// The corner notice. See `SwarmNotice`. Set from anywhere, drawn once by `RootView`.
    ///
    ...
    /// The conversation that belongs to Swarm rather than to a workspace. See `AskModel`.
    ///
    ...
    /// The mark the Ask Swarm row wears, or nil when it has nothing to say.
    ///
    ...
    /// started. The report was the rule on Ask Swarm's own pane sitting still through a whole
    ...
    /// Workspaces whose worktree exists and in which nothing has started yet, so that the row the
    /// pending one hands over to does not drop to Idle for that moment. See `WorkspaceStartHold`.
    ///
    /// Observed because the sidebar and Home resolve their marks from it.
    ...
    /// Taken before the `reload` that retires the pending row, so the two land in one update.
    ...
    /// set here rather than in an initialiser, because `SwarmApp` builds this as `@State` and
    ...
    func bootstrap() async {
        ...
        do {
            ...
            // closed here and the rows that asked them say what happened instead. Swarm denies
    ...
    ///
    /// The entry names an absolute path and is written once, so dragging Swarm from
    ...
    /// Quitting Swarm has to take everything it started with it. macOS does not kill a process's
    ...
    func shutdownEverything() async {
        ...
        // one after another. Ask Swarm is signalled with them, and only if it was ever opened:
    ...
    func reload() async {
        ...
        do {
            ...
            // A project added, removed, hidden or unhidden takes rows out of the sidebar or puts
            // them in, and the curve has to be on this write for the list to play it: keyed to a
            // value in the sidebar, the rows went in a single frame. Both lists go in the one
            // transaction, because removing a project removes its workspaces in the same reload.
            // Not on launch, where the first fill would otherwise slide in.
            if isLoaded, !reduceMotion,
               ProjectVisibilityMotion.movesProjectRows(from: repos, to: loadedRepos) {
                withAnimation(.easeOut(duration: ProjectVisibilityMotion.seconds)) {
                    repos = loadedRepos
                ...
                if workspaces != reconciled {
                    // A diff stat moving and nothing else is the one reload the figures roll on,
                    // and the curve is on this write because a list row ignores any it starts
                    // itself. See `WorkspaceListReconciliation.changesOnlyDiffStats`.
    ...
    /// the runner's event pump. Swarm has already put out two performance fires, a busy indicator
    ...
    /// when the user edits files outside Swarm.
    ...
    private func refreshSelectedChangedFiles() async {
        ...
        // A worktree removed outside Swarm would make git walk up to the parent repository and
    ...
    func refreshDiffStats() async -> Set<WorkspaceID> {
        ...
        // seconds on an idle machine is what put Swarm under "Using Significant Energy".
        //
        ...
        let pending = workspaces.filter {
            // A worktree that has been removed outside Swarm would make git walk up to the parent
    ...
    /// The projects most recently started in, newest first, archived workspaces included. See
    /// `RecentProjects`.
    func recentProjects() async -> [Repo] {
        RecentProjects.ordered(repos, workspaces: workspaces + (await archivedWorkspaces()))
    }
    ...
    /// width now depends on it and `SwarmApp` cannot see a private computed property on a view.
    ...
    /// back to "Swarm". The Workspace menu no longer asks: what a menu item acts on is
    ...
    private func recomputeAgentTurns() {
        ...
        // Ask Swarm is in neither set, because a `SessionActivity` carries a workspace id and
    ...
    var runningAgentCount: Int {
        // Ask Swarm counts, and only if it has been opened this launch. It is an agent mid turn
    ...
    /// `SidebarReorder` is what turns them into the store's own order, and it lives in `SwarmCore`
