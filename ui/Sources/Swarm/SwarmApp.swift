...
struct SwarmApp: App {
    ...
    init() {
        ...
        // A development affordance: `Swarm --snapshot <dir>` draws the interface straight to PNG
        ...
        // A development affordance too: `Swarm --frame-probe <out.json>` drags the sidebar divider
        // and records how long each frame actually took. See `FrameProbe`.
        if FrameProbe.isRequested { FrameProbe.schedule() }

        // And another: `Swarm --switch-probe <out.json>` times the path from clicking a workspace
        // to seeing it. See `SwitchProbe`.
        if SwitchProbe.isRequested { SwitchProbe.schedule() }

        // And its sibling one level in: `Swarm --tab-probe <out.json>` times the path from picking
        // a tab in the centre column to seeing it. See `TabProbe`.
        if TabProbe.isRequested { TabProbe.schedule() }

        // And the one that answers "the chat does not scroll smoothly": `Swarm --scroll-probe
        ...
        // change list on screen at once": `Swarm --resize-probe <out.json>` puts all three up and
        ...
        // `Swarm --stream-probe <out.json>` types into the composer and then streams a turn into
        ...
        // `Swarm --jump-probe <out.json>` asks for the live end from several starting positions
        // and reports how far short of it the view came to rest. See `JumpProbe`.
        if JumpProbe.isRequested { JumpProbe.schedule() }

        // And the one that catches a bug rather than timing a gesture: `Swarm --composer-probe
        ...
        // And the one that answers "the battery menu says Swarm is using significant energy":
        // `Swarm --idle-probe <out.json> --idle-worktrees <list>` runs the diff stat pass the six
        ...
        // And two last ones. `Swarm --menu-probe <out.png>` opens one of the centre pane's split
        // submenus and photographs it, which nothing else can; `Swarm --menu-action <title>`
    ...
    var body: some Scene {
        // A single `Window` rather than a `WindowGroup`. Swarm's whole model is one window
        ...
        Window("Swarm", id: Self.mainWindowID) {
            RootView()
                ...
                // The title bar is painted in Swarm's chrome rather than in the system's window
        ...
        // Command-click the title for the path above it. Swarm's workspaces are folders, so that
        ...
        .commands {
            ...
            // only once the window was already open. It is a row of `SwarmCommands`' File group
            // now, which is where `MenuBarCatalogue` says it is.
            SwarmCommands(model: model)
        }
