...
@Suite struct LoginShellPathTests {
    ...
    /// A relative entry resolved against Bloom's working directory is a different directory from
    ...
    @Test func mergeWithoutAnAnswerIsWhatBloomAlreadyHad() {
        let merged = LoginShellPath.merge(
    ...
    @Test func theProbeCanBeTurnedOff() async {
        let found = await LoginShellPath.discover(
            shell: "/bin/zsh", environment: ["BLOOM_LOGIN_SHELL_PATH": "0"]
        )
