...
struct BridgeOwnerTokenTests {
    ...
    /// A Swarm that cannot be coupled to anything until somebody deletes a file they were never
    ...
    @Test("it lives beside the database, so two copies of Swarm cannot share one")
    func besideTheDatabase() {
        let one = BridgeOwnerToken.beside(databasePath: "/x/Swarm/swarm.sqlite")
        let other = BridgeOwnerToken.beside(databasePath: "/x/Swarm Dev/swarm.sqlite")
...
struct BridgeOwnerCommandTests {
    private func attachment(
        shim: String = "/Applications/Swarm.app/Contents/MacOS/swarm-bridge",
        socket: String = "/tmp/swarm-bridge-abc.sock",
    ...
    func shape() {
        ...
        #expect(command.contains("-e 'SWARM_UI_BRIDGE_SOCKET=/tmp/swarm-bridge-abc.sock'"))
        #expect(command.contains("-e 'SWARM_UI_BRIDGE_TOKEN=deadbeef'"))
        #expect(command.contains("-e 'SWARM_UI_BRIDGE_ROLE=owner'"))
        #expect(command.hasSuffix("-- '/Applications/Swarm.app/Contents/MacOS/swarm-bridge'"))
    }
    ...
    func codexShape() {
        ...
        #expect(command.contains("--env 'SWARM_UI_BRIDGE_SOCKET=/tmp/swarm-bridge-abc.sock'"))
        #expect(command.contains("--env 'SWARM_UI_BRIDGE_TOKEN=deadbeef'"))
        #expect(command.contains("--env 'SWARM_UI_BRIDGE_ROLE=owner'"))
        #expect(command.hasSuffix("-- '/Applications/Swarm.app/Contents/MacOS/swarm-bridge'"))
    }
    ...
    /// The name the owner registers under cannot be the one Swarm's own `--mcp-config` uses: that
    ...
    /// meant Swarm Dev's command silently evicted the owner's real registration. The table is
    /// `Store.databaseDirectoryName`, so a shared name now implies a shared database.
    @Test("each copy of Swarm registers under a name of its own")
    ...
    func quotesPaths() {
        let command = BridgeRegistration.ownerAddCommand(
            attachment(shim: "/Users/me/Applications/Swarm Dev.app/Contents/MacOS/swarm-bridge")
        )

        #expect(command.hasSuffix(
            "-- '/Users/me/Applications/Swarm Dev.app/Contents/MacOS/swarm-bridge'"
        ))
    }
