...
/// How a turn Swarm composed is cut up for the bubble that draws it.
///
...
struct SentTurnTests {
    ...
    @Test("the segments are the turn again", arguments: [
        ...
        "Open a pull request.\n\nFollow the instructions in `.swarm/scratch/pr-instructions.md`.",
    ...
    @Test("Swarm's merge rules are one chip")
    ...
    /// A turn an older Swarm sent is still in the database and still has to draw.
    ...
    @Test("Swarm's own instruction files carry the same titles as the blocks", arguments: [
    ...
    func filesAreStillFiles() {
        let turn = "Open a pull request.\n\nFollow the instructions in "
            + "`.swarm/scratch/pr-instructions.md`."

        #expect(SentTurn.segments(in: turn).contains(.file(".swarm/scratch/pr-instructions.md")))
    }

    /// A block is Swarm's only where Swarm put it, which is at the head of a paragraph. Quoted in
