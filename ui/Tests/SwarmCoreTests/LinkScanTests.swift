...
struct LinkScanTests {
    ...
    @Test("a local development host is a link", arguments: [
        "http://swarm.test",
        "https://swarm.test/one/two?three=four#five",
    ...
    @Test("a file path is not a link", arguments: [
        "/Users/freek/dev/code/Swarm/Sources/SwarmCore/LinkScan.swift",
        "Sources/Swarm/Views/Transcript/UserTurnRowView.swift",
        "./build.sh",
        "~/Library/Application Support/Swarm/swarm.sqlite",
    ...
    func gitRemote() {
        #expect(written("git@github.com:spatie/swarm.git").isEmpty)
        #expect(written("origin  git@github.com:spatie/swarm.git (fetch)").isEmpty)
    }
    ...
    @Test("a scheme Swarm does not open is not a link")
    func otherSchemes() {
        #expect(written("file:///Applications/Swarm.app").isEmpty)
