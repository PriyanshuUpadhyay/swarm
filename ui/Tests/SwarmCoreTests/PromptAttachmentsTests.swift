...
struct PromptAttachmentsTests {
    ...
    /// Inside the worktree, under Swarm's own corner of it, because the agent has to be able to
    /// read it without being asked for permission Swarm cannot answer.
    @Test("a copy lands under Swarm's scratch folder, keyed by its id")
