...
/// ruler it was measured with was the local `main`, which Swarm never moves and which was still
...
struct BaselineTests {
    ...
    /// every repository Swarm is ever pointed at.
    private func clone(of server: TempRepo, named name: String) async throws -> TempRepo {
        ...
        try await Shell.check("git", ["config", "user.email", "test@swarm.local"], cwd: path)
        try await Shell.check("git", ["config", "user.name", "Swarm Test"], cwd: path)
    ...
    /// A repository with no remote at all, which Swarm supports everywhere else.
