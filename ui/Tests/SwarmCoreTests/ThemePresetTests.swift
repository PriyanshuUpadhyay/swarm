...
struct ThemePresetTests {
    @Test func migrationPreservesLegacySettings() throws {
        let name = "swarm-theme-test-\(UUID())"
    ...
    @Test func independentSchemesAndFallbacks() {
        ...
        changes.codeScheme = "swarm"
        changes.terminalSource = .builtin("charcoal")
        #expect(changes.codeScheme(for: .charcoalGlass) == .swarm)
        #expect(changes.terminalScheme(for: .swarm) == .charcoal)
    ...
    @Test func syntaxCacheIncludesPaletteAndExactBytes() {
        let first = SyntaxCacheKey(line: "café", language: .swift, carry: LexState(), scheme: .swarm)
        let otherScheme = SyntaxCacheKey(line: "café", language: .swift, carry: LexState(), scheme: .charcoal)
        let otherBytes = SyntaxCacheKey(line: "cafe\u{301}", language: .swift, carry: LexState(), scheme: .swarm)
        var edited = CodeScheme.swarm
    ...
    @Test func archiveKeepsThemesSeparate() throws {
        ...
        #expect(restored.themes["swarm"] == nil)
        var future = original
...
struct ThemePreferenceStateTests {
    @Test func switchingResetAndPersistence() throws {
        ...
        state.choice = .swarm
        #expect(state.glass == ColourTheme.swarm.glass && state.codeScheme == .swarm)
        ...
        #expect(state.codeScheme == .swarm && state.terminalScheme == .charcoal)
        ...
        #expect(reloaded.codeScheme == .swarm && reloaded.glass == .regular)
    ...
    @Test func legacyDefaultsRemainInherited() throws {
        let domain = "swarm-theme-defaults-\(UUID())"
        let defaults = try #require(UserDefaults(suiteName: domain))
        defer { defaults.removePersistentDomain(forName: domain) }
    ...
    @Test func unreadableArchivePreservesLegacyChoices() throws {
        let domain = "swarm-theme-recovery-\(UUID())"
        let defaults = try #require(UserDefaults(suiteName: domain))
        defer { defaults.removePersistentDomain(forName: domain) }
    ...
    @Test func archiveWrittenPerPresetMigratesTypographyFromLegacySettings() throws {
        let domain = "swarm-theme-typography-\(UUID())"
        let defaults = try #require(UserDefaults(suiteName: domain))
        defer { defaults.removePersistentDomain(forName: domain) }
    ...
    @Test func followingGhosttyInOnePresetFollowsItInAll() throws {
        let domain = "swarm-theme-ghostty-\(UUID())"
        let defaults = try #require(UserDefaults(suiteName: domain))
        defer { defaults.removePersistentDomain(forName: domain) }
        ...
        let older = Data(#"{"schemaVersion":1,"themes":{"swarm":{"terminalSource":{"ghostty":{}}},"neutral":{"glass":"thin"}}}"#.utf8)
        defaults.set(older, forKey: "themeOverrides")
        defaults.set("neutral", forKey: ColourTheme.defaultsKey)
        let state = ColourThemePreference(defaults: defaults)
        ...
        let without = try JSONEncoder().encode(ThemeOverrides.Archive(themes: ["swarm": ThemeOverrides()]))
