...
/// **Swarm had been putting two different things in one list.**
///
...
@Suite(.scratchDirectory) struct SubagentOutputReadingTests {
    ...
    /// on the parent's own stream and Swarm stored every one of them.
    @Test func aRunningSubagentIsReadFromTheStreamSwarmAlreadyStored() {
        let lines = [
