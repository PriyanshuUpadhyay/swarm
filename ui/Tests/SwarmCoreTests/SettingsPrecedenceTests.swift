...
struct SettingsPrecedenceTests {
    ...
    private func makeRepo(_ files: [String: String]) throws -> String {
        let root = TestScratch.unique("swarm-prec")
    ...
    @Test(".swarm outranks .conductor at the same tier, and .local outranks the shared file")
    func swarmOutranksConductor() throws {
        let repo = try makeRepo([
            ".conductor/settings.toml": "[scripts]\nsetup = \"conductor shared\"\n",
            ".swarm/settings.toml": "[scripts]\nsetup = \"swarm shared\"\n",
        ])
        #expect(SettingsLoader.load(repo: repo).setupScript == "swarm shared")
        ...
        #expect(SettingsLoader.load(repo: all).setupScript == "swarm local")
    }
