...
struct RepositoryStarterTests {
    ...
    private func folder(_ files: [String: String] = [:]) throws -> String {
        let root = TestScratch.unique("swarm-start")
    ...
    func emptyFolder() async throws {
        ...
        try await Git.addWorktree(repo: root, path: worktree, branch: "swarm/test", base: outcome.branch)
    ...
    func keepsSecretsOut() async throws {
        ...
        // And the files are still on disk. Swarm excludes them, it does not remove them, and
...
struct RepositoryStartFailureTests {
    ...
    /// A GitHub failure after a good commit still leaves a project Swarm can run, which is what
...
/// somebody can be told about, and it must not leave a repository behind that Swarm made and
/// nobody asked for.
@Suite("Stopping a repository setup", .tags(.git), .scratchDirectory)
struct RepositoryStartAbandonmentTests {
    private func folder(_ files: [String: String] = [:]) throws -> String {
        let root = TestScratch.unique("swarm-stop")
        try FileManager.default.createDirectory(atPath: root, withIntermediateDirectories: true)
        for (relative, contents) in files {
            let full = (root as NSString).appendingPathComponent(relative)
            try contents.write(toFile: full, atomically: true, encoding: .utf8)
    ...
    @Test("an initialised repository with no commit is Swarm's to remove")
    ...
    @Test("only the kept project counts as one Swarm can run", arguments: [
    ...
    func abandonRemovesTheRepository() async throws {
        ...
        // Everything that was the user's is untouched. Only what Swarm made is gone.
