...
struct NoticeLifetimeTests {
    ...
    func units() {
        ...
        #expect(NoticeLifetime.units(in: "Swarm named this workspace") == 4)
    ...
    func scales() {
        ...
        let long = "Swarm named this workspace Describe fade-in animation feel. Its branch is still "
    ...
    func dismissal() {
        #expect(SwarmNotice(message: "Swarm named this workspace Foxglove.").lifetime != nil)
        #expect(SwarmNotice(message: "It moved.", dismissal: .untilDismissed).lifetime == nil)
    }
}
...
struct NoticeTextTests {
    ...
    func split() {
        let text = NoticeText("Swarm named this workspace Foxglove. Its branch is still `add-a-toggle`.")
        #expect(text.fact.map(\.text).joined() == "Swarm named this workspace Foxglove.")
    ...
    func plain() {
        let message = "Swarm named this workspace Foxglove. Its branch is still `add-a-toggle`, because it has 2 commits on it."
