...
struct CommandDisplayTests {
    private static let worktree = "/Users/freek/swarm/workspaces/there-there/freekmurze-hibiki-sea"
    ...
    func anotherWorkspace() {
        let command = "cd /Users/freek/swarm/workspaces/swarm/main && git log --oneline -5"
        let display = Self.of(command)
        #expect(display.leftTheWorkspace)
        #expect(display.lead == .prefix("cd /Users/freek/swarm/workspaces/swarm/main &&"))
    ...
    func spaces() {
        let spaced = "/Users/freek/swarm/workspaces/there there/hibiki sea"
        for command in [
            "cd '\(spaced)' && ls",
            "cd \"\(spaced)\" && ls",
            "cd /Users/freek/swarm/workspaces/there\\ there/hibiki\\ sea && ls",
        ] {
    ...
    func bashRowOpaque() {
        for command in ["cd ~/swarm && ls", "cd $TMPDIR && ls", "cd /tmp"] {
            let input = JSONValue.object(["command": .string(command)])
            let row = ToolPresenter.present(name: "Bash", input: input, worktree: Self.worktree)
