...
///
/// **The wording is tested because the wording is the feature.** What Swarm says about a 529 is
...
@Suite struct AgentRetryTests {
    ...
    /// there is nothing more useful to say about one Swarm has not met.
...
/// The `rate_limit_event` that rides along in the same capture, and the figure Swarm used to
/// invent from it.
...
@Suite struct TurnFailureTests {
    ...
    @Test func theFailureIsExplainedInSwarmsWordsAndThenInTheClis() throws {
    ...
    /// An ending Swarm has no reading of does not get a sentence pretending otherwise.
