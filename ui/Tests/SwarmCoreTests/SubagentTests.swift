...
@Suite struct SubagentCaptureTests {
    private func events() throws -> [AgentEvent] {
        try swarmFixtureLines("claude-api-retry.ndjson").compactMap(AgentEvent.decode(line:))
    }
...
@Suite struct SubagentRowTests {
    ...
    /// are always the nested rows Swarm stored, and because the file is named on the line that
    ...
    /// and never moved. Swarm's own clock is the floor under it.
...
@Suite struct SubagentTranscriptTests {
    ...
    @Test func aFailedSubagentStillLeftAReadableFile() throws {
        let text = try swarmFixtureLines("subagent-output-529.jsonl").joined(separator: "\n")
