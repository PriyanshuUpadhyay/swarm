...
struct BridgeServerTests {
    ...
    private func makeBridge(
        ...
        try server.start()
        let attachment = server.attach(session: session, workspace: workspace, shimPath: "/tmp/swarm-bridge")
    ...
    func endToEnd() async throws {
        ...
        // `project_list` is here because a workspace agent may name another project to
        // `workspace_start`, and this is how it learns the names.
        #expect(names == [
            "agent_list", "chat_list", "chat_read", "project_list", "quick_prompt_create",
            "quick_prompt_list", "whoami", "workspace_diff", "workspace_rename",
        ])
        ...
        #expect(answer["workspace"]?["branch"]?.stringValue == "swarm/fix-the-index")
    ...
    /// an older Swarm. The requirement is that it fails with a sentence rather than hanging, and
    ...
    /// The owner's own registration is applied to every `claude` on the machine, the ones Swarm
    ...
    func ownerTokenInsideAWorkspace() async throws {
        ...
        let repo = try await store.upsert(Repo(name: "billing", path: "/tmp/billing", defaultBranch: "main"))
        let workspace = try await store.upsert(Workspace(
            repoID: repo.id, name: "standing here", branch: "swarm/standing-here", path: directory, baseBranch: "main"
        ))
        let server = try BridgeServer(store: store)
        try server.start()
        defer { server.stop() }
    ...
    @Test("a session token this launch did not mint is told to quit and reopen Swarm")
    ...
    /// session token gets is the one remedy this caller cannot use: it would restart Swarm, present
    /// the identical token and be refused again. What has actually happened is a registration
    /// pointing at another Swarm, a token regenerated in Settings, or a configuration copied from
    ...
    func unknownOwnerToken() async throws {
        ...
        var caller = try Caller(socketPath: server.socketPath)
        let welcome = try await caller.hello(BridgeHello(
            token: "a token from another Swarm",
        ...
        #expect(!problem.lowercased().contains("restart swarm"))
    ...
    func droppedServerIsReleased() async throws {
        ...
        let socketPath = NSTemporaryDirectory() + "swarm-drop-\(UUID().uuidString.prefix(8)).sock"
    ...
    func retiringASessionRemovesItsConfig() async throws {
        ...
        let socketPath = NSTemporaryDirectory() + "swarm-retire-\(UUID().uuidString.prefix(8)).sock"
        // A socket file is one per test and nothing else removes it: a successor unlinks the path
        // it is about to bind, and these names are unique, so every run left one behind. The `.d`
        // beside it is the config directory the server derives from the same path, and it is left
        // the same way. See `BridgeServer.configDirectory`.
    ...
    func startingSweepsTheDirectory() async throws {
        ...
        let socketPath = NSTemporaryDirectory() + "swarm-sweep-\(UUID().uuidString.prefix(8)).sock"
        // A socket file is one per test and nothing else removes it: a successor unlinks the path
        // it is about to bind, and these names are unique, so every run left one behind. The `.d`
        // beside it is the config directory the server derives from the same path, and it is left
        // the same way. See `BridgeServer.configDirectory`.
