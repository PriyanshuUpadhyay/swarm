...
/// The schema change behind Ask Swarm: `sessions.workspace_id` is nullable, which took the first
...
struct SessionWithoutWorkspaceTests {
    ...
    func roundTrips() async throws {
        ...
        let session = try await store.upsert(Session(workspaceID: nil, title: "Ask Swarm"))

        let loaded = try #require(try await store.session(id: session.id))
        #expect(loaded.workspaceID == nil)
        #expect(loaded.title == "Ask Swarm")
    }
    ...
    func invisibleToWorkspaceLists() async throws {
        ...
        let ask = try await store.upsert(Session(workspaceID: nil, title: "Ask Swarm"))
    ...
    func staysOutOfActivity() async throws {
        ...
        var ask = Session(workspaceID: nil, title: "Ask Swarm")
    ...
    func rebuildKeepsMessages() async throws {
        ...
        let ask = try await reopened.upsert(Session(workspaceID: nil, title: "Ask Swarm"))
