...
struct LocalPageTests {
    ...
    /// `/w/swarm` must not claim `/w/swarm-old/index.html`, which is what comparing the two
    ...
    func refusesASiblingWithTheSamePrefix() throws {
        ...
        let root = parent.appendingPathComponent("swarm", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let sibling = try write("swarm-old/index.html", in: parent)
