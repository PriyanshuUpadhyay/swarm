...
/// strongest words there is how people learn to click through them, on the one action in Swarm
...
struct ArchiveConfirmationTests {
    private func makeWorkspace(name: String = "Fix the login redirect") -> Workspace {
        Workspace(
            repoID: .new(),
            name: name,
            branch: "swarm/fix-the-login-redirect",
            path: "/tmp/swarm/fix-the-login-redirect",
            baseBranch: "main"
        )
    }
