...
struct RepoIconDetectorTests {
    ...
    private func repository(_ files: [String: Data]) throws -> String {
        let root = TestScratch.unique("swarm-icon")
    ...
    func icnsBeatsIconBundle() throws {
        let root = try repository([
            ...
            "Resources/Swarm.icon/icon.json": Data("""
            {"groups": [{"layers": [{"image-name": "mark.svg"}]},
                        {"layers": [{"image-name": "ground.svg"}]}]}
            """.utf8),
            "Resources/Swarm.icon/Assets/mark.svg": svgData(),
            "Resources/Swarm.icon/Assets/ground.svg": svgData(),
        ])

        let all = RepoIconDetector.candidates(in: root)
        #expect(all.map { ($0.path as NSString).lastPathComponent } == ["AppIcon.icns", "Swarm.icon"])
    ...
    func iconBundleLayers() throws {
        let root = try repository([
            "Swarm.icon/icon.json": Data("""
            {"groups": [{"layers": [{"image-name": "mark.svg"}]},
                        {"layers": [{"image-name": "panel.svg"}]},
                        {"layers": [{"image-name": "ground.svg"}]}]}
            """.utf8),
            "Swarm.icon/Assets/mark.svg": svgData(),
            "Swarm.icon/Assets/panel.svg": svgData(),
            "Swarm.icon/Assets/ground.svg": svgData(),
        ])

        let bundle = (root as NSString).appendingPathComponent("Swarm.icon")
    ...
    func skippedDirectories() throws {
        let root = try repository([
            ...
            ".build/debug/Swarm.app/Contents/Resources/AppIcon.icns": icns(["ic10"]),
    ...
    @Test("Swarm's own checkout answers with the icon it ships")
    func swarmItself() throws {
        // Four levels up from Tests/SwarmCoreTests/<this file>. Symlinks resolved first, because
        ...
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .path
        // A layered document, because that is the only icon Swarm ships now. The flat AppIcon.icns
        ...
        try #require(FileManager.default.fileExists(atPath: root + "/Resources/Swarm.icon"))
        #expect(!FileManager.default.fileExists(atPath: root + "/Resources/AppIcon.icns"))

        let best = RepoIconDetector.detect(in: root)
        #expect(name(of: best) == "Swarm.icon")
...
struct RepoIconStorageTests {
    ...
    func roundTrip() async throws {
        ...
        let repo = try await store.upsert(Repo(
            name: "runswarm",
            path: "/tmp/runswarm",
            iconPath: "/tmp/runswarm/public/favicon.svg",
            iconSource: .detected
        ))

        let read = try #require(try await store.repo(id: repo.id))
        #expect(read.iconPath == "/tmp/runswarm/public/favicon.svg")
    ...
    func backToTheMonogram() async throws {
        ...
        var repo = try await store.upsert(Repo(
            name: "runswarm", path: "/tmp/runswarm",
    ...
    @Test("a project added before Swarm looked for icons is not silently redrawn")
    func existingProjectsKeepTheirMonogram() async throws {
        let path = TestScratch.unique("swarm-icon-migrate") + ".sqlite"
        let id: String
        do {
