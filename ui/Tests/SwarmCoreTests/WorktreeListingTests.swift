...
struct WorktreeListingTests {
    ...
    func readsARepositoryOnThisMachine() async throws {
        ...
        #expect(holders["feature"]?.isSwarmWorkspace == false)
...
struct BranchHolderTests {
    ...
    /// The whole bug in one assertion. Swarm's database knew about the two workspaces under
    /// `~/swarm/workspaces`, and about nothing else, so the row for pull request #362 looked free
    /// and the create ran until git refused it.
    @Test("a worktree Swarm did not make holds its branch just as firmly")
    func seesAWorktreeFromAnotherApplication() {
        ...
        // One of Swarm's own that the database has no row for. Git still says it is taken, and it
        // is: the honest answer is the path, because there is no workspace to name.
        #expect(
            holders["freekmurze/review-repo-changes"]
                == .otherWorktree(
                    path: "/Users/freek/swarm/workspaces/there-there/freekmurze-molucca-sea"
                )
        )
    ...
    func comparesPathsRatherThanStrings() {
        for path in [
            ...
            "/Users/freek/dev/code/./there-there",
            "/Users/freek/dev/code/swarm/../there-there",
        ] {
            let holders = BranchHolder.byBranch(worktrees: thereThere, projectPath: path)
            #expect(holders["main"]?.isSwarmWorkspace == false)
            if case .projectCheckout = holders["main"] {} else {
    ...
    /// read differently, which is the point, because only one of them is somewhere Swarm can take
    /// you.
    ...
    func notesReadDifferently() {
        ...
        #expect(BranchHolder.workspace("Quiet Harbour").isSwarmWorkspace)
        #expect(!BranchHolder.otherWorktree(path: "/tmp/x").isSwarmWorkspace)
        #expect(!BranchHolder.projectCheckout(path: "/tmp/x").isSwarmWorkspace)
    }
