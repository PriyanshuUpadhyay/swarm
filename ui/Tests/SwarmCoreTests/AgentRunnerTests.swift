...
struct AgentRunnerArgvTests {
    @Test("Ask Swarm receives host instructions on new and resumed conversations",
          arguments: [false, true], [false, true])
    func askSwarmInstructions(hasWorkspace: Bool, resumed: Bool) {
    ...
    /// Swarm has no name for, which is why `ComposerOption.adding` exists. Passing it through is
    ...
    @Test("an effort Swarm does not recognise is still passed through")
    func unknownEffort() {
    ...
    func fastModeOn() {
        ...
        // starts, so the only thing Swarm may ever send here is a literal.
    ...
    /// The picker that started this: Claude Code gained output styles, and Swarm offers them in
    ...
    /// A style is named by a file somebody else wrote, so the name is not Swarm's to trust. Built
    ...
    @Test("the settings object is passed once and holds only what Swarm states")
...
struct AgentRunnerPersistenceTests {
    ...
    /// an `init` and closed by a real `result`, without Swarm sending it anything. Nothing moved
    ...
    @Test("a result for a turn Swarm never started closes nothing")
...
struct AgentRunnerProcessTests {
    ...
    func runsATurn() async throws {
        ...
        // 25 event rows plus the user turn Swarm wrote itself.
...
private func fixtureSessionLines() throws -> [String] {
    try swarmFixtureLines("session-basic.jsonl")
}
...
struct AgentRunnerPermissionTests {
    ...
    /// has no project to store a rule against. Ask Swarm's card does not offer the button, and
    ...
    @Test("quitting denies in words too, and says it was Swarm that did it")
