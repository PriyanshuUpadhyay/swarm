...
/// The real `swarm-bridge` binary, launched as a subprocess and spoken to on its stdin exactly as
/// an agent CLI speaks to an MCP server.
///
/// `Tools/test-core.sh` builds the shim into the throwaway package and names it in
/// `SWARM_UI_BRIDGE_SHIM`; without that there is nothing to drive, and these are skipped the way the
...
struct BridgeShimTests {
    ...
    /// and not the running test's own directory either: that is a `swarm-scratch-` plus a whole
    ...
    func relaysAConversation() async throws {
        ...
        #expect(text.contains("swarm/cut-the-index"))
    ...
    func withoutAnApp() async throws {
        ...
        #expect(await process.exitStatus == BridgeShim.Exit.cannotReachSwarm)
        #expect(complaints.joined(separator: " ").contains(socketPath))
    }

    /// Swarm quitting with a call in flight. The shim used to exit 0 and write nothing anywhere,
    ...
    @Test("says so and fails when Swarm goes away with a call in flight", .timeLimit(.minutes(1)))
    func swarmQuitsMidCall() async throws {
        let socketPath = scratchSocket()
        let welcome = String(
        ...
        let listener = try UnixSocketListener(path: socketPath) { connection in
            Task {
                ...
                // The call arrives, and then Swarm is gone without having answered it.
                _ = await incoming.next()
                connection.close()
            }
        }
    ...
    /// The Sparkle case, from the shim's side: the bundle was replaced underneath a running Swarm,
    ...
    @Test("prints Swarm's refusal and exits when the protocol does not match", .timeLimit(.minutes(1)))
    func refusedAtTheHandshake() async throws {
        let socketPath = scratchSocket()
        let refusal = BridgeWelcome.refusing("Swarm speaks 1 and this swarm-bridge speaks 2. Quit and reopen Swarm.")
