...
struct CrewToolTests {
    ...
    private func fixture(_ label: String) async throws -> Fixture {
        ...
        let workspace = try await store.upsert(Workspace(
            repoID: repo.id, name: "crew", branch: "swarm/crew",
    ...
    /// `agent_list` reads rows and reaches nothing else, so a Swarm with no app behind it serves
    ...
    @Test("Swarm answers its own permission question about all four")
    func selfApproved() {
    ...
    /// **Swarm sweeps nothing**, so a finished subagent leaves the owner's sidebar when the
