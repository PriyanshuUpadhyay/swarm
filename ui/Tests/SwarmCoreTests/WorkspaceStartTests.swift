...
struct WorkspaceStartTests {
    ...
    /// row and a teardown looking for whatever holds `$SWARM_UI_PORT` had nothing to look for. See
...
/// the owner's own `be.spatie.swarm` domain, which is the one thing the house rules say never to
/// touch.
@Suite("Which tab a new workspace opens on")
struct WorkspaceOpeningTabTests {
    private func scratchDefaults() -> UserDefaults {
        let suite = "swarm.tests.startmode.\(UUID().uuidString)"
