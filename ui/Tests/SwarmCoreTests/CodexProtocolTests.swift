...
private func codexFixture(_ name: String) throws -> [String] {
    try swarmFixtureLines(name)
}

private func codexFixtureJSON(_ name: String) throws -> JSONValue {
    let text = try swarmFixtureLines(name).joined(separator: "\n")
...
@Suite struct CodexEventTests {
    @Test func readsAWholeTurnOffTheRecording() throws {
        ...
        // The methods Swarm does not lift out are still seen, with their names, rather than lost.
    ...
    @Test func readsTheUserMessageItem() throws {
        ...
        #expect(message.text == "Reply with exactly one word: swarm")
    ...
    @Test func mapsTokensOntoSwarmsOwnUsageWithoutDoubleCountingTheCache() {
...
@Suite struct CodexModelTests {
    ...
    /// set from the five Swarm hardcodes today.
    ...
    /// Swarm had to be edited to offer it. A hardcoded list would have missed all of it.
    @Test func readsAGenerationOfModelsNothingHereHadHeardOf() throws {
        ...
        // The name the picker draws is the server's, so a model Swarm has never heard of is still
        ...
        // Its own default rather than Swarm's `high`, which is why the model carries one.
