...
/// `Swarm --open-url` is a harness affordance, and a harness that silently drops its argument is
...
struct OpenURLArgumentTests {
    ...
    func repairedPromptRoundTrips() throws {
        ...
        // Read back the way `SwarmDeepLink.values(from:)` reads it: split on & and =, then map
    ...
    func otherSchemesRepairToo() throws {
        let url = try #require(OpenURLArgument.url(from: "swarmdev://prompt=two words&path=/t"))
        #expect(url.scheme == "swarmdev")
    }
}
