...
struct CheckFailureHandoffTests {
    ...
    func readsJob() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/spatie/swarm/actions/runs/42/job/99"
        )
    ...
    func readsRerun() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/spatie/swarm/actions/runs/42/attempts/2/job/99"
        )
        #expect(target == CheckFailureHandoff.LogTarget(runID: "42", jobID: "99"))
    }
    ...
    func readsRun() {
        let target = CheckFailureHandoff.logTarget(
            detailsURL: "https://github.com/spatie/swarm/actions/runs/42"
        )
    ...
    func refusesForeignURLs() {
        ...
        #expect(CheckFailureHandoff.logTarget(detailsURL: "https://github.com/spatie/swarm/pull/7") == nil)
    ...
    func writesTheSentence() {
        ...
        let sentence = CheckFailureHandoff.sentence(
            ...
            state: .failed,
            detailsURL: "https://github.com/spatie/swarm/actions/runs/42/job/99",
            logPath: ".swarm/attachments/aB3dEf/Run tests.log",
            excerpt: excerpt
        )
        #expect(sentence.contains("\"Run tests\""))
        #expect(sentence.contains("in CI"))
        #expect(sentence.contains("failed."))
        #expect(sentence.contains(AttachmentDraft.token(for: ".swarm/attachments/aB3dEf/Run tests.log")))
    ...
    func theSentenceIsAValidDraft() {
        let path = ".swarm/attachments/aB3dEf/Run tests.log"
