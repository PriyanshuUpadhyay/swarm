...
struct BridgeSocketPathTests {
    ...
    func sharesTheFingerprint() throws {
        let database = "/Users/someone/Library/Application Support/Swarm/swarm.sqlite"
        ...
        #expect(path == "/tmp/swarm-bridge-\(fingerprint).sock")
        ...
        let dev = try BridgeSocketPath.derive(
            databasePath: "/Users/someone/Library/Application Support/Swarm Dev/swarm.sqlite",
            directory: "/tmp"
        )
        #expect(dev != path)
    }
    ...
    func fitsInTheRealPlace() throws {
        let path = try BridgeSocketPath.derive(databasePath: "/Users/someone/Library/Application Support/Swarm/swarm.sqlite")
...
struct BridgeRegistrationTests {
    private let attachment = BridgeAttachment(
        ...
        socketPath: "/var/folders/xx/T/swarm-bridge-1a2b3c4d.sock",
        token: "t0ken",
        role: .workspace
    )

    /// The finding this test exists for, measured on codex-cli 0.147.0: a `-c` override of
    /// `mcp_servers.swarm` against a user's own entry of that name does NOT shadow it. `command`
...
struct BridgeHandshakeTests {
    ...
    func matching() {
        let hello = BridgeHello(token: "t", role: "parent", shim: "/tmp/swarm-bridge")
    ...
    /// shim meets an older Swarm, or an old per-session config names a binary that has been
