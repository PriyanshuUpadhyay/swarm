...
struct RepoIconRefreshTests {
    ...
    private func project(
        ...
        icon: String? = nil,
        path: String = "/projects/swarm"
    ) -> Repo {
        Repo(name: "swarm", path: path, iconPath: icon, iconSource: source)
    }
    ...
    private let folderOnly: (String) -> Bool = { $0 == "/projects/swarm" }
    ...
    @Test("a project added before Swarm looked for icons is searched even with a stale path on it")
    func neverLookedWithAPath() {
        let stale = project(.undetected, icon: "/projects/swarm/public/favicon.svg")
    ...
    func plainerArtwork() {
        ...
        let badged = project(.detected, icon: "/projects/swarm/public/favicon-unread-1.svg")
        let publicFolder = [
        ...
        let plain = project(.detected, icon: "/projects/swarm/public/favicon.svg")
        #expect(
        ...
        let onlyDark = project(.detected, icon: "/projects/swarm/assets/logo-dark.svg")
        #expect(
        ...
        #expect(
            RepoIconRefresh.reasonToSearch(
                project(.detected, icon: "/projects/swarm/public/favicon-unread.png"),
                exists: everything,
    ...
    func chosenIsTheLastWord() {
        ...
        // Not even when the picture they named has gone. Unsetting it would be Swarm deciding
    ...
    @Test("artwork Swarm found and can still read is left exactly as it is")
    func detectedAndPresentIsLeftAlone() {
        let detected = project(.detected, icon: "/projects/swarm/public/favicon.svg")
    ...
    func aMissingFolderIsSkipped(source: RepoIconSource) {
        let away = project(source, icon: "/volumes/work/swarm/favicon.svg", path: "/volumes/work/swarm")
    ...
    func homeRelativePathsAreExpanded() {
        let repo = Repo(name: "swarm", path: "~/dev/swarm", iconSource: .undetected)
        var asked: [String] = []
        ...
        #expect(asked == [(("~/dev/swarm") as NSString).expandingTildeInPath])
    ...
    @Test("a search that finds something stores it as Swarm's own guess rather than as a choice")
    func somethingFoundIsStoredAsDetected() {
        var repo = project(.undetected)
        let found = RepoIconCandidate(
            path: "/projects/swarm/public/favicon.svg", format: .svg, origin: .favicon, pixels: 0
        )
        let answer = RepoIconAnswer(found: found)
        answer.apply(to: &repo)

        #expect(repo.iconPath == "/projects/swarm/public/favicon.svg")
        #expect(repo.iconSource == .detected)
        #expect(repo.hasIcon)
        // Still Swarm's guess, so the file going missing is grounds for looking again, and a file
    ...
    func anUnchangedAnswerWritesNothing() {
        ...
        let same = project(.detected, icon: "/projects/swarm/public/favicon.svg")
