...
/// reads "Swarm: agent start" behind a puzzle piece: the transport and the word "extension", where
...
struct CrewPresentationTests {
    ...
    func startingNamesTheAgent() {
        let row = present(CrewToolName.start, [
            ...
            "task": .string("Read every file under Sources/SwarmCore/Git and report."),
        ])
    ...
    func sayingNamesTheTarget() {
        let row = present(CrewToolName.say, [
            "to": .string("tests"),
            "message": .string("The parser moved to Sources/SwarmCore/Git/DiffParser.swift."),
        ])
    ...
    /// The generic row is still the right answer for the rest of Swarm's own tools, and for
