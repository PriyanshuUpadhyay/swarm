...
struct PaletteContrastTests {
    ...
    /// The accent stopped being only Swarm's business the day `NSAccentColorName` went into the
    /// bundle, and this is the floor that came with it.
    ///
    /// Before that the accent reached only what Swarm drew, so a wrong one was a wrong chip. Now
    ...
    /// Why the browser bar's address is not drawn in Swarm's own tertiary ink.
    ///
    ...
    @Test("a translucent ground costs Swarm's tertiary ink its floor in dark")
    ...
    func theInkOnGlassDiffersBecauseTheGroundDoes() {
        ...
        // Light: the ground barely moves, Swarm's own ink clears the floor, and AppKit's does not.
        let lit = Contrast.composited(0xFFFFFF, over: PaletteInk.surfaceSunken.light, at: 0.25)
    ...
    /// are the closest pair of meaning colours Swarm deliberately draws apart, at 27.9 in light and
    ...
    /// The neighbour a blue busy mark has that an orange one did not: Swarm's own tertiary ink.
    ///
    ...
    func theHouseBlueCannotDoThisJob() {
        ...
        // Swarm's own accent is, which is the whole of "the same family, moved off the teal".
