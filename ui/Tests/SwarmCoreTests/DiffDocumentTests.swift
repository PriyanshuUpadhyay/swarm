...
/// The one sequential pass over a diff, which lived in `Sources/Swarm/Views/Inspector` and so could
/// not be tested at all: the test target depends on SwarmCore alone. `SyntaxHighlighter` and
