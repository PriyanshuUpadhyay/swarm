...
///
/// They exist because the agent protocol is the one part of Swarm that cannot be proven correct
/// against a fixture. A fixture only proves we still decode what the CLI emitted the day it was
/// captured.
private let liveEnabled = ProcessInfo.processInfo.environment["SWARM_UI_LIVE"] == "1"
...
/// a captured envelope only proves Swarm still reads what the CLI emitted on the day it was
/// captured. The flags this invocation depends on (`--json-schema`, `--tools ""`,
/// `--system-prompt`, `--safe-mode`) are the CLI's, not Swarm's, and this is the only thing that
/// notices when one of them changes.
///
///     SWARM_UI_LIVE=1 ./Tools/test-core.sh LiveNaming
@Suite("LiveNaming", .enabled(if: liveEnabled), .tags(.subprocess), .scratchDirectory)
struct LiveNamingTests {
    @Test("a real model names a real task, in a shape Swarm will accept", .timeLimit(.minutes(2)))
