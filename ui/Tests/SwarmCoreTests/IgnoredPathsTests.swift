...
/// main checkout: .swarm/attachments/.gitignore, .swarm/attachments/9JVKW4/IMG_4395.jpeg, .env,
...
struct IgnoredPathsTests {
    ...
    /// Swarm creates `.swarm/attachments` itself, to hold the files somebody dragged onto a
    /// prompt. Listing it as work at risk is the app warning the user about the app.
    @Test(
        "Swarm's own directory is never reported",
        arguments: [".swarm/", ".swarm/attachments/.gitignore", ".swarm/attachments/9JVKW4/IMG_4395.jpeg"]
    )
    func swarmsOwn(path: String) {
        #expect(ReproduciblePaths.canBeRebuilt(path))
    }
    ...
    func envSurvivesTheNoise() async throws {
        ...
        // The file Swarm copies into every worktree, edited by the agent. This is the loss the
        ...
        // Swarm's own directory, created by Swarm, reported to the user by Swarm.
        try worktree.write(".swarm/attachments/9JVKW4/IMG_4395.jpeg", "not really a jpeg\n")
        ...
        #expect(sentence.contains(".swarm") == false)
    }
