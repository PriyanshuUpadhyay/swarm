...
private func codexEvents(_ name: String) throws -> [CodexEvent] {
    try swarmFixtureLines(name).compactMap { line in
...
@Suite struct CodexTranslationTests {
    @Test func turnsARecordedTurnIntoSwarmsOwnEvents() throws {
...
@Suite struct CodexPermissionTests {
    ...
    /// The narrowest rule there is. Swarm will not invent a pattern, because Codex offers no
    ...
    /// the rest of this session, Swarm keeps the durable record itself, and no file belonging to
    /// the user is written.
...
@Suite struct CodexPatchCountingTests {
    /// A Codex patch states its own numbers. Swarm's Claude Code counting estimates them from the
    ...
    @Test func aRecordedPatchCarriesADiffThatCanBeCounted() throws {
        let change = try swarmFixtureLines("codex-approval.ndjson")
    ...
    @Test func anEditCarriesAHunkAndCountsBothWays() throws {
        let change = try swarmFixtureLines("codex-edit-patch.ndjson")
            .compactMap { line -> CodexFileChange? in
                guard case .notification(let notification)? = CodexFrame.decode(line: line),
