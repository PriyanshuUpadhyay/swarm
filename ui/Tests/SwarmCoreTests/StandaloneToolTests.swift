...
/// Naming a project from outside Swarm, which no caller could do until the owner's own client
...
struct BridgeProjectLookupTests {
    ...
    private let swarm = Repo(name: "swarm", path: "/Users/me/dev/swarm", defaultBranch: "main")

    @Test("the name in the sidebar finds it, whatever case it is typed in")
    func byName() {
        #expect(BridgeProjectLookup.find("FLARE", in: [flare, swarm]) == .found(flare))
    }

    @Test("the path finds it, tilde and trailing slash and all")
    func byPath() {
        #expect(BridgeProjectLookup.find("/Users/me/dev/swarm/", in: [flare, swarm]) == .found(swarm))
        #expect(BridgeProjectLookup.find("/Users/me/dev/./flare", in: [flare, swarm]) == .found(flare))
    }

    /// `/tmp` is a symlink to `/private/tmp` on this machine, and a project registered through one
    /// used to be a miss when named through the other. The `swarm://` link resolved symlinks and
    ...
    func byID() {
        #expect(BridgeProjectLookup.find(flare.id.rawValue, in: [flare, swarm]) == .found(flare))
    }
    ...
    @Test("a miss names what Swarm does have and where to go next")
    func missNamesTheAlternatives() {
        let outcome = BridgeProjectLookup.find("flair", in: [flare, swarm])
        let refusal = BridgeProjectLookup.refusal(for: "flair", outcome: outcome, projects: [flare, swarm])
...
struct ProjectAddToolTests {
    ...
    @Test("a repository is registered and turns up in Swarm's projects")
    ...
    /// A worktree Swarm cut is not a project. Registering one would give Swarm a project whose
    /// workspaces are worktrees of a worktree.
    @Test("one of Swarm's own workspaces is refused")
    func refusesAWorktree() async throws {
        ...
        guard case .refuse(let refusal) = verdict, case .insideSwarmsWorkspaces = refusal else {
    ...
    /// A near miss on the name of Swarm's worktree root must not be read as being inside it.
...
struct ProjectListToolTests {
    ...
    /// A workspace agent may name another project to `workspace_start`, and this is how it finds
    /// out what it may name.
    @Test("the owner and every workspace agent see it")
    func roleGate() {
    ...
    /// A read a workspace agent makes before handing work on, while nobody may be watching.
    @Test("Swarm answers its permission question, and still asks about the project tools that change things")
    func selfApproved() {
    ...
    @Test("a workspace agent is listed the projects, and is not told to call a tool it lacks")
    func listsToAWorkspace() async throws {
        ...
        var hidden = try await store.upsert(Repo(name: "swarm", path: "/tmp/swarm", defaultBranch: "main"))
        hidden.hidden = true
        ...
        #expect(result.text.contains("\"name\" : \"swarm\""))
        #expect(result.text.contains("\"hidden_projects\" : 1"))
        #expect(!result.text.contains("project_unhide"))
    }

    @Test("an empty Swarm says what to do rather than answering with nothing")
...
@Suite("workspace_start from outside Swarm", .tags(.persistence), .scratchDirectory)
struct OwnerWorkspaceStartTests {
    ...
    @Test("a project Swarm does not have is refused, and nothing is registered on the way")
...
struct OwnerStartRateTests {
    ...
    func theRefusalDoesNotInviteARetry() async throws {
        ...
        // No path inside Swarm, and no command line. See `WorkspaceStartTrouble`.
    ...
    @Test("workspaces made in Swarm's own window do not count against the tool")
...
@Suite("whoami from outside Swarm", .tags(.persistence), .scratchDirectory)
struct OwnerWhoamiTests {
    /// Swarm and Swarm Dev both listen, on their own sockets, and a configuration pointing at the
    /// wrong one works perfectly in the wrong database. Naming the database is what tells them
    /// apart.
    @Test("it names the copy of Swarm that answered and what it is holding")
