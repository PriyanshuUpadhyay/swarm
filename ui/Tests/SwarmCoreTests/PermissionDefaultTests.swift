...
/// open, a `swarm://` link and a session row written with no mode stated each used to reach their
...
/// Swarm where the difference is the whole point.
...
struct PermissionDefaultTests {
    ...
    /// Swarm whose Settings, Models tab has ever been saved holds a row for this key, and that row
    ...
    /// exists to stop.** Every unstated starting point above agrees with the fallback. Ask Swarm's
