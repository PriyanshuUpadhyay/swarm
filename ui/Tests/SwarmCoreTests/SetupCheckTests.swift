...
struct SetupFixTests {
    ...
    @Test("only the interactive commands claim Swarm can run them")
    func interactivity() {
...
struct SetupToolTests {
    @Test("the agent rows name agents Swarm can actually drive")
    ...
    @Test("every agent Swarm can drive has a row, so a new backend cannot be forgotten")
