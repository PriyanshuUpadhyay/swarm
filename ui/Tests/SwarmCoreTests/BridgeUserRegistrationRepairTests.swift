...
/// The owner moved Swarm from `~/Applications` to `/Applications` and every agent started from his
...
struct BridgeUserRegistrationRepairTests {
    private let moved = "/Applications/Swarm.app/Contents/MacOS/swarm-bridge"
    private let gone = "/Users/freek/Applications/Swarm.app/Contents/MacOS/swarm-bridge"

    private var attachment: BridgeAttachment {
        BridgeAttachment(
            shimPath: moved,
            socketPath: "/tmp/swarm-bridge-abc123.sock",
    ...
    private func config(_ servers: [String: Any]) throws -> Data {
        try JSONSerialization.data(withJSONObject: [
            ...
            "projects": ["/Users/freek/dev/code/swarm": ["allowedTools": [String]()]],
            "mcpServers": servers,
        ])
    }
    ...
    func correctEntry() throws {
        ...
        let data = try config(["swarm": entry(command: moved)])
    ...
    func repairsTheMove() throws {
        let data = try config(["swarm": entry(command: gone)])
        ...
        #expect(from == gone)
        #expect(to == moved)
        let repointed = try command(in: rewritten, serverNamed: "swarm")
    ...
    func keepsEverythingElse() throws {
        ...
        let ours = try #require(servers["swarm"] as? [String: Any])
    ...
    func somebodyElsesEntry() throws {
        ...
        let strangeSocket = try config(["swarm": entry(command: gone, socket: "/tmp/swarm-bridge-other.sock")])
        let strangeToken = try config(["swarm": entry(command: gone, token: "deadbeef")])
        let noEnvironment = try config(["swarm": ["command": gone]])
        ...
        let wrapper = try config(["swarm": entry(command: "/Users/freek/bin/bridge-wrapper")])
    ...
    func deliberateCrossWire() throws {
        // The shim is a relay that takes its socket and its token out of the environment, so Swarm
        ...
        let elsewhere = "/Users/freek/Applications/Swarm Dev.app/Contents/MacOS/swarm-bridge"
        let data = try config(["swarm": entry(command: elsewhere)])
    ...
    func theBundleIdentities() throws {
        ...
        let devShim = "/Users/freek/Applications/Swarm Dev.app/Contents/MacOS/swarm-bridge"
        let dev: [String: Any] = [
            "command": devShim,
            "env": [
                BridgeProtocol.socketVariable: "/tmp/swarm-bridge-def456.sock",
                BridgeProtocol.tokenVariable: "fedcba9876543210",
                BridgeProtocol.roleVariable: "owner",
            ],
        ]
        let data = try config(["swarm": entry(command: gone), "swarm-dev": dev])
        ...
        let ours = try command(in: rewritten, serverNamed: "swarm")
        let theirs = try command(in: rewritten, serverNamed: "swarm-dev")
        #expect(ours == moved)
        ...
        let devAttachment = BridgeAttachment(
            shimPath: devShim,
            socketPath: "/tmp/swarm-bridge-def456.sock",
    ...
    func malformed() {
        ...
        #expect(decide(Data(#"{"mcpServers": {"swarm":"#.utf8)) == .leaveAlone(.malformed))
        ...
        #expect(decide(Data(#"{"mcpServers": {"swarm": "none"}}"#.utf8)) == .leaveAlone(.absent))
    }
    ...
    func writesThroughToDisk() throws {
        ...
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("swarm-repair-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let path = directory.appendingPathComponent("claude.json").path

        try config(["swarm": entry(command: gone)]).write(to: URL(fileURLWithPath: path))
        ...
        let repointed = try command(in: written, serverNamed: "swarm")
        #expect(repointed == moved)
    ...
    func absentFileIsNotWritten() {
        let path = NSTemporaryDirectory() + "/swarm-repair-\(UUID().uuidString)/claude.json"
