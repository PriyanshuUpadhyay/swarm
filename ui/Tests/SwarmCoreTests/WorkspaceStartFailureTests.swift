...
struct WorkspaceStartFailureTests {
    ...
    private func emptyRepository() async throws -> String {
        let path = TestScratch.unique("swarm-git")
    ...
    /// Anything unrecognised still reaches the caller, but without the argv Swarm built. The
    ...
    func unexplainedDropsTheCommandLine() async throws {
        ...
        let error = ShellError(
            command: "git worktree add -b do-thing -- /Users/freek/swarm/workspaces/swarm-git-711961F7/do-thing main",
            status: 128,
        ...
        #expect(!trouble.sentence.contains("swarm-git-711961F7"))
    }

    /// The leak that started this. None of the sentences may carry the argv or a path inside
    /// Swarm's own worktree root, whatever the cause.
    ...
    func nothingLeaks() async throws {
        ...
        for trouble in troubles {
            ...
            #expect(!trouble.sentence.contains("workspaces/swarm-git"))
        }
    }
