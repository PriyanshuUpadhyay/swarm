...
/// project loses the icon Swarm found for it and goes back to drawing its initials, or gets its
...
struct RepoWriteIsolationTests {
    private func seed(_ store: Store) async throws -> Repo {
        try await store.upsert(Repo(
            name: "swarm", path: TestScratch.unique("project"), defaultBranch: "main", accent: "4C8DF6"
        ))
    }
    ...
    func accentDoesNotEraseTheIcon() async throws {
        ...
        try await store.update(repoID: repo.id) {
            $0.iconPath = "/projects/swarm/public/favicon.png"
        ...
        #expect(stored.iconPath == "/projects/swarm/public/favicon.png")
    ...
    func iconWriteDoesNotUndoARename() async throws {
        ...
        try await store.update(repoID: repo.id) { $0.name = "Swarm" }
