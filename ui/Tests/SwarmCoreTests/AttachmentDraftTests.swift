...
struct AttachmentDraftTests {
    static let copy = ".swarm/attachments/9JVKW4/shot.png"
    static let spaced = ".swarm/attachments/aB3xZ9/Pasted 2026-08-20 at 22.29.20.png"
    static let inWorktree = "Sources/Swarm/Views/Center/ComposerView.swift"
    ...
    @Test("A file in Swarm's own folder is recognised from the text alone")
    func copiesNeedNoTelling() {
        // The pull request instructions, in both of the places they can be.
        #expect(AttachmentDraft.isAttachment(".swarm/scratch/pr-instructions.md"))
        #expect(AttachmentDraft.isAttachment(".swarm/pr-instructions.md"))
        // Somebody else's file, which is only a chip when the composer vouches for it.
        #expect(!AttachmentDraft.isAttachment("Sources/Swarm/Views/Center/ComposerView.swift"))
        #expect(!AttachmentDraft.isAttachment(".swarm/"))
        #expect(AttachmentDraft.isAttachment(Self.copy))
        #expect(AttachmentDraft.isAttachment(Self.spaced))
        #expect(!AttachmentDraft.isAttachment(".swarm/attachments/9JVKW4"))
        #expect(!AttachmentDraft.isAttachment(".swarm/attachments/9JVKW4/"))
        #expect(!AttachmentDraft.isAttachment(".swarm/attachments//shot.png"))
        #expect(!AttachmentDraft.isAttachment("git status"))
        #expect(!AttachmentDraft.isAttachment(""))
        #expect(!AttachmentDraft.isAttachment(".swarm/attachments/9J\nVKW4/a.png"))
    }
