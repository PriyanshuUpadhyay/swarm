...
/// Swarm runs several agents at once, so a notification is only useful when it names a workspace
...
/// live in SwarmCore precisely so they can be asserted on without a bundle or a permission.
...
struct NotificationPreferencesTests {
    ...
    private func makeDefaults() throws -> (UserDefaults, String) {
        let name = "swarm.notifications.\(UUID().uuidString)"
...
struct NotificationDigestTests {
    ...
    func singleKeepsTheWorkspaceName() {
        ...
        #expect(prepared?.identifier == "swarm.turnFinished.w1")
    }
    ...
    func collapsesABatchIntoASummary() {
        ...
        #expect(prepared?.workspaceID == WorkspaceID("w1"))
        #expect(prepared?.threadIdentifier == "swarm.turnFinished")
    }
    ...
    func everyEventHasASummary() {
        for event in NotificationEvent.allCases {
            ...
            #expect(prepared?.identifier == "swarm.\(event.rawValue).digest")
        }
    }
