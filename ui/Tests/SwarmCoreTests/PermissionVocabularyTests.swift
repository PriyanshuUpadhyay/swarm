...
/// A user running Codex inside Swarm read the composer's permission menu, compared it with the
/// Codex app's, and could not match the two up. Two faults behind that, and both are pinned here:
/// Swarm printed Claude Code's names over a Codex chat, and it had no row at all for the preset
