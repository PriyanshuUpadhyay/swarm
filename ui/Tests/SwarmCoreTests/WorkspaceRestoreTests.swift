...
struct WorkspaceRestoreTests {
    ...
    func preferredPathIsFlatUnderTheProject() {
        ...
        #expect(
            WorktreePath.preferred(branch: "dark-mode", project: "swarm", under: root)
                == "/w/swarm/dark-mode"
        )
        // Nested, and the directory would be too, under a name no row records.
        #expect(
            WorktreePath.preferred(branch: "freek/dark/mode", project: "swarm", under: root)
                == "/w/swarm/freek-dark-mode"
        )
    }
