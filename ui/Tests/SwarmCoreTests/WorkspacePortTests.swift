...
/// an archive script take down what a setup script put up, since it is what makes `$SWARM_UI_PORT`
...
struct WorkspacePortTests {
    ...
    /// value and in `$SWARM_UI_PORT`, so there is nothing to backfill and nothing to guess at: the
