...
struct NewProjectStarterTests {
    ...
    private func scratch() -> (location: String, name: String) {
        (TestScratch.unique("swarm-new"), "sparkline")
    }
    ...
    func createsAProjectAWorktreeCanStartFrom() async throws {
        ...
        // Nothing of Swarm's own: no README, no .gitignore. The first thing in the history is
        ...
        try await Git.addWorktree(
            repo: target, path: worktree, branch: "swarm/test", base: creation.branch
        )
    ...
    @Test("abandoning takes away exactly what Swarm made")
    func discardRemovesTheFolderSwarmMade() async throws {
        let (location, name) = scratch()
        let target = (location as NSString).appendingPathComponent(name)
        try FileManager.default.createDirectory(atPath: target, withIntermediateDirectories: true)
    ...
    /// A folder Swarm did not make is not Swarm's to remove, however empty it is.
    ...
    /// The moment there is a commit the folder holds work, and work is never Swarm's to delete.
