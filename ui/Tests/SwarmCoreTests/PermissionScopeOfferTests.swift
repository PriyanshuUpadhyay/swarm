...
struct PermissionScopeOfferTests {
    ...
    func inAProject() {
        let offer = PermissionScopeOffer.of(ask: ask(), project: "swarm")
        #expect(offer.scopes == [.project, .session, .once])
        #expect(offer.prominent == .project)
        #expect(offer.widens)
        #expect(offer.explanation.contains("swarm"))
    }
    ...
    /// question of scope is the CLI's here, and Swarm does not get a second opinion.
    @Test("an ask that cannot be widened is one call, in a project or out of one")
    func cannotWiden() {
        for project in ["swarm", nil] {
