...
/// One typed line, and the four things Swarm can do about it.
///
...
struct ProjectTargetTests {
    ...
    private let workspaces = "/Users/tester/swarm/workspaces"
    ...
    /// A slash anywhere means a place. It used to mean a name Swarm would not take, and the
    /// refusal for that is still there for a colon, which is the separator a folder name really
    /// cannot hold.
    @Test("a relative path is a place Swarm cannot find, not a name it will not take")
    func aRelativePath() {
    ...
    /// Add it as a project rather than creating a new one over the top of it", which is Swarm
    ...
    func theDefaultLocationIsRefusableAndItsChildrenAreNot() {
        let parent = ProjectTargetVerdict.of(
            facts(
                ...
                children: ["ableton-mcp", "ansible", "swarm", "flare"]
            )
        )
        #expect(parent == .refuse(.folder(.containerOfProjects(
            ["ableton-mcp", "ansible", "flare", "swarm"]
        ))))
    ...
    func offersTheEnclosingRepository() {
        let verdict = ProjectTargetVerdict.of(
            facts(
                path: "/Users/tester/dev/code/swarm/Tools",
                exists: true, isDirectory: true, enclosing: "/Users/tester/dev/code/swarm"
            )
        )
        ...
        #expect(refusal.alternative == "/Users/tester/dev/code/swarm")
        #expect(refusal.sentence.contains("/Users/tester/dev/code/swarm"))
    }
    ...
    /// places Swarm will not work in. Both lists read in one register on purpose.
    ...
    func refusesByTheOtherList() {
        let inside = ProjectTargetVerdict.of(
            facts(path: "/Users/tester/swarm/workspaces/thing")
        )
        #expect(inside == .refuse(.target(.insideSwarmsWorkspaces("~/swarm/workspaces/thing"))))
    ...
    func theAddBlock() {
        let said = ProjectConsequence.of(
            .add(root: "/Users/tester/dev/code/swarm"), path: "/Users/tester/dev/code/swarm",
            home: home, branch: "main"
        )
        #expect(said.lead == "~/dev/code/swarm is a git repository.")
