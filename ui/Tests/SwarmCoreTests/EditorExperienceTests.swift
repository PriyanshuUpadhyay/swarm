...
struct EditorExperienceTests {
    ...
    @Test(.enabled(if: ProcessInfo.processInfo.environment["SWARM_UI_LOCAL_LSP"] == "1"))
    ...
    @Test(.enabled(if: ProcessInfo.processInfo.environment["SWARM_UI_LARAVEL_LSP_VENDOR"] != nil), .timeLimit(.minutes(2)))
    func laravelViewsAndPhpDefinitionsWorkTogether() async throws {
        let vendor = try #require(ProcessInfo.processInfo.environment["SWARM_UI_LARAVEL_LSP_VENDOR"])
