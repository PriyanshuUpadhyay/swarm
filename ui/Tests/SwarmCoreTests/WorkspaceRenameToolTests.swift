...
struct WorkspaceRenameToolTests {
    ...
    private func seed(_ store: Store, named name: String = "test") async throws -> Workspace {
        let repo = try await store.upsert(Repo(name: "swarm", path: TestScratch.unique("repo")))
        return try await store.upsert(Workspace(
            repoID: repo.id, name: name, branch: "swarm/redesign",
    ...
    private func started(by starter: Workspace, named name: String, in store: Store) async throws -> Workspace {
        try await store.upsert(Workspace(
            repoID: starter.repoID, name: name, branch: "swarm/started-\(UUID().uuidString)",
            path: TestScratch.unique("worktree"), baseBranch: "main",
    ...
    @Test("it is served by a Swarm with no app behind it, because a name is one column of one row")
    ...
    @Test("Swarm answers its own permission question about it")
    func selfApproved() {
    ...
    func agentRenamesItsOwn() async throws {
        ...
        #expect(json["branch"]?.stringValue == "swarm/redesign")
    }
    ...
    /// this call to fix.
    @Test("a token whose workspace is no longer in Swarm is told that, not told to try again")
    ...
    func renameIsIsolated() async throws {
        ...
        #expect(stored.branch == "swarm/redesign")
    ...
    func theLookup() {
        let repo = Repo(name: "swarm", path: "/tmp/swarm")
        let first = Workspace(
