...
struct SlashCommandTests {
    ...
    @Test("the built in list only holds things a Swarm turn can actually carry out")
...
///
///     SWARM_UI_LOCAL_SKILLS=1 SWARM_UI_LOCAL_PROJECT=$PWD ./Tools/test-core.sh LocalSlashCommandTests
@Suite("Local slash commands", .enabled(if: ProcessInfo.processInfo.environment["SWARM_UI_LOCAL_SKILLS"] == "1"))
struct LocalSlashCommandTests {
    ...
    func theRealTree() {
        ...
        let project = ProcessInfo.processInfo.environment["SWARM_UI_LOCAL_PROJECT"]
