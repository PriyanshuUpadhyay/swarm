...
/// `swarm-unscoped-` directories and 219 template roots** in its temporary directory. That is a
...
enum TestProcessScratch {
    ...
    static func path(pid: pid_t) -> String {
        NSTemporaryDirectory() + "swarm-test-run-\(pid)"
    }
}
...
struct TempRepo {
    ...
    init(defaultBranch: String = "main") async throws {
        ...
        path = TestScratch.unique("swarm-git")
    ...
    /// `~/swarm/workspaces` on the machine where it was found. Reading the worktree list here
...
func swarmFixtureLines(_ name: String) throws -> [String] {
    let starts = [
