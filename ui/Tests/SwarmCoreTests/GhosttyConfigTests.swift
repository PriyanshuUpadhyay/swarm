...
struct GhosttyConfigTests {
    ...
    func palettePerSlot() {
        ...
        // Untouched slots keep Ghostty's own defaults rather than borrowing Swarm's palette.
    ...
    /// terminal, in Ghostty and in Swarm alike, for a day before anyone connected the two.
    ///
    ...
    func fixturesCannotEscapeTheScratchDirectory() throws {
        ...
        let outside = NSTemporaryDirectory() + "swarm-ghostty-never-written-\(UUID().uuidString)"
    ...
    @Test("no config file anywhere means no opinion, so Swarm keeps its own colours")
