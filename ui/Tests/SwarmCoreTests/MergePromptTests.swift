...
/// Merging is the one destructive, off-machine thing Swarm offers, and it is now done by an agent
...
///
/// They were a file until they were not: written into `.swarm/scratch/merge-instructions.md` on
...
struct MergeInstructionsTests {
    ...
    /// would do both, and its local half fails from a worktree, which is the whole reason Swarm
    ...
    /// Swarm is an app built on worktrees, and the copy on this machine is the thing a reader is
...
/// no in words the reader can act on. A button that refused first would be Swarm guessing an
