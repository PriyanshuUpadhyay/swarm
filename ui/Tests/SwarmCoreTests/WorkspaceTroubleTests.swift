...
/// A directory that Swarm recorded and that has stopped being a checkout, asked to do each of the
/// things Swarm does in it.
///
...
struct WorkspaceTroubleTests {
    ...
    func creatingInAFolderThatIsNoLongerACheckout() async throws {
        ...
        let trouble = await WorkspaceTrouble.creating(
            ...
            project: "swarm", projectPath: repo.path, baseBranch: "main"
        )
        #expect(trouble == .projectNotACheckout(project: "swarm", path: repo.path))
    ...
    func creatingFromABranchThatIsGone() async throws {
        ...
        let trouble = await WorkspaceTrouble.creating(
            ...
            project: "swarm", projectPath: repo.path, baseBranch: "release"
        )
        #expect(trouble == .baseBranchGone(branch: "release", project: "swarm"))
    ...
    func restoringOntoABranchAnotherWorktreeHas() async throws {
        ...
        let trouble = await WorkspaceTrouble.restoring(
            ...
            project: "swarm", projectPath: repo.path
        )
...
struct WorkspaceTroubleShapeTests {
    ...
    private static let all: [WorkspaceTrouble] = [
        .projectGone(project: "swarm", path: "/tmp/swarm"),
        .projectNotACheckout(project: "swarm", path: "/tmp/swarm"),
        .projectHasNoCommits(project: "swarm"),
        .baseBranchGone(branch: "main", project: "swarm"),
        .createBranchInUse(branch: "main", holder: .workspace("Sidebar blue")),
        .createBranchInUse(branch: "main", holder: .projectCheckout(path: "/tmp/swarm")),
    ...
    /// git's own sentence, with the argv taken off. Swarm does not know enough about it to say
    /// where a break would belong.
    @Test("the one Swarm did not write is left alone")
