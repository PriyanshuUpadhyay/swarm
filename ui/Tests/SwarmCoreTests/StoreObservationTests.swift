...
struct StoreObservationTests {
    ...
    /// Swarm opens the same file twice in one process: the app's `Store`, and `IntentDatabase`'s
    ...
    /// rewritten, WAL churn, and a change announced to everything listening. An idle Swarm now
    /// says nothing at all.
