...
/// response", adding that a nullable field "does not clear a previously observed value". Swarm
...
struct QuotaMergeTests {
    ...
    /// `merged` answers what Swarm knows; `resolved` answers what is worth writing. The difference
...
/// The request Swarm writes and the answer it reads back, both asserted against what the real
/// binaries produced rather than against a schema.
@Suite("Quota sources")
struct QuotaSourceTests {
    @Test func writesTheControlRequestClaudeCodeAnswers() {
        let line = ClaudeCodeQuotaSource.request(id: "swarm-usage-1")
        let json = JSONValue.parse(line)

        #expect(json?["type"]?.stringValue == "control_request")
        #expect(json?["request_id"]?.stringValue == "swarm-usage-1")
