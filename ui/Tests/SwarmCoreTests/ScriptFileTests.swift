...
struct ScriptFileTests {
    private func makeRepo(_ files: [String: String] = [:]) throws -> String {
        let root = TestScratch.unique("swarm-scripts")
    ...
    @Test("a file named in .swarm overrides a string Conductor embedded")
    func swarmOutranksConductorAcrossForms() throws {
        let repo = try makeRepo([
    ...
    func aRunScriptCanNameAFile() throws {
        let repo = try makeRepo([
            ".swarm/settings.toml": """
            [scripts.run.dev]
            file = ".swarm/run-dev.sh"
            """,
            ".swarm/run-dev.sh": "#!/bin/zsh\nbun dev\n",
        ])
    ...
    func aProgramBecomesAFile() throws {
        ...
        #expect(read(repo, ".swarm/setup.sh") == script)
        let settings = read(repo, ".swarm/settings.toml") ?? ""
        #expect(settings.contains("setup_file = \".swarm/setup.sh\""))
    ...
    @Test("the file Swarm writes can actually be run")
    func theFileIsExecutable() throws {
        ...
        #expect(try mode(repo, ".swarm/setup.sh") == 0o755)
    }
    ...
    func aScriptEndsWithANewline() throws {
        ...
        #expect(read(repo, ".swarm/setup.sh") == "#!/bin/zsh\nbun install\n")
    }
    ...
    func oneLineStaysInline() throws {
        ...
        #expect(read(repo, ".swarm/setup.sh") == nil)
        #expect(read(repo, ".swarm/settings.toml")?.contains("setup = \"bun install\"") == true)
    }
    ...
    func anInlineConductorScriptIsPromoted() throws {
        ...
        #expect(read(repo, ".swarm/setup.sh")?.hasPrefix("#!/bin/zsh") == true)
        #expect(read(repo, ".swarm/settings.toml")?.contains("setup_file") == true)
    ...
    @Test("a string Swarm itself wrote earlier is moved to a file, not left behind")
    func anInlineSwarmScriptIsMigrated() throws {
        let script = "#!/bin/zsh\nbun install\n"
        let repo = try makeRepo([
            ".swarm/settings.toml": "[scripts]\nsetup = \"\"\"\n#!/bin/zsh\nbun install\n\"\"\"\n",
        ])
        ...
        let written = read(repo, ".swarm/settings.toml") ?? ""
        #expect(written.contains("setup_file = \".swarm/setup.sh\""))
        #expect(!written.contains("bun install"))
        #expect(read(repo, ".swarm/setup.sh")?.contains("bun run build") == true)
    }

    @Test("a path somebody chose by hand is kept rather than repointed at Swarm's own name")
    func aStatedPathIsKept() throws {
        ...
        #expect(read(repo, ".swarm/setup.sh") == nil)
        #expect(read(repo, ".swarm/settings.toml")?.contains("bin/dev-setup.sh") == true)
    }
    ...
    func aLocalScriptGetsItsOwnFile() throws {
        ...
        #expect(read(repo, ".swarm/setup.local.sh")?.contains("mine, longer") == true)
        #expect(read(repo, ".swarm/setup.sh") == nil)
        #expect(read(repo, ".swarm/settings.local.toml")?.contains("setup.local.sh") == true)
    }
    ...
    func theIgnoreRuleCoversScripts() throws {
        ...
        let ignore = read(repo, ".swarm/.gitignore") ?? ""
    ...
    func aLongRunScriptBecomesAFile() throws {
        ...
        #expect(settings.contains("file = \".swarm/run-test.sh\""))
        #expect(read(repo, ".swarm/run-test.sh")?.contains("bun test --watch") == true)
        #expect(read(repo, ".swarm/run-dev.sh") == nil)
    }
    ...
    func roundTripsThroughTheLoader() throws {
        ...
        #expect(settings.scriptFiles[.setup]?.path == ".swarm/setup.sh")
        #expect(settings.scriptFiles[.archive]?.path == ".swarm/archive.sh")
    }
    ...
    func clearingRemovesThePointer() throws {
        ...
        #expect(read(repo, ".swarm/settings.toml")?.contains("setup_file") == false)
        ...
        #expect(read(repo, ".swarm/setup.sh")?.contains("bun install") == true)
    }
    ...
    func anExecutableFileIsRunDirectly() throws {
        ...
        #expect(launch == .executable(path: (repo as NSString).appendingPathComponent(".swarm/setup.sh")))
    ...
    func aFileWithoutTheBitIsSourced() throws {
        ...
        try FileManager.default.setAttributes(
            ...
            ofItemAtPath: (repo as NSString).appendingPathComponent(".swarm/setup.sh")
        )
        let settings = SettingsLoader.load(repo: repo)
    ...
    func aMissingFileIsItsOwnAnswer() throws {
        ...
        #expect(launch == .missing(path: ".swarm/setup.sh"))
