...
/// What a browser tab wears: which of a page's declarations Swarm asks for, what it files the
...
/// at 14 points inside Swarm's own chrome, so `read` is the gate, and every case here is a way a
...
struct BrowserFaviconTests {
    ...
    /// a page Swarm does not go looking at.
