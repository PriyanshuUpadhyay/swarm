...
/// Rules the user granted, and the promise that Swarm never widens one.
///
/// The matching rule under test is exact equality and nothing else. That is the entire safety
/// argument for letting Swarm answer an ask on the user's behalf, so most of this file is about
/// the things that must *not* match: a prefix, a different case, a trimmed space, a resolved path.
/// Every one of those would be Swarm deciding that two rules in a syntax the CLI owns mean the
/// same thing.
...
struct PermissionGrantTests {
    ...
    /// The wildcard works because it is in the string the CLI composed, not because Swarm knows
    /// what a star means. Two different invocations that the CLI describes with the same rule
    /// match each other; Swarm never expands anything.
    ...
    /// Every one of these is a way Swarm could have decided two rules mean the same thing. None
    ...
    /// The CLI's two flags bind Swarm as hard as they bind the buttons. An ask the user was never
    ...
    func stored() async throws {
        ...
        let repo = try await store.upsert(Repo(name: "Swarm", path: "/tmp/swarm-\(newID())"))
    ...
    func grantsAreScopedToTheirProject() async throws {
        let store = try makeTestStore()
        let swarm = try await store.upsert(Repo(name: "Swarm", path: "/tmp/swarm-\(newID())"))
        ...
        #expect(try await store.permissionGrants(repoID: swarm.id).count == 1)
...
/// call is aborted, or until stdin closes. So a blocked agent stays blocked for as long as Swarm
/// takes, and a Swarm that forgot the question on quit would come back to a transcript that simply
...
struct PendingPermissionAskTests {
    ...
    /// A row Swarm cannot read is a question it cannot draw. Skipping it beats an ask with no
...
/// The block that used to be copied into both runners, under identical comments. It is Swarm's own
...
struct PermissionGrantWritingTests {
    ...
    /// The CLI offering no rule is the case a path outside the worktree produces. Swarm does not
