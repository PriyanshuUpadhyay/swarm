...
/// whether Swarm is allowed to draw a button that widens a scope.
...
struct PermissionAskTests {
    ...
    /// The whole of approach 4 rests on this string being the CLI's rather than Swarm's.
    ...
    /// all. Swarm does not compose one to fill the gap.
    @Test("no suggestion means no rule, not a rule Swarm made up")
    func noSuggestion() {
        let ask = Self.ask(Self.realAsk.replacingOccurrences(
    ...
    /// them as separate options; Swarm offers the one the ask is about.
    ...
    @Test("a control request Swarm has no business answering stays unknown")
    ...
    @Test("the CLI answering Swarm is not a question")
    ...
    func allowForSession() throws {
        ...
        // And the rules themselves are untouched: the destination is the only thing Swarm changes.
        #expect(sent?["rules"] == ask.allowSuggestion?.raw["rules"])
    }

    /// Project scope is Swarm's own bookkeeping. On the wire it is identical to session scope,
    /// because the only thing the live CLI needs to know is "stop asking me this for now". Where
    /// the grant survives to is Swarm's business, and involves no file.
    ...
    func scopeCopy() {
        for scope in PermissionScope.allCases {
            #expect(!scope.consequence(rule: "Bash(bin/test:*)", project: "Swarm").isEmpty)
            #expect(!scope.buttonLabel.isEmpty)
        }

        #expect(PermissionScope.project.consequence(rule: "R", project: "Swarm").contains("Swarm"))
        #expect(PermissionScope.session.consequence(rule: "R", project: "Swarm").contains("R"))
    }
}
