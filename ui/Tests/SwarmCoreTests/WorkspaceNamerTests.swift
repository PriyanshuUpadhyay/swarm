...
struct WorkspaceNamerTests {
    ...
    func promptRenders() {
        let prompt = WorkspaceNamer.prompt(task: "Add a dark mode toggle", project: "Swarm")
        #expect(prompt.contains("Add a dark mode toggle"))
        #expect(prompt.contains("Swarm"))
    ...
    func promptCapsTheTask() {
        ...
        let prompt = WorkspaceNamer.prompt(task: huge, project: "Swarm")
    ...
    func failedProcess() async {
        ...
        let suggestion = await namer.suggest(
            task: "task", project: "Swarm", template: "{{task}}"
        )
    ...
    func emptyTemplate() async {
        ...
        _ = await namer.suggest(task: "   ", project: "Swarm", template: "{{task}}")
