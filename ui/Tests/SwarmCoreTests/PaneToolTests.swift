...
struct PaneToolTests {
    ...
    func aBrowserKeepsItsURL() {
        #expect(
            read(kind: "browser", url: "https://runswarm.app")
                == .order(PaneOrder(kind: .browser, url: "https://runswarm.app"))
        )
    }
    ...
    func refusesSchemesTheOwnerDidNotChoose() {
        ...
        for url in ["http://localhost:3000", "https://runswarm.app"] {
    ...
    func theConfirmationSaysWhereItWent() {
        ...
        #expect(
            PaneOrder(kind: .browser, url: "https://runswarm.app").confirmation
                .contains("https://runswarm.app")
        )
    }
    ...
    /// no `workspaceID` by definition: it is the person reaching Swarm from a client of their own,
    ...
    @Test("both are Swarm's own tools and need no confirmation")
...
/// How Swarm's own bridge calls read in a transcript.
///
...
@Suite("Swarm's own tools, as a reader meets them")
struct SwarmBridgePresentationTests {
    ...
    @Test("Swarm's own tools do not wear the extension glyph")
    func swarmHasItsOwnGlyph() {
