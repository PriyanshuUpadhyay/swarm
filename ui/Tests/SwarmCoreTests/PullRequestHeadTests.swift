...
/// cut from `origin/main` itself, and Swarm was asking gh about the branch written down when the
...
struct PullRequestHeadTests {
    ...
    func fallsBackWhenDetached() {
        ...
        #expect(
            PullRequestHead.branch(recorded: "swarm/preview-crash", checkedOut: nil, base: "main")
                == "swarm/preview-crash"
        )
        #expect(
            PullRequestHead.branch(recorded: "swarm/preview-crash", checkedOut: "  ", base: "main")
                == "swarm/preview-crash"
        )
    }
    ...
    func refusesTheBaseBranch() {
        ...
        #expect(
            PullRequestHead.branch(recorded: "swarm/preview-crash", checkedOut: "main", base: "main")
                == "swarm/preview-crash"
        )
    }
...
struct PullRequestHeadBranchTests {
    ...
    func readsTheBranchOffDisk() async throws {
        ...
        let workspace = Workspace(
            repoID: RepoID.new(),
            name: "Preview crash",
            branch: "swarm/preview-crash",
            path: repo.path,
            baseBranch: "main"
        )

        // Standing on the base branch, so the recorded name is what gets asked about.
        #expect(await GitHub.headBranch(of: workspace) == "swarm/preview-crash")
