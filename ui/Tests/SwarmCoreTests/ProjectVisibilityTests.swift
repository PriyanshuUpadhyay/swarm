...
struct ProjectVisibilityTests {
    ...
    private let swarm = Repo(name: "swarm", path: "/Users/me/dev/swarm", sortOrder: 1, hidden: true)
    ...
    func filtering() {
        #expect(ProjectVisibility.listed([flare, swarm, site], showingHidden: false).map(\.name)
            == ["flare", "site"])
        #expect(ProjectVisibility.listed([flare, swarm, site], showingHidden: true).map(\.name)
            == ["flare", "swarm", "site"])
    }
    ...
    func orderIsUntouched() {
        let shown = ProjectVisibility.listed([flare, swarm, site], showingHidden: true)

        #expect(shown.map(\.id) == [flare.id, swarm.id, site.id])
    }

    @Test("the count is what the toggle says out loud")
    func counting() {
        #expect(ProjectVisibility.hiddenCount([flare, swarm, site]) == 1)
...
struct ProjectReturnTests {
    ...
    func theRule() {
        let hidden = Repo(name: "swarm", path: "/Users/me/dev/swarm", hidden: true)
...
struct ProjectReturnStoreTests {
    ...
    func writesOneColumn() async throws {
        ...
        let project = try await store.upsert(
            Repo(name: "swarm", path: "/tmp/swarm", sortOrder: 3, hidden: true)
        )
        ...
        #expect(!stored.hidden)
        #expect(stored.name == "Swarm")
        #expect(stored.iconPath == "/tmp/swarm/icon.png")
