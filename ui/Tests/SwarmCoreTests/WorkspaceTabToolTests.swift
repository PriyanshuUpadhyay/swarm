...
/// both `@MainActor` singletons in `Sources/Swarm`, which `Tests/SwarmCoreTests` does not depend
...
struct WorkspaceTabToolTests {
    ...
    private func browser(
        _ number: Int, title: String = "Swarm", address: String = "http://localhost:3000"
    ) -> WorkspaceTabReport {
        WorkspaceTabReport(
            number: number,
            title: title,
        ...
        [
            ...
            chat(
                ...
                panes: [
                    ...
                    WorkspaceTabPane(kind: .browser, title: "Swarm", browser: 1),
                ]
            ),
            terminal(3),
            review(4, file: "Sources/SwarmCore/Bridge/PaneCensus.swift"),
        ]
    }
    ...
    func aReviewNamesItsFile() {
        guard case .object(let onFile) = review(1, file: "Sources/Swarm/App.swift").json,
              case .object(let file)? = onFile["review"] else {
            Issue.record("expected a review block"); return
        }
        #expect(file["file"] == .string("Sources/Swarm/App.swift"))
    ...
    // MARK: - What Swarm answers for itself
