...
struct SettingsEntriesTests {
    private func makeRepo(_ files: [String: String] = [:]) throws -> String {
        let root = TestScratch.unique("swarm-entries")
    ...
    func absentAndScriptFilesOnly() throws {
        ...
        let repo = try makeRepo([
            ...
            ".swarm/archive.sh": "#!/bin/zsh\ndocker compose down\n",
        ])
        let settings = SettingsLoader.load(repo: repo)
        #expect(settings.setupScript == "#!/bin/zsh\npnpm install\n")
        #expect(settings.archiveScript == "#!/bin/zsh\ndocker compose down\n")
        #expect(settings.scriptFiles[.setup] == ScriptFile(path: ".swarm/setup.sh", isMissing: false))
    ...
    func malformedFile() throws {
        ...
        #expect(!settings.sources.contains(path(repo, ".swarm/settings.toml")))
        let found = issues(settings, in: repo)
        #expect(found.count == 1)
        #expect(found.first?.path == path(repo, ".swarm/settings.toml"))
    ...
    func missingRunScriptFile() throws {
        let repo = try makeRepo([".swarm/settings.toml": "[scripts.run.dev]\nfile = \".swarm/gone.sh\"\n"])
        let settings = SettingsLoader.load(repo: repo)
        ...
        #expect(issues(settings, in: repo).map(\.message) == [
            "Run script \u{201C}dev\u{201D} cannot run: .swarm/gone.sh does not exist.",
        ])
    }
    ...
    func validQuickPrompts() throws {
        ...
        #expect(settings.quickPrompts == [
            ProjectQuickPrompt(
                ...
                symbol: "terminal", source: path(repo, ".swarm/settings.toml")
            ),
            ProjectQuickPrompt(
                name: "Write the changelog", text: "Write a changelog entry.",
                opensNewChat: true, source: path(repo, ".swarm/settings.toml")
            ),
        ])
    ...
    func unknownSymbolFallsBack() throws {
        let repo = try makeRepo([
            ".swarm/settings.toml": "[[quick_prompts]]\nname = \"A\"\nprompt = \"B\"\nsymbol = \"not.a.symbol\"\n",
        ])
        let settings = SettingsLoader.load(repo: repo)
    ...
    func quickPromptLayering() throws {
        ...
        let layered = try makeRepo([
            ...
            ".swarm/settings.toml": "[[quick_prompts]]\nname = \"Swarm\"\nprompt = \"Swarm words.\"\n",
            ".swarm/settings.local.toml": "[git]\nbranch_prefix = \"freek\"\n",
        ])
        #expect(SettingsLoader.load(repo: layered).quickPrompts.map(\.name) == ["Swarm"])

        let cleared = try makeRepo([
            ".swarm/settings.toml": shared,
            ".swarm/settings.local.toml": "quick_prompts = []\n",
        ])
    ...
    func homeFilesStateNoQuickPrompts() throws {
        ...
        SettingsLoader.apply(toml, from: "/home/.swarm/settings.toml", to: &settings)
        #expect(settings.quickPrompts.isEmpty)
    }
}
