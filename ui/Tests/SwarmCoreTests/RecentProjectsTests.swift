...
struct RecentProjectsTests {
    ...
    private let swarm = Repo(name: "swarm", path: "/swarm", sortOrder: 1, hidden: true)
    ...
    func newestFirst() {
        let workspaces = [
            ...
            workspace(in: swarm, at: 200),
            workspace(in: flare, at: 300)
        ]

        #expect(RecentProjects.ordered([flare, swarm, site], workspaces: workspaces).map(\.name)
            == ["flare", "swarm"])
    }
