...
struct WorkspaceCheckoutTests {
    ...
    /// It used to be dropped as the branch the project's checkout is on, and somebody looking for
    /// main on the Existing branch tab was told nothing matched. It leads, marked when it is taken.
    ...
    func offersTheDefaultBranchFirst() {
        ...
        #expect(branches.first?.inUseBy == .projectCheckout(path: "/dev/flare"))
    }
    ...
    func skipsBranchesWithAnOpenPullRequest() {
        ...
        // The default branch leads, as it does everywhere on this tab since it was offered there.
    ...
    func keepsExcludingWhatAPullRequestSpeaksFor() {
        ...
        // The default branch is offered and wears what holds it; the pull request's head is not.
    ...
    func doesNotReachIntoAnotherProject() {
        ...
        let here = Workspace(
            repoID: mine, name: "Swarm develop", branch: "develop",
            path: "/tmp/swarm", baseBranch: "main"
        )
