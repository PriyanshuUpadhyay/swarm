...
struct BrowserPaneToolTests {
    private func report(
        _ number: Int, address: String = "http://localhost:3000", name: String = "Swarm"
    ) -> BrowserPaneReport {
    ...
    func severalNeedANumber() {
        let browsers = [
            ...
            report(2, address: "https://runswarm.app"),
        ]
        ...
        #expect(refusal.sentence.contains("2 on https://runswarm.app"))
    }
    ...
    /// browser pane cannot come to disagree about what Swarm will open on the owner's behalf.
    ...
    func goTakesTheTwoSchemes() {
        ...
        #expect(
            (try? BrowserGoTool.address("  https://runswarm.app ").get()) == "https://runswarm.app"
        )
    }
        ...
        let census = PaneCensus(entries: [
            ...
            PaneCensusEntry(
                ...
                browser: BrowserPaneReport(
                    number: 1, name: "Swarm", address: "http://localhost:3000", isLoading: true
                )
            ),
    ...
    func theNoteFollowsTheBrowsers() {
        let withBrowser = PaneCensus(entries: [
            PaneCensusEntry(
                ...
                name: "Swarm",
                isShowing: true,
                browser: BrowserPaneReport(number: 1, name: "Swarm", address: "http://x.test")
            ),
        ])
    ...
    /// Reading the window's own furniture is answered by Swarm. Anything that moves a page, or
    /// carries one off this machine, is answered by the person.
    @Test("only the two that report the chrome are self-approved")
    func whatSwarmAnswersForItself() {
