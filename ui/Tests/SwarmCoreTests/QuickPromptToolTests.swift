...
@Suite("Asking Swarm about quick prompts")
struct QuickPromptToolArgumentTests {
    ...
    @Test("a mark Swarm cannot draw is refused, with marks that work")
    ...
    @Test("every mark the refusal recommends is one Swarm can draw")
...
/// Who may reach the owner's library, and which of the four Swarm answers for itself.
...
struct QuickPromptToolRoleTests {
    ...
    ///
    /// `.workspace` keeps the two that cannot lose anything, because the owner mostly talks to Swarm
    /// from inside Swarm and "save that as a quick prompt" is typed into a workspace chat. It does
    ...
    @Test("Swarm answers for the listing and for none of the other three")
    func selfApproval() {
...
struct QuickPromptToolCallTests {
    ...
    @Test("an update to an id Swarm does not have changes nothing and lists what it has")
    ...
    func deleteCarriesThePromptBack() async throws {
        ...
        // on is Swarm's own built-in, seeded by the read this tool does before it deletes.
    ...
    func listsEverything() async throws {
        ...
        // Swarm's own built-in is seeded by the read, so the count is the two written here plus
        // whatever Swarm ships with. See `QuickPromptCall`.
