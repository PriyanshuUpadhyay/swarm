...
struct WorkspaceMergeToolTests {
    ...
    @Test("a merge method Swarm does not have is refused by name rather than merged some other way")
    ...
    /// Nothing in Swarm resolves a workspace by name, because two are allowed to share one. So a
    ...
    @Test("gh's own words reach the caller, and the command line Swarm built does not")
    ...
    @Test("the workspace and the pull request reach the app exactly as Swarm read them")
