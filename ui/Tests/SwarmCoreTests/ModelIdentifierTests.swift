...
/// with its backend written in front of it. Nothing in Swarm composes that string; it arrives
...
struct ModelIdentifierTests {
    ...
    /// A colon whose left side is not a backend Swarm has. Some other tool's namespace is not
    /// Swarm's to unpick, and the open-set rule stands: keep it, show it, let it be changed.
