...
///
/// `SwarmCommands` is a `Commands` body, which is a view, so before this table existed none of
...
struct MenuBarCatalogueTests {
    ...
    /// Every menu Swarm claims to fill has something in it. An empty menu title in the bar is a
