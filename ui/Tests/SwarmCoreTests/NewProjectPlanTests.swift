...
struct NewProjectVerdictTests {
    ...
    @Test("an existing folder Swarm cannot write to is refused, and names itself")
    ...
    @Test("Swarm's own worktree folder is refused")
    func refusesTheWorkspacesRoot() {
        let verdict = NewProjectVerdict.of(
            facts(location: "~/swarm/workspaces", path: "/Users/tester/swarm/workspaces/sparkline")
        )
        #expect(verdict == .refuse(.insideSwarmsWorkspaces("~/swarm/workspaces/sparkline")))
    }
    ...
    /// A project one level inside Documents is the owner's own filing and none of Swarm's
    ...
    func everyRefusalSpeaks() {
        let all: [NewProjectRefusal] = [
            ...
            .insideRepository("~/y"), .insideSwarmsWorkspaces("~/z"),
