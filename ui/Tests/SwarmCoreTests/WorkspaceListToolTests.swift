...
struct WorkspaceListToolTests {
    ...
    @Test("it is served by a Swarm with no app behind it, because it needs no seam into one")
    ...
    @Test("an empty Swarm says so rather than answering with nothing")
    func emptySwarm() async throws {
    ...
    func theFields() async throws {
        ...
        // The path is the point: it is what lets the caller stop asking Swarm for a diff.
    ...
    func narrowingByProject() async throws {
        ...
        let swarm = try await store.upsert(
        ...
        _ = try await store.upsert(Workspace(
            repoID: swarm.id, name: "in swarm", branch: "b2", path: "/tmp/w2", baseBranch: "main"
        ))
        ...
        let byPath = await WorkspaceListTool().call(
            request(["project": .string("/tmp/swarm/")]), as: .owner, store: store
        )

        #expect(try workspaces(byName).map { $0["name"]?.stringValue } == ["in flare"])
        #expect(try workspaces(byPath).map { $0["name"]?.stringValue } == ["in swarm"])
    }

    /// Refused in `BridgeProjectLookup`'s existing words, which name what Swarm does have. A
    /// caller told only "no such project" guesses, and every guess is another call.
    @Test("a project Swarm does not have is refused, and told what it does have")
