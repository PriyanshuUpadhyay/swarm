...
struct BridgeUserRegistrationTests {
    private let attachment = BridgeAttachment(
        shimPath: "/Applications/Swarm.app/Contents/MacOS/swarm-bridge",
    ...
    private func state(_ data: Data?) -> BridgeUserRegistration.State {
        BridgeUserRegistration.state(userConfig: data, serverNamed: "swarm", matching: attachment)
    }

    @Test("An entry naming this Swarm, this socket and this token is the offer already taken")
    ...
    func ignoresTheRole() throws {
        ...
        let data = try config(["swarm": entry(role: "parent")])
        #expect(state(data) == .registered)
    }
    ...
    func theLegacyName() throws {
        // `swarm-owner-bridge` was one constant for every copy of Swarm at once, so an entry under
        // it may be pointing at a build that has gone. See `BridgeRegistration.ownerServerName`.
        let data = try config(["swarm-owner-bridge": entry()])
    ...
    func stale() throws {
        ...
        let elsewhere = try config(["swarm": entry(command: "/Volumes/Old/swarm-bridge")])
        let otherSocket = try config(["swarm": entry(socket: "/tmp/swarm-bridge-other.sock")])
        let otherToken = try config(["swarm": entry(token: "deadbeef")])
        let noEnvironment = try config(["swarm": ["command": attachment.shimPath]])
    ...
    func regenerated() throws {
        ...
        let data = try config(["swarm": entry(token: old.token)])
    ...
    @Test("The name looked for is the one the command registers, per copy of Swarm")
    func namedPerCopy() throws {
        // The dev build's entry is not the owner build's, which is the whole reason the name is
        // derived. A window that looked for one name would tell Swarm Dev it was already set up.
