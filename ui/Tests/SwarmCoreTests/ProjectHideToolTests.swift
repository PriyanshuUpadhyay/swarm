...
struct ProjectHideToolTests {
    ...
    private func seed(_ store: Store) async throws -> (Repo, Repo) {
        ...
        let swarm = try await store.upsert(Repo(name: "swarm", path: "/tmp/swarm", sortOrder: 1))
        return (flare, swarm)
    }
    ...
    @Test("neither is answered by Swarm on the owner's behalf")
    ...
    func hidingEverything() async throws {
        ...
        _ = await tool.call(request("project_hide", "flare"), as: .owner, store: store)
        let last = await tool.call(request("project_hide", "swarm"), as: .owner, store: store)
    ...
    @Test("an unknown name names what Swarm does have and says retrying will not help")
