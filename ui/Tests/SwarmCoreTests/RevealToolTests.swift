...
struct RevealToolTests {
    ...
    func refusesBothAtOnce() {
        for extra in [("project", "swarm"), ("search", "flake"), ("scope", "running")] {
    ...
    func findsWorkspace() throws {
        ...
        #expect(byName.sentence.contains("swarm"))
    ...
    func homeFilter() throws {
        ...
        let reveal = try resolve(
            try parse(project: "swarm", scope: "archived", search: "parser"),
        ...
        #expect(reveal.sentence.contains("swarm"))
    ...
    @Test("Swarm answers its own question for it")
    func selfApproved() {
    ...
    func callsThrough() async throws {
        ...
        let repo = try await store.upsert(Repo(name: "swarm", path: "/tmp/swarm"))
    ...
    private func world() -> ([Workspace], [Repo]) {
        let swarm = Repo(name: "swarm", path: "/tmp/swarm")
        let mailcoach = Repo(name: "mailcoach", path: "/tmp/mailcoach")
        return (
            [
                Workspace(
                    repoID: swarm.id, name: "Fix the flake", branch: "b1", path: "/p1",
                    baseBranch: "main"
                ),
                Workspace(
