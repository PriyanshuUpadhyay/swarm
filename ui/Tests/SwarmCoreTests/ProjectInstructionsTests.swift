...
/// What a project adds to the two turns Swarm composes about landing a branch, and what a turn
...
/// every repository, because Swarm wrote its own instructions to disk in order to attach them
/// back.
...
struct ProjectInstructionsTests {
    ...
    func silentByDefault(subject: ProjectInstructions.Subject) throws {
        ...
        // Nothing was written either. The whole point of taking Swarm's own words out of a file is
    ...
    /// The whole reason the spilled copy is rewritten rather than kept. Swarm's own instructions
    ...
    /// against the real git binary. Swarm's own file went out in a user's pull request once, and
    ...
    /// project asked for. Swarm's own steps arrive either way now, because they are in the words.
    ...
    @Test("the merge turn carries Swarm's rules whatever the project says")
    ...
    @Test("resolving a conflict adds no rules of Swarm's own here")
    func conflictsAddNothingOfSwarmsOwn() {
    ...
    func subjectsAreNotConfused() {
        ...
        #expect(ProjectInstructions.projectPath(for: .merge) == ".swarm/merge-instructions.md")
        #expect(ProjectInstructions.projectPath(for: .fixConflicts)
            == ".swarm/conflict-instructions.md")
    }
