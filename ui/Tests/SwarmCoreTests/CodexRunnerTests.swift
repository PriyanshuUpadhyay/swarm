...
@Suite(.scratchDirectory) struct CodexRunnerTests {
    ...
    @Test("Ask Swarm sends host instructions when starting and resuming Codex",
          arguments: [false, true], [false, true])
    func askSwarmInstructions(hasWorkspace: Bool, resumed: Bool) async throws {
    ...
    /// The bug a user reported: Swarm's Codex menu had no row for the preset the Codex app calls
    ...
    @Test func replaysARecordedTurnIntoTheTranscript() async throws {
        ...
        try await runner.send("Reply with exactly one word: swarm")
        for line in try swarmFixtureLines("codex-turn.ndjson") {
    ...
    @Test func aStoppedTurnIsCancelledRatherThanFailed() async throws {
        ...
        for line in try swarmFixtureLines("codex-interrupt.ndjson") {
    ...
    @Test func asksTheQuestionTheServerAskedAndAnswersIt() async throws {
        ...
        for line in try swarmFixtureLines("codex-approval.ndjson") {
    ...
    @Test func aStoredGrantAnswersTheQuestionWithoutAsking() async throws {
        ...
        let paths = try swarmFixtureLines("codex-approval.ndjson")
    ...
    /// away, because `acceptForSession` is remembered by the process rather than by Swarm.
    ...
    /// it as "Swarm was not running when this was asked", which is not what happened.
