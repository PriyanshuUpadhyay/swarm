...
/// Swarm asks the coding agent to open pull requests rather than shelling out to `gh` itself, so
...
struct PromptRegistryTests {
    ...
    func createPullRequestDefaultRendersFully() throws {
        ...
        let context = PullRequestPromptContext(
            ...
            changes: "- Sources/SwarmCore/PromptTemplate.swift (added, +90/-0)"
        )
...
struct PullRequestInstructionsTests {
    @Test("Swarm's own copy is written into the shielded scratch folder, not next to the user's work")
    ...
    /// commit it". An agent obeying that reaches for `git add -A`, and Swarm's scratch file went
    /// out in a user's pull request and was merged. A unit test on a path string would not have
    /// caught it. This does: after `add -A` there must be nothing of Swarm's staged.
    @Test("an agent told to commit everything cannot commit Swarm's copy")
    ...
    /// A repository that committed Swarm's default before the bug was found, which is exactly
    /// what happened. Deleting it would show up as a deletion in every workspace cut from that
    /// repository and would be committed by the next agent told to commit what it finds. Swarm
    ...
    /// An untracked copy an older Swarm left behind is the one thing that may be moved, because
    /// nothing but Swarm could have written it and moving it is not a deletion in anybody's diff.
    @Test("an untracked copy of a default Swarm shipped is reclaimed into the scratch folder")
    ...
    /// and this is the only attachment Swarm makes for itself: nobody picked the file, so nothing
    ...
    func namesAFileThatIsThere(setup: InstructionsSetup) async throws {
        ...
        // And the guarantee that must survive all of this: nothing of Swarm's reaches the commit,
    ...
    /// a `.swarm/pr-instructions.md` folder would be attached to the prompt and the agent told to
...
enum InstructionsSetup: String, Sendable, CaseIterable, CustomStringConvertible {
    ...
    case projectOwn
    /// An untracked copy of a default an older Swarm left behind, which is the one file that may
    /// be moved into the scratch folder.
    case strayDefault
    /// A repository that committed Swarm's default back when the bug put it there. It stays
    /// exactly where it is.
    ...
    func lay(out repo: TempRepo) async throws {
        ...
        case .strayDefault:
            // A retired default by preference, because recognising what an older Swarm wrote is
...
struct PromptOverridesTests {
    ...
    private func makeDefaults() throws -> (UserDefaults, String) {
        let name = "swarm.prompts.\(UUID().uuidString)"
