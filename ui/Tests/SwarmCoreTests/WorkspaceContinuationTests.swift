...
struct ContinuationGateTests {
    /// A workspace as it stands the moment its pull request goes in: still on the branch Swarm
...
struct WorkspaceContinuationTests {
    ...
    private func makeMergedWorkspace() async throws -> (
        ...
        let clonePath = TestScratch.unique("swarm-clone")
        try await Shell.check("git", ["clone", "-q", "--", origin.path, clonePath], cwd: origin.path)
        let repo = TempRepo(existing: clonePath)
        try await Shell.check("git", ["config", "user.email", "test@swarm.local"], cwd: repo.path)
        try await Shell.check("git", ["config", "user.name", "Swarm Test"], cwd: repo.path)
    ...
    func continuesInPlace() async throws {
        ...
        // The name is not touched. Swarm names a workspace from what was asked for, and nobody has
    ...
    func continuesFromTheAgentsOwnBranch() async throws {
        ...
        // named branch off the one Swarm made and did the work there. Nothing writes that back to
