...
/// which is the half most likely to be wrong: whether `--mcp-config` pointed at a file Swarm wrote
...
///
///     SWARM_UI_LIVE=1 ./Tools/test-core.sh LiveBridge
///
...
struct LiveBridgeTests {
    ...
    func callsWhoami() async throws {
        ...
        let workspace = try await store.upsert(Workspace(
            ...
            branch: "swarm/cut-the-invoice-index",
            path: repo.path,
            baseBranch: "main",
...
final class LiveBridgeLog: @unchecked Sendable {
    ...
    var bridgeTools: [String] {
        ...
        return initTools.filter { $0.lowercased().contains("swarm") }
    }
