...
struct EditorCatalogTests {
    ...
    private func preferences() -> (OpenInPreferences, UserDefaults) {
        ...
        // menu does in the copy of Swarm the user is running.
        let name = "swarm.tests.openIn.\(UUID().uuidString)"
    ...
    func aRepoRemembersItsOwn() {
        ...
        preferences.record("com.apple.dt.Xcode", repo: RepoID("swarm"))

        #expect(preferences.lastUsed(repo: RepoID("laravel")) == "com.jetbrains.PhpStorm")
        #expect(preferences.lastUsed(repo: RepoID("swarm")) == "com.apple.dt.Xcode")
    }
