...
struct TerminalCommandMemoryTests {
    ...
    @Test("What Swarm sent wins over what the machine calls the same process")
    func sentWins() {
    ...
    func fallsBackToTheMachine() {
        #expect(TerminalCommandMemory.remembered(sent: nil, running: "php artisan serve")
            == "php artisan serve")
        // Swarm's own text is put through the same rules, so an unusable one falls through rather
