...
struct AgentCatalogTests {
    ...
    func rejectsBrokenOverride() async {
        let missing = TestScratch.unique("swarm-no-such-agent")
    ...
    func resolvesConfigPaths() throws {
        let base = TestScratch.unique("swarm-agentcat")
...
///
///     SWARM_UI_LOCAL_AGENTS=1 ./Tools/test-core.sh AgentCatalogLocal
private let localAgentsEnabled = ProcessInfo.processInfo.environment["SWARM_UI_LOCAL_AGENTS"] == "1"
