...
struct DocumentPreviewTests {
    ...
    func containment() throws {
        let root = try worktree()
        let outside = try #require(URL(string: "swarm-preview://worktree/../outside.txt"))
        #expect(DocumentPreview.file(for: outside, root: root) == nil)
        let encoded = try #require(URL(string: "swarm-preview://worktree/docs/%2E%2E/%2E%2E/%2E%2E/etc/passwd"))
    ...
    func symlink() throws {
        ...
        let address = try #require(URL(string: "swarm-preview://worktree/docs/link.txt"))
    ...
    func folderIndex() throws {
        ...
        let address = try #require(URL(string: "swarm-preview://worktree/docs/"))
    ...
    func sameDocument() throws {
        let root = try worktree()
        #expect(try decide("swarm-preview://worktree/docs/report.html", root: root, clicked: false) == .allow)
        #expect(try decide("swarm-preview://worktree/docs/report.html#costs", root: root) == .allow)
    }

    @Test("a click on another worktree file opens it in Swarm, a script moving the page does not")
    func otherFile() throws {
        let root = try worktree()
        #expect(try decide("swarm-preview://worktree/docs/plan.md", root: root) == .openFile(root + "/docs/plan.md"))
        #expect(try decide("swarm-preview://worktree/docs/plan.md", root: root, clicked: false) == .refuse)
        #expect(try decide("swarm-preview://worktree/docs/plan.md", root: root, mainFrame: false, clicked: false) == .allow)
    }

    @Test("a web link opens outside Swarm only when clicked, and a frame may embed the web")
