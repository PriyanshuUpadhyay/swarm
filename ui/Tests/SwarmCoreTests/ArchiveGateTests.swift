...
struct ArchiveGateTests {
    ...
    /// exist and leaves `repos.path` real, so archiving in Swarm Dev deleted a branch in the
    ...
    func aMissingWorktreeKeepsItsBranch() async throws {
        ...
        // Remove the worktree behind Swarm's back, which is the state a detached dev database is
