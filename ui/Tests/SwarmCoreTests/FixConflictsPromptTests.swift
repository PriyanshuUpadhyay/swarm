...
/// Swarm's own steps for resolving a conflict, and the file they go in.
///
...
struct ConflictInstructionsTests {
    ...
    /// `ProjectInstructions` already writes `.swarm/scratch/conflict-instructions.md` for the
    ...
    @Test("Swarm's file and the project's spilled settings are two different files")
    ...
    @Test("an agent told to commit the resolution cannot commit Swarm's file")
