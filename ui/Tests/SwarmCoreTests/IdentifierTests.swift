...
/// What a typed identifier has to keep doing, now that every id in Swarm is one.
///
...
struct IdentifierTests {
    /// The conformance the whole change rests on, and it is not Swarm's.
    ///
    ...
    /// copied field for field: it is the only value Swarm stores as JSON with an id inside it, and
