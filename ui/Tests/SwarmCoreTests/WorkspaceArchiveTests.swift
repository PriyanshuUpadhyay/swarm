...
/// `WorkspaceManager.archive` is the only thing in Swarm that deletes a git worktree and can
...
struct WorkspaceArchiveTests {
    ...
    private func commit(in worktree: String, message: String) async throws {
        ...
        try await Shell.check("git", [
            "-c", "user.email=test@swarm.local", "-c", "user.name=Swarm Test",
    ...
    /// The archive script used to be handed `SWARM_UI_PORT=0`, whatever block the workspace was
    ...
    func theArchiveScriptSeesTheWorkspacePort() async throws {
        let (repo, registered, manager, workspace) = try await makeWorkspace(settings: """
        [scripts]
        archive = 'printf %s "$SWARM_UI_PORT" > "$SWARM_UI_ROOT_PATH/archived-on-port"; printf %s "$CONDUCTOR_PORT" > "$SWARM_UI_ROOT_PATH/archived-on-alias"'
        """)
        defer { repo.cleanUp() }
    ...
    func theArchivePortComesFromTheRow() async throws {
        let (repo, registered, manager, workspace) = try await makeWorkspace(settings: """
        [scripts]
        archive = 'printf %s "$SWARM_UI_PORT" > "$SWARM_UI_ROOT_PATH/archived-on-port"'
        """)
        defer { repo.cleanUp() }
    ...
    func archivesAnUnrecognizedFolder(force: Bool, prune: Bool) async throws {
        ...
        delete_branch_on_archive = true
        [scripts]
        archive = 'touch "$SWARM_UI_ROOT_PATH/archive-script-ran"; exit 9'
        """)
        defer { repo.cleanUp() }
    ...
    func ignoredFilesAreInvisibleToTheReport() async throws {
        ...
        // Swarm itself copies `.env*` into every new worktree, so this is the file most likely to
        ...
        // deletes them silently. Swarm copies `.env*` into every worktree, so this was the
