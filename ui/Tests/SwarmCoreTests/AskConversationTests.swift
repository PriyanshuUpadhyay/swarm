...
@Suite("Ask Swarm's conversation", .tags(.persistence), .scratchDirectory)
struct AskConversationTests {
    /// The permission decision, not the tidiness one. Swarm's default mode is `acceptEdits`, and a
    ...
    func directoryIsBesideTheDatabase() {
        let directory = AskConversation.directory(besideDatabaseAt: "/tmp/somewhere/swarm.sqlite")
        #expect(directory == "/tmp/somewhere/Ask")
        #expect(!directory.hasSuffix("swarm.sqlite"))
    }

    /// Beside the database rather than at a fixed path, so the separation between Swarm and Swarm
    ...
    @Test("two copies of Swarm get two directories")
    func devCopyIsSeparate() {
        let owner = AskConversation.directory(besideDatabaseAt: "/tmp/Swarm/swarm.sqlite")
        let dev = AskConversation.directory(besideDatabaseAt: "/tmp/Swarm Dev/swarm.sqlite")
        #expect(owner != dev)
    }

    @Test("the directory is made, and is empty")
    func directoryIsMadeEmpty() throws {
        let database = TestScratch.unique("ask-cwd") + "/swarm.sqlite"
    ...
    /// chat cannot do, in Swarm's own vocabulary ("this chat has no worktree"). It says what to ask
