...
struct MediaShowToolTests {
    ...
    @Test("only Swarm's own media tool becomes inline content")
    ...
    func containment() throws {
        let root = FileManager.default.temporaryDirectory
            .appending(path: "swarm-media-\(UUID().uuidString)", directoryHint: .isDirectory)
        let outside = FileManager.default.temporaryDirectory
            .appending(path: "swarm-outside-\(UUID().uuidString).png")
