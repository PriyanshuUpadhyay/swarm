...
struct LegacyDefaultsTests {
    /// Two throwaway domains standing in for `be.spatie.baton` and `be.spatie.swarm`.
    private func domains() -> (old: String, new: String, defaults: UserDefaults) {
        let stamp = UUID().uuidString
        let old = "swarm.test.legacy.\(stamp)"
        let new = "swarm.test.current.\(stamp)"
...
struct LegacyDatabaseTests {
    private func scratch() -> URL {
        URL(fileURLWithPath: TestScratch.unique("swarm-dbmigrate"))
    }
    ...
    func migrates() async throws {
        ...
        let destination = root.appendingPathComponent("Swarm/swarm.sqlite")
